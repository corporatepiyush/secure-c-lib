...
scl_error_t scl_concurrent_graph_init(scl_concurrent_graph_t *g, size_t vertex_count)
{
    if (!g) return SCL_ERR_NULL_PTR;
    if (vertex_count == 0) return SCL_ERR_INVALID_ARG;
    g->adj = calloc(vertex_count, sizeof(scl_concurrent_adj_node_t *));
...
void scl_concurrent_graph_destroy(scl_concurrent_graph_t *g)
{
    if (!g) return;
    spin_lock(&g->lock);
...
scl_error_t scl_concurrent_graph_add_edge(scl_concurrent_graph_t *g, size_t from, size_t to, int weight)
{
    if (!g) return SCL_ERR_NULL_PTR;
    if (from >= g->vertex_count || to >= g->vertex_count) return SCL_ERR_INVALID_INDEX;
    scl_concurrent_adj_node_t *node = malloc(sizeof(scl_concurrent_adj_node_t));
...
scl_error_t scl_concurrent_graph_remove_edge(scl_concurrent_graph_t *g, size_t from, size_t to)
{
    if (!g) return SCL_ERR_NULL_PTR;
...
bool scl_concurrent_graph_has_edge(const scl_concurrent_graph_t *g, size_t from, size_t to)
{
