...
scl_error_t scl_atomic_graph_init(scl_allocator_t *alloc, scl_atomic_graph_t *g, size_t vertex_count)
{
    if (!g) return SCL_ERR_NULL_PTR;
    if (vertex_count == 0) return SCL_ERR_INVALID_ARG;
    g->adj = scl_calloc(alloc, vertex_count, sizeof(scl_atomic_adj_node_t *), alignof(max_align_t));
...
void scl_atomic_graph_destroy(scl_allocator_t *alloc, scl_atomic_graph_t *g)
{
    ...
    for (size_t i = 0; i < g->vertex_count; i++) {
        scl_atomic_adj_node_t *cur = g->adj[i];
        while (cur) {
            scl_atomic_adj_node_t *next = cur->next;
            scl_free(alloc, cur);
            cur = next;
        }
    }
...
scl_error_t scl_atomic_graph_add_edge(scl_allocator_t *alloc, scl_atomic_graph_t *g, size_t from, size_t to, int weight)
{
    if (!g) return SCL_ERR_NULL_PTR;
    if (from >= g->vertex_count || to >= g->vertex_count) return SCL_ERR_INVALID_INDEX;
    scl_atomic_adj_node_t *node = scl_alloc(alloc, sizeof(scl_atomic_adj_node_t), alignof(max_align_t));
...
scl_error_t scl_atomic_graph_remove_edge(scl_allocator_t *alloc, scl_atomic_graph_t *g, size_t from, size_t to)
{
    if (!g) return SCL_ERR_NULL_PTR;
    if (from >= g->vertex_count) return SCL_ERR_INVALID_INDEX;
    spin_lock(&g->lock);
    scl_atomic_adj_node_t **prev = &g->adj[from];
    scl_atomic_adj_node_t *cur = g->adj[from];
    while (cur) {
        if (cur->to == to) {
            *prev = cur->next;
            scl_free(alloc, cur);
