...
static void test_cdlist_push_pop_front(scl_test_runner_t *tr) {
  ...
  int v = 42;
  err = scl_cdlist_push_front(alloc, &l, &v);
  SCL_EXPECT_OK(tr, err);
  SCL_EXPECT_EQ_SZ(tr, scl_cdlist_count(&l), 1);

  int out = 0;
  err = scl_cdlist_pop_front(alloc, &l, &out);
  SCL_EXPECT_OK(tr, err);
...
static void test_cdlist_push_pop_back(scl_test_runner_t *tr) {
  ...
  int v = 99;
  err = scl_cdlist_push_back(alloc, &l, &v);
  SCL_EXPECT_OK(tr, err);
  SCL_EXPECT_EQ_SZ(tr, scl_cdlist_count(&l), 1);

  int out = 0;
  err = scl_cdlist_pop_back(alloc, &l, &out);
  SCL_EXPECT_OK(tr, err);
...
static void test_cdlist_insert_remove_at(scl_test_runner_t *tr) {
  ...
  int v = 7;
  err = scl_cdlist_insert_at(alloc, &l, 0, &v);
  SCL_EXPECT_OK(tr, err);
  SCL_EXPECT_EQ_SZ(tr, scl_cdlist_count(&l), 1);

  int out = 0;
  err = scl_cdlist_remove_at(alloc, &l, 0, &out);
  SCL_EXPECT_OK(tr, err);
