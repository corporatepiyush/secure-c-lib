...
static void test_scalars(scl_test_runner_t *tr) {
    ...
    struct { const char *in; scl_parse_json_type_t ty; } cases[] = {
    ...
    for (size_t i = 0; i < SCL_ARRAY_SIZE(cases); i++) {
        ...
        if (v) SCL_EXPECT_EQ_I(tr, scl_parse_json_get_type(v), cases[i].ty);
...
static void test_object_array(scl_test_runner_t *tr) {
    ...
    const char *in = "{\"name\":\"x\",\"nums\":[10,20,30],\"ok\":true,\"f\":1.25}";
    ...
    if (root) {
        ...
        if (name) SCL_EXPECT_EQ_STR(tr, scl_parse_json_get_string(name), "x");
        ...
        if (nums) {
            ...
            SCL_EXPECT_EQ_I(tr, scl_parse_json_get_int(scl_parse_json_array_get(nums, 0)), 10);
            SCL_EXPECT_EQ_I(tr, scl_parse_json_get_int(scl_parse_json_array_get(nums, 2)), 30);
            SCL_EXPECT_NULL(tr, scl_parse_json_array_get(nums, 3));   /* OOB index */
        }
        SCL_EXPECT_EQ_I(tr, scl_parse_json_get_bool(scl_parse_json_object_get(root, "ok")), 1);
        SCL_EXPECT_TRUE(tr, scl_parse_json_get_double(scl_parse_json_object_get(root, "f")) == 1.25);
...
static void test_unicode_escapes(scl_test_runner_t *tr) {
    ...
    if (v) SCL_EXPECT_EQ_STR(tr, scl_parse_json_get_string(v), "\xc3\xa9\xe2\x82\xac");
    scl_parse_json_free(a, v);

    /* Surrogate pair for U+1F600 (😀) -> 4-byte F0 9F 98 80. */
    v = NULL;
    SCL_EXPECT_OK(tr, scl_parse_json_parse(a, "\"\\uD83D\\uDE00\"", &v));
    if (v) SCL_EXPECT_EQ_STR(tr, scl_parse_json_get_string(v), "\xf0\x9f\x98\x80");
    scl_parse_json_free(a, v);

    /* Lone high surrogate -> U+FFFD replacement (EF BF BD). */
    v = NULL;
    SCL_EXPECT_OK(tr, scl_parse_json_parse(a, "\"\\uD83D\"", &v));
    if (v) SCL_EXPECT_EQ_STR(tr, scl_parse_json_get_string(v), "\xef\xbf\xbd");
    scl_parse_json_free(a, v);
}
...
static void test_wide_tree_frees(scl_test_runner_t *tr) {
    ...
    size_t pos = 0;
    buf[pos++] = '[';
    for (size_t i = 0; i < n; i++) { buf[pos++] = '0'; if (i + 1 < n) buf[pos++] = ','; }
    buf[pos++] = ']';
    buf[pos] = '\0';

    scl_parse_json_value_t *root = NULL;
    SCL_EXPECT_OK(tr, scl_parse_json_parse(a, buf, &root));
    if (root) SCL_EXPECT_EQ_SZ(tr, scl_parse_json_array_len(root), n);
    scl_parse_json_free(a, root);
    free(buf);
}

/* ── Counting allocator: portable leak check (macOS ASan has no LSan) ──── */
typedef struct { scl_allocator_t *base; long live; } count_state_t;
static void *c_malloc(void *st, size_t sz, size_t al) {
    count_state_t *s = st; void *p = scl_alloc(s->base, sz, al); if (p) s->live++; return p;
}
static void *c_calloc(void *st, size_t n, size_t sz, size_t al) {
    count_state_t *s = st; void *p = scl_calloc(s->base, n, sz, al); if (p) s->live++; return p;
}
static void *c_realloc(void *st, void *ptr, size_t o, size_t n, size_t al) {
    count_state_t *s = st; void *p = scl_realloc(s->base, ptr, o, n, al);
    if (p && !ptr) s->live++;   /* realloc(NULL) behaves like malloc */
    return p;
}
static void c_free(void *st, void *ptr) {
    count_state_t *s = st; scl_free(s->base, ptr); s->live--;   /* only called for non-NULL */
}
