...
static int write_file(const char *tag, const unsigned char *data, size_t len, char *path, size_t cap) {
...
static size_t zip_add(unsigned char *buf, size_t pos, const char *name,
    ...
    put_le16(buf+pos+10, 0); put_le16(buf+pos+12, 0);
    ...
    memcpy(buf+pos, data, dlen); pos += dlen;
    return pos;
}
...
static void test_xlsx(scl_test_runner_t *tr) {
    ...
    size_t pos = 0;
    const char *wb = "<workbook><sheets><sheet name=\"Sheet1\" sheetId=\"1\"/></sheets></workbook>";
...
static void test_docx(scl_test_runner_t *tr) {
    ...
    if (txt) SCL_EXPECT_TRUE(tr, strstr(txt, "Hello World") != NULL);
...
static void test_pdf(scl_test_runner_t *tr) {
    ...
    char val[64]; size_t vlen = sizeof(val);
    ...
    size_t zero = 0;
    SCL_EXPECT_TRUE(tr, scl_parse_pdf_get_info(&p, "Title", val, &zero) == SCL_ERR_INVALID_ARG);
...
static void test_icelake(scl_test_runner_t *tr) {
    ...
    if (!mkdtemp(dir)) { SCL_EXPECT_TRUE(tr, 0); return; }
    ...
    if (f) {
        const char *json =
            "{\"snapshot-id\":12345,"
            "\"manifests\":[{\"manifest-path\":\"m1.avro\"},{\"manifest-path\":\"m2.avro\"}],"
    ...
    const char *dp = NULL; size_t dl = 0;
