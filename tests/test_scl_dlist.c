...
static void test_dlist_push_front_back(scl_test_runner_t *tr) {
  ...
  scl_dlist_t list;
  scl_error_t err = scl_dlist_init(&list, sizeof(int));
  SCL_EXPECT_OK(tr, err);

  int val1 = 10, val2 = 20, val3 = 30;

  err = scl_dlist_push_front(alloc, &list, &val1);
  SCL_EXPECT_OK(tr, err);
...
static void test_dlist_pop_front_back(scl_test_runner_t *tr) {
  ...
  int values[] = {10, 20, 30};
  for (int i = 0; i < 3; i++) {
    err = scl_dlist_push_back(alloc, &list, &values[i]);
    SCL_EXPECT_OK(tr, err);
  }

  int out;
  err = scl_dlist_pop_front(alloc, &list, &out);
  SCL_EXPECT_OK(tr, err);
...
static void test_dlist_search(scl_test_runner_t *tr) {
  ...
  int search_val = 200;
  int found;
  err = scl_dlist_search(&list, &search_val, int_cmp, &found);
  SCL_EXPECT_OK(tr, err);
