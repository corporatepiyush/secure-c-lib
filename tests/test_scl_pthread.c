...
static void test_trylock(scl_test_runner_t *tr) {
    ...
    SCL_EXPECT_EQ_I(tr, scl_mutex_trylock(&m) != SCL_OK, 1); /* already held (debug) or EBUSY */
    scl_mutex_unlock(&m);
    SCL_EXPECT_OK(tr, scl_mutex_trylock(&m));
    scl_mutex_unlock(&m);
...
static void test_null_errors(scl_test_runner_t *tr) {
    ...
    SCL_EXPECT_ERROR(tr, scl_thread_create_named(NULL, "x", NULL, NULL), SCL_ERR_NULL_PTR);
}
