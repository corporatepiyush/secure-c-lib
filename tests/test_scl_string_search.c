...
typedef struct { size_t arr[64]; size_t n; } collector_t;
static bool collect_cb(size_t pos, void *u) {
    collector_t *c = (collector_t *)u;
    if (c->n < 64) c->arr[c->n++] = pos;
    return true;
}
...
static int has_exactly(collector_t *c, const size_t *expected, size_t m) {
    ...
    for (size_t i = 0; i < m; i++) if (c->arr[i] != expected[i]) return 0;
    return 1;
}
...
static void test_first_match(scl_test_runner_t *tr) {
    ...
    /* not found */
    SCL_EXPECT_TRUE(tr, scl_search_boyer_moore(a, text, tlen, "xyz", 3, &bp) == SCL_ERR_NOT_FOUND);
    SCL_EXPECT_TRUE(tr, scl_search_kmp(a, text, tlen, "xyz", 3, &kp) == SCL_ERR_NOT_FOUND);

    /* pattern longer than text */
    SCL_EXPECT_TRUE(tr, scl_search_boyer_moore(a, "hi", 2, "hello", 5, &bp) == SCL_ERR_NOT_FOUND);
    SCL_EXPECT_TRUE(tr, scl_search_kmp(a, "hi", 2, "hello", 5, &kp) == SCL_ERR_NOT_FOUND);
...
static void test_find_all_overlap(scl_test_runner_t *tr) {
    ...
    /* "aaaa" / "aa" -> overlapping matches at 0,1,2 */
    {
        ...
        size_t bn = 0, kn = 0;
        SCL_EXPECT_OK(tr, scl_search_boyer_moore_all(a, "aaaa", 4, "aa", 2, collect_cb, &bc, &bn));
        SCL_EXPECT_OK(tr, scl_search_kmp_all(a, "aaaa", 4, "aa", 2, collect_cb, &kc, &kn));
        size_t exp[] = { 0, 1, 2 };
    ...
    /* "abababab" / "abab" -> overlapping at 0,2,4 */
    {
        collector_t bc = {{0}, 0}, kc = {{0}, 0};
        size_t bn = 0, kn = 0;
        SCL_EXPECT_OK(tr, scl_search_boyer_moore_all(a, "abababab", 8, "abab", 4, collect_cb, &bc, &bn));
        SCL_EXPECT_OK(tr, scl_search_kmp_all(a, "abababab", 8, "abab", 4, collect_cb, &kc, &kn));
        size_t exp[] = { 0, 2, 4 };
        SCL_EXPECT_TRUE(tr, has_exactly(&bc, exp, 3));
        SCL_EXPECT_TRUE(tr, has_exactly(&kc, exp, 3));
    }
    ...
    {
        size_t bn = 0, kn = 0;
        SCL_EXPECT_OK(tr, scl_search_boyer_moore_all(a, "mississippi", 11, "issi", 4, NULL, NULL, &bn));
        SCL_EXPECT_OK(tr, scl_search_kmp_all(a, "mississippi", 11, "issi", 4, NULL, NULL, &kn));
    ...
    {
        size_t n = 99;
        SCL_EXPECT_TRUE(tr, scl_search_kmp_all(a, "abcabc", 6, "abcd", 4, NULL, NULL, &n) == SCL_ERR_NOT_FOUND);
...
static void test_suffix_trie(scl_test_runner_t *tr) {
    ...
    SCL_EXPECT_EQ_SZ(tr, scl_suffix_trie_count(t, "ana", 3), 2);   /* 1,3 (overlap) */
    ...
    {
        collector_t c = {{0}, 0}; size_t n = 0;
        SCL_EXPECT_OK(tr, scl_suffix_trie_find_all(t, "ana", 3, collect_cb, &c, &n));
        size_t exp[] = { 1, 3 };
    ...
    {
        size_t n = 7;
        SCL_EXPECT_TRUE(tr, scl_suffix_trie_find_all(t, "zzz", 3, NULL, NULL, &n) == SCL_ERR_NOT_FOUND);
        SCL_EXPECT_EQ_SZ(tr, n, 0);
        SCL_EXPECT_TRUE(tr, scl_suffix_trie_find_all(t, "", 0, NULL, NULL, &n) == SCL_ERR_INVALID_ARG);
    }
    ...
    size_t bm_n = 0;
    SCL_EXPECT_OK(tr, scl_search_boyer_moore_all(a, big, blen, "the", 3, NULL, NULL, &bm_n));
...
static void test_suffix_trie_bounds(scl_test_runner_t *tr) {
    ...
    SCL_EXPECT_TRUE(tr, scl_suffix_trie_build(a, &t, buf, 200, 100) == SCL_ERR_SIZE_OVERFLOW);
