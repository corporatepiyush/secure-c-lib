...
static void test_init_load_store(scl_test_runner_t *tr) {
    ...
    SCL_EXPECT_EQ_I(tr, scl_atomic_load_explicit(&a, scl_memory_order_acquire), 200);
    ...
    SCL_EXPECT_EQ_SZ(tr, scl_atomic_load_sz_explicit(&s, scl_memory_order_relaxed), 50);
}
...
static void test_flag(scl_test_runner_t *tr) {
    ...
    SCL_EXPECT_FALSE(tr, scl_atomic_flag_test_and_set_explicit(&f, scl_memory_order_acquire));
}
...
static void test_fence(scl_test_runner_t *tr) {
    ...
    SCL_EXPECT_TRUE(tr, 1); /* fence smoke test — if it compiles and runs it's OK */
}
