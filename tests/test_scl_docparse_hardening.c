 ...
 *
 *   gcc -fsanitize=address,undefined -I libs/...
 * tests/test_scl_docparse_hardening.c \ libs/docparse/<...>.c
 * libs/common/scl_common.c libs/string/scl_string.c ...
 */
 ...
 */
static int write_temp(const char *tag, const unsigned char *data, size_t len,
...
static void test_csv_tsv(scl_test_runner_t *tr) {
  ...
  size_t flen;
  int guard = 0;
  while (scl_parse_csv_next_field(&csv, &fld, &flen) == SCL_OK &&
         guard++ < 100) {
    if (scl_parse_csv_next_row(&csv) != SCL_OK)
      break;
  }
  ...
  guard = 0;
  while (scl_parse_tsv_next_field(&tsv, &fld, &flen) == SCL_OK &&
         guard++ < 100) {
    if (scl_parse_tsv_next_row(&tsv) != SCL_OK)
      break;
  }
...
static void test_pdf_bad_xref(scl_test_runner_t *tr) {
  ...
  char path[256];
  write_temp("pdf", (const unsigned char *)doc, strlen(doc), path,
             sizeof(path));
...
static size_t build_forged_zip(unsigned char *buf, const char *name,
                               const char *payload) {
...
static void test_xlsx_forged_zip(scl_test_runner_t *tr) {
  ...
  size_t pos =
      build_forged_zip(buf, "xl/sharedStrings.xml", "<si><t>x</t></si>");
  write_temp("xlsx_ss", buf, pos, path, sizeof(path));
  scl_parse_xlsx_t x1;
  if (scl_parse_xlsx_open(a, &x1, path) == SCL_OK)
    scl_parse_xlsx_close(&x1);
  remove(path);
  SCL_EXPECT_TRUE(tr, 1);
...
static void test_parquet_runaway_varint(scl_test_runner_t *tr) {
  ...
  for (int i = 0; i < 8; i++)
    footer[fl++] = 0x80; /* never-terminating varint */

  unsigned char buf[64];
  size_t pos = 0;
  memcpy(buf + pos, "PAR1", 4);
  pos += 4;
  memcpy(buf + pos, footer, fl);
  pos += fl;
  put_le32(buf + pos, (uint32_t)fl);
  pos += 4;
  memcpy(buf + pos, "PAR1", 4);
  pos += 4;
