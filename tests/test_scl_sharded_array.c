...
static void test_append_get(scl_test_runner_t *tr) {
  ...
  for (int i = 0; i < 10; i++) {
    ...
    if (i == 0)
      firstptr = (int *)scl_sharded_array_get(&sa, 0);
  }
...
static void test_struct_elems(scl_test_runner_t *tr) {
  ...
  scl_sharded_array_t sa;
  SCL_EXPECT_OK(
      tr, scl_sharded_array_init(a, &sa, sizeof(rec_t), 0)); /* default shard */
  ...
  if (r) {
    SCL_EXPECT_EQ_I(tr, r->a, 777);
    SCL_EXPECT_EQ_I(tr, r->b, 1554);
  }
  scl_sharded_array_destroy(&sa);
  TEST_TRACE_END();
}
...
static void test_errors(scl_test_runner_t *tr) {
  ...
  scl_sharded_array_t sa;
  SCL_EXPECT_TRUE(tr,
                  scl_sharded_array_init(a, &sa, 0, 4) == SCL_ERR_INVALID_ARG);
  SCL_EXPECT_TRUE(tr,
                  scl_sharded_array_init(a, NULL, 4, 4) == SCL_ERR_NULL_PTR);
  TEST_TRACE_END();
}
