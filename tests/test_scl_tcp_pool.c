...
static void *freelist_worker(void *p) {
  ...
  for (int i = 0; i < ITERS; i++) {
    ...
    if (!c)
      continue; /* exhausted: retry */
    int prev =
        atomic_exchange_explicit(&ctx->held[c->index], 1, memory_order_acq_rel);
...
static void test_pool_freelist(scl_test_runner_t *tr) {
  ...
  SCL_EXPECT_OK(tr, scl_tcp_pool_init(scl_allocator_default(), &ctx->pool,
                                      POOL_CAP, 256));
  for (int i = 0; i < POOL_CAP; i++)
    atomic_init(&ctx->held[i], 0);
...
static void *handoff_worker(void *p) {
  ...
  if (arg->thread_id < half) { /* producer */
    ...
    for (;;) {
      ...
      if (c) {
        int prev = atomic_exchange_explicit(&ctx->held[c->index], 1,
        ...
        if (atomic_load_explicit(&ctx->produced, memory_order_acquire) ==
                target &&
            atomic_load_explicit(&ctx->consumed, memory_order_acquire) ==
                target)
          break;
        scl_cpu_pause();
      }
...
static void test_pool_handoff(scl_test_runner_t *tr) {
  ...
  scl_test_run_concurrent(tr, 8, handoff_worker,
                          ctx); /* 4 producers, 4 consumers */
