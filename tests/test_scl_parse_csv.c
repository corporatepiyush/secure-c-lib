...
static void test_basic(scl_test_runner_t *tr) {
    ...
    /* row 1 */
    SCL_EXPECT_OK(tr, scl_parse_csv_next_field(&p, &f, &n)); SCL_EXPECT_TRUE(tr, feq(f, n, "a"));
    SCL_EXPECT_OK(tr, scl_parse_csv_next_field(&p, &f, &n)); SCL_EXPECT_TRUE(tr, feq(f, n, "b"));
    SCL_EXPECT_OK(tr, scl_parse_csv_next_field(&p, &f, &n)); SCL_EXPECT_TRUE(tr, feq(f, n, "c"));
    SCL_EXPECT_TRUE(tr, scl_parse_csv_next_field(&p, &f, &n) == SCL_ERR_EMPTY);
    SCL_EXPECT_OK(tr, scl_parse_csv_next_row(&p));
    /* row 2 */
    SCL_EXPECT_OK(tr, scl_parse_csv_next_field(&p, &f, &n)); SCL_EXPECT_TRUE(tr, feq(f, n, "1"));
    SCL_EXPECT_OK(tr, scl_parse_csv_next_field(&p, &f, &n)); SCL_EXPECT_TRUE(tr, feq(f, n, "2"));
    SCL_EXPECT_OK(tr, scl_parse_csv_next_field(&p, &f, &n)); SCL_EXPECT_TRUE(tr, feq(f, n, "3"));
    SCL_EXPECT_TRUE(tr, scl_parse_csv_next_field(&p, &f, &n) == SCL_ERR_EMPTY);
...
static void test_quoted(scl_test_runner_t *tr) {
    ...
    const char *f; size_t n;
    SCL_EXPECT_OK(tr, scl_parse_csv_next_field(&p, &f, &n)); SCL_EXPECT_TRUE(tr, feq(f, n, "a"));
    SCL_EXPECT_OK(tr, scl_parse_csv_next_field(&p, &f, &n)); SCL_EXPECT_TRUE(tr, feq(f, n, "b\"c"));
    SCL_EXPECT_OK(tr, scl_parse_csv_next_field(&p, &f, &n)); SCL_EXPECT_TRUE(tr, feq(f, n, "d,e"));
    SCL_EXPECT_OK(tr, scl_parse_csv_next_field(&p, &f, &n)); SCL_EXPECT_TRUE(tr, feq(f, n, "f\ng"));
    SCL_EXPECT_TRUE(tr, scl_parse_csv_next_field(&p, &f, &n) == SCL_ERR_EMPTY);
    SCL_EXPECT_TRUE(tr, scl_parse_csv_next_row(&p) == SCL_ERR_EMPTY);
    scl_parse_csv_destroy(&p);
}
...
static void test_empty_fields_and_crlf(scl_test_runner_t *tr) {
    ...
    const char *f; size_t n;
    SCL_EXPECT_OK(tr, scl_parse_csv_next_field(&p, &f, &n)); SCL_EXPECT_TRUE(tr, feq(f, n, ""));
    SCL_EXPECT_OK(tr, scl_parse_csv_next_field(&p, &f, &n)); SCL_EXPECT_TRUE(tr, feq(f, n, "x"));
    SCL_EXPECT_OK(tr, scl_parse_csv_next_field(&p, &f, &n)); SCL_EXPECT_TRUE(tr, feq(f, n, ""));
    SCL_EXPECT_TRUE(tr, scl_parse_csv_next_field(&p, &f, &n) == SCL_ERR_EMPTY);
    SCL_EXPECT_TRUE(tr, scl_parse_csv_next_row(&p) == SCL_ERR_EMPTY);
    scl_parse_csv_destroy(&p);
