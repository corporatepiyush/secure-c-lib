...
static void test_interpolation_adversarial(scl_test_runner_t *tr) {
    ...
    SCL_EXPECT_TRUE(tr, scl_search_interpolation_search(arr, n, 42, &idx) == SCL_ERR_NOT_FOUND);

    /* Uniform large positives. */
    int64_t up[] = { 1000000000000LL, 2000000000000LL, 3000000000000LL };
    SCL_EXPECT_OK(tr, scl_search_interpolation_search(up, 3, 2000000000000LL, &idx));
...
static void test_hash_tombstones(scl_test_runner_t *tr) {
    ...
    for (int i = 0; i < 20; i++) {
        snprintf(key, sizeof(key), "key%d", i);
        SCL_EXPECT_OK(tr, scl_search_ht_insert(ht, key, (void *)(intptr_t)(i + 100)));
    }
    ...
    void *v = NULL;
    SCL_EXPECT_TRUE(tr, scl_search_ht_search(ht, "absent", &v) == SCL_ERR_NOT_FOUND);
...
static void test_sentinel(scl_test_runner_t *tr) {
    ...
    size_t n = SCL_ARRAY_SIZE(arr), idx = 0;
    int key = 7;
    SCL_EXPECT_OK(tr, scl_search_sentinel_linear_search(a, arr, n, sizeof(int), &key, cmp_int, &idx));
    SCL_EXPECT_EQ_SZ(tr, idx, 4);
    key = 5;
    SCL_EXPECT_OK(tr, scl_search_sentinel_linear_search(a, arr, n, sizeof(int), &key, cmp_int, &idx));
    SCL_EXPECT_EQ_SZ(tr, idx, 0);
    key = 100;
    SCL_EXPECT_TRUE(tr, scl_search_sentinel_linear_search(a, arr, n, sizeof(int), &key, cmp_int, &idx) == SCL_ERR_NOT_FOUND);
...
static void test_quickselect(scl_test_runner_t *tr) {
    ...
    for (int shape = 0; shape < 3; shape++) {
        for (size_t i = 0; i < n; i++) {
            if (shape == 0) arr[i] = (int)i;                 /* sorted */
            else if (shape == 1) arr[i] = (int)(n - i);      /* reverse */
            else arr[i] = (int)((i * 2654435761u) % 1000);   /* pseudo-random */
            ref[i] = arr[i];
        }
        ...
        for (size_t ki = 0; ki < SCL_ARRAY_SIZE(ks); ki++) {
            int out = -1;
            SCL_EXPECT_OK(tr, scl_search_quickselect(a, arr, n, sizeof(int), cmp_int, ks[ki], &out));
...
static void test_exp_string(scl_test_runner_t *tr) {
    ...
    const char *strs[] = { "alpha", "bravo", "charlie", "delta", "echo", "foxtrot" };
    ...
    SCL_EXPECT_TRUE(tr, scl_search_exponential_string(strs, n, "zulu", &idx) == SCL_ERR_NOT_FOUND);

    /* A NULL element must be rejected, not dereferenced. */
    const char *bad[] = { "a", NULL, "c", "d" };
    SCL_EXPECT_TRUE(tr, scl_search_exponential_string(bad, 4, "d", &idx) == SCL_ERR_INVALID_ARG);
}
