...
static void test_dwcas_basic(scl_test_runner_t *tr) {
  ...
  SCL_EXPECT_TRUE(tr,
                  scl_dwcas(&cell, &expected, desired)); /* matches -> swap */
  ...
  SCL_EXPECT_FALSE(tr,
                   scl_dwcas(&cell, &wrong, other)); /* mismatch -> no swap */
...
static void *lf_stack_worker(void *p) {
  ...
  for (int i = 0; i < LF_ITERS; i++) {
    ...
    if (!n)
      continue; /* contended empty: retry */
    ...
    for (;;) {
      ...
      if (scl_mpmc_dequeue(&ctx->q, &v)) {
        ...
        if (idx >= 0 && idx < MPMC_TOTAL) {
          unsigned char prev = atomic_exchange_explicit(&ctx->seen[idx], 1,
...
static void test_mpmc_concurrent(scl_test_runner_t *tr) {
  ...
  int missing = 0;
  for (int i = 0; i < MPMC_TOTAL; i++)
    if (atomic_load_explicit(&ctx->seen[i], memory_order_relaxed) == 0)
      missing++;
  SCL_EXPECT_EQ_I(tr, missing, 0);
