...
static void test_kmeans_fit_predict(scl_test_runner_t *tr) {
  ...
  for (size_t i = 0; i < 10; i++)
    SCL_EXPECT_EQ_SZ(tr, (size_t)labels[i], (size_t)l0);
  for (size_t i = 10; i < 20; i++)
    SCL_EXPECT_EQ_SZ(tr, (size_t)labels[i], (size_t)l1);
...
static void test_kmeans_serialization(scl_test_runner_t *tr) {
  ...
  for (size_t i = 5; i < 10; i++) {
    ds.data[i * ds.row_stride] = 100.0f;
    ds.targets[i] = 1.0f;
  }
  SCL_EXPECT_OK(tr, scl_ml_dataset_prepare(&ds, a));
  ...
  for (size_t i = 0; i < 10; i++)
    SCL_EXPECT_EQ_SZ(tr, (size_t)labels_load[i], (size_t)labels_orig[i]);
