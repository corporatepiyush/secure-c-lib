...
int main(void) {
  ...
  scl_test_group("URL: simple http URL");
  {
    ...
    if (u.path)
      SCL_EXPECT_EQ_STR(&tr, u.path, "/path");
  ...
  scl_test_group("URL: with query string");
  {
    ...
    SCL_EXPECT_OK(&tr, e);
    if (u.path)
      SCL_EXPECT_EQ_STR(&tr, u.path, "/search");
    if (u.query)
      SCL_EXPECT_EQ_STR(&tr, u.query, "q=hello");
  }
  ...
  if (server_port != 0) {
    scl_test_group("Client: GET existing file");
    {
      scl_http_client_t *c = NULL;
      SCL_EXPECT_OK(&tr,
                    scl_http_client_init(scl_allocator_default(), &c, 65536));
      char url[256];
      ...
      if (resp.body)
        SCL_EXPECT_EQ_STR(&tr, (const char *)resp.body, "hello world\n");
      scl_http_client_destroy(c);
    }
    ...
    scl_test_group("Client: GET JSON (MIME type check)");
    {
      ...
      if (ct)
        SCL_EXPECT_TRUE(&tr, scl_strstr(ct, "application/json") != NULL);
      scl_http_client_destroy(c);
    }
    ...
    scl_test_group("Client: keep-alive, multiple requests");
    {
      ...
      snprintf(url1, sizeof(url1), "http://127.0.0.1:%u/hello.txt",
               server_port);
      snprintf(url2, sizeof(url2), "http://127.0.0.1:%u/data.json",
               server_port);
    ...
    scl_test_group("Client: reconnect after connection:close");
    {
      ...
      scl_http_client_response_t r1;
      scl_error_t e1 =
          scl_http_client_get(c, url, "Connection: close\r\n", &r1);
      SCL_EXPECT_OK(&tr, e1);
      SCL_EXPECT_EQ_I(&tr, r1.status, 200);
    ...
    scl_test_group("Client: custom headers");
    {
      ...
      scl_http_client_response_t resp;
      SCL_EXPECT_OK(
          &tr, scl_http_client_get(c, url, "X-Custom: test-value\r\n", &resp));
      SCL_EXPECT_EQ_I(&tr, resp.status, 200);
      scl_http_client_destroy(c);
    }
    ...
    scl_test_group("Client: path traversal");
    {
      ...
      SCL_EXPECT_OK(&tr,
                    scl_http_client_init(scl_allocator_default(), &c, 65536));
      char url[256];
      snprintf(url, sizeof(url), "http://127.0.0.1:%u/../../../etc/passwd",
               server_port);
      scl_http_client_response_t resp;
      SCL_EXPECT_OK(&tr, scl_http_client_get(c, url, NULL, &resp));
  ...
  scl_test_group("Client: connection refused");
  {
    ...
    scl_http_client_response_t resp;
    scl_error_t e =
        scl_http_client_get(c, "http://127.0.0.1:1/nonexistent", NULL, &resp);
  ...
  scl_test_group("Client: find header with populated response");
  {
    ...
    if (ct)
      SCL_EXPECT_EQ_STR(&tr, ct, "text/plain");

    const char *cl = scl_http_client_find_header(&r, "Content-Length");
    SCL_EXPECT_NOT_NULL(&tr, cl);
    if (cl)
      SCL_EXPECT_EQ_STR(&tr, cl, "42");
