...
static void test_bst_insert_find(scl_test_runner_t *tr) {
  ...
  int val = 42;
  err = scl_bst_insert(alloc, &tree, &val);
  SCL_EXPECT_OK(tr, err);
...
static void test_bst_remove(scl_test_runner_t *tr) {
  ...
  err = scl_bst_remove(alloc, &tree, &val);
  SCL_EXPECT_OK(tr, err);
  SCL_EXPECT_EQ_SZ(tr, scl_bst_count(&tree), 0);
...
static void test_bst_min_max(scl_test_runner_t *tr) {
  ...
  for (int i = 0; i < 5; i++) {
    err = scl_bst_insert(alloc, &tree, &values[i]);
    SCL_EXPECT_OK(tr, err);
  }

  int minval, maxval;
  err = scl_bst_min(&tree, &minval);
  SCL_EXPECT_OK(tr, err);
  err = scl_bst_max(&tree, &maxval);
  SCL_EXPECT_OK(tr, err);
...
static void test_bst_ordered_insert(scl_test_runner_t *tr) {
  ...
  for (int i = 0; i < 7; i++) {
    err = scl_bst_insert(alloc, &tree, &entries[i]);
    SCL_EXPECT_OK(tr, err);
  }
  ...
  for (int i = 0; i < 7; i++) {
    int out;
    err = scl_bst_find(&tree, &entries[i], &out);
    SCL_EXPECT_OK(tr, err);
