...
static void test_simd_elementwise(scl_test_runner_t *tr) {
  ...
  for (size_t i = 0; i < 4; i++)
    SCL_ML_NEAR(tr, z[i], a[i] + b[i], 1e-5f);
  ...
  for (size_t i = 0; i < 4; i++)
    SCL_ML_NEAR(tr, z[i], a[i] * 2, 1e-5f);
  scl_ml_simd.add_s(z, a, 10.0f, 4);
  for (size_t i = 0; i < 4; i++)
    SCL_ML_NEAR(tr, z[i], a[i] + 10, 1e-5f);
  TEST_TRACE_END();
}
static void test_simd_activations(scl_test_runner_t *tr) {
  ...
  for (size_t i = 0; i < 8; i++)
    SCL_ML_NEAR(tr, out[i], in[i] > 0 ? in[i] : 0, 1e-5f);
  ...
  for (size_t i = 0; i < 8; i++) {
    ...
    scl_expect_true(tr, diff < 1e-2f || rel < 3e-1f, __FILE__, __LINE__,
                    "vexp near");
  }
  TEST_TRACE_END();
}
static void test_simd_distances(scl_test_runner_t *tr) {
  ...
  float dot_ab = 1 * 4 + 2 * 3 + 3 * 2 + 4 * 1, na = sqrtf(1 + 4 + 9 + 16),
        nb = sqrtf(16 + 9 + 4 + 1);
...
static void test_simd_threshold(scl_test_runner_t *tr) {
  ...
  for (size_t i = 0; i < 6; i++)
    SCL_ML_NEAR(tr, out[i], expected[i], 1e-5f);
  TEST_TRACE_END();
}
static void test_simd_argminmax(scl_test_runner_t *tr) {
  ...
  size_t amin, amax;
  scl_ml_simd.argminmax(x, 6, &amin, &amax);
