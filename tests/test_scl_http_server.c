...
static bool health_handler(const scl_http_request_t *req, scl_http_response_t *resp, void *user) {
    ...
    if (scl_strcmp(req->path, "/echo") == 0) {
        resp->status = 200;
        resp->content_type = req->content_type ? req->content_type : "application/octet-stream";
        resp->body = req->body;
...
int main(void) {
    ...
    scl_test_group("HTTP: Transfer-Encoding takes precedence over Content-Length");
    {
        const char *req =
            "POST /echo HTTP/1.1\r\n"
            "Host: x\r\n"
