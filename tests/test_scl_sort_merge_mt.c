...
static void test_sizes_threads(scl_test_runner_t *tr) {
  ...
  for (size_t si = 0; si < SCL_ARRAY_SIZE(sizes); si++) {
    ...
    for (size_t ti = 0; ti < SCL_ARRAY_SIZE(threads); ti++) {
      ...
      SCL_EXPECT_OK(tr, scl_sort_merge_sort_mt(a, arr, n, sizeof(int), cmp_int,
                                               threads[ti]));
...
static void test_stability(scl_test_runner_t *tr) {
  ...
  for (size_t i = 1; i < n; i++) {
    if (arr[i - 1].key > arr[i].key)
      ordered = 0;
    if (arr[i - 1].key == arr[i].key && arr[i - 1].idx > arr[i].idx)
      stable = 0;
  }
...
static void test_edges(scl_test_runner_t *tr) {
  ...
  SCL_EXPECT_TRUE(tr, scl_sort_merge_sort_mt(a, NULL, 10, sizeof(int), cmp_int,
                                             4) == SCL_ERR_NULL_PTR);
  SCL_EXPECT_TRUE(tr, scl_sort_merge_sort_mt(a, one, 1, 0, cmp_int, 4) ==
                          SCL_ERR_INVALID_ARG);
