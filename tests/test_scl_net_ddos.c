...
static void test_full_table_fails_closed(scl_test_runner_t *tr) {
  ...
  for (int i = 0; i < 64; i++) {
    ...
    if (scl_ddos_check(d, SA(s)))
      admitted++; /* leaves conn_count=1 */
  }
  ...
  char ip0[32];
  snprintf(ip0, sizeof(ip0), "10.0.0.0");
