...
scl_error_t scl_cdeque_push_front(scl_allocator_t *alloc, scl_concurrent_deque_t *deque, const void *element)
{
    (void)alloc;
...
scl_error_t scl_cdeque_push_back(scl_allocator_t *alloc, scl_concurrent_deque_t *deque, const void *element)
{
    (void)alloc;
    if (!deque || !element) return SCL_ERR_NULL_PTR;
    scl_spinlock_lock(&deque->lock);
