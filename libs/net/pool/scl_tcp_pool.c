 ...
 *     to prevent data leaking between connections (Heartbleed-class
 * mitigation). • Overflow-safe allocation: the contiguous rbuf_arena uses
 * scl_mul_overflow to guard against size_t wrap-around in capacity * rbuf_cap.
 ...
 *                            link member for the Treiber stack's intrusive
 * list) rbuf_arena            — one big block of capacity * rbuf_cap bytes,
 * sliced per-slot during init (no per-slot malloc overhead)
 *
...
scl_error_t scl_tcp_pool_init(scl_allocator_t *alloc, scl_tcp_pool_t *pool,
  ...
  if (scl_unlikely(!alloc || !pool))
    return SCL_ERR_NULL_PTR;
  if (capacity == 0 || rbuf_cap == 0)
  ...
  pool->slots = (scl_tcp_conn_t *)scl_calloc(
      alloc, capacity, sizeof(scl_tcp_conn_t), _Alignof(scl_tcp_conn_t));
  if (!pool->slots)
    return SCL_ERR_OUT_OF_MEMORY;

  pool->rbuf_arena =
      (unsigned char *)scl_alloc(alloc, arena_bytes, _Alignof(max_align_t));
  if (!pool->rbuf_arena) {
...
scl_tcp_conn_t *scl_tcp_pool_get_ready(scl_tcp_pool_t *pool) {
  if (scl_unlikely(!pool))
    return NULL;
  uintptr_t v;
  if (!scl_mpmc_dequeue(&pool->ready, &v))
    return NULL;
...
void scl_net_init(void) { scl_once(&scl_net_once, scl_net_once_fn); }
...
scl_error_t scl_tcp_listen(const char *host, uint16_t port, int backlog,
  ...
  *out_fd = -1;
  ...
  for (struct addrinfo *ai = res; ai; ai = ai->ai_next) {
    ...
    if (fd >= 0) {
      /* Best-effort: set O_CLOEXEC if available. */
...
do {
  ...
  if (fd < 0) {
    /* Honour the documented contract. Returning SCL_OK here (as this
     * function once did) hands the caller conn->fd == -1 and, on a
     * non-blocking listen socket, turns every EAGAIN into a 100%-CPU
     * accept/close spin that also blocks server shutdown. */
    if (errno == EAGAIN || errno == EWOULDBLOCK ||
        errno == ECONNABORTED /* peer gone before accept: transient */)
      return SCL_ERR_TIMEOUT;
    return SCL_ERR_IO;
  }

  /* Set O_CLOEXEC and TCP_NODELAY in one pass. */
  int flags = fcntl(fd, F_GETFD, 0);
  if (flags >= 0)
    fcntl(fd, F_SETFD, flags | FD_CLOEXEC);
  int one = 1;
