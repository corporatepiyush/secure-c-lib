...
/* HTTP/1.1 client. Keep-alive connection reuse, bounded response buffering, percent-decoding, redirect following. */
 ...
 * ── Security design ──────────────────────────────────────────────────────────
 *
 * The biggest risk in an HTTP client is parsing an attacker-controlled response:
 * a rogue or compromised server can send arbitrarily large or malformed headers,
 * lie about Content-Length, or attempt response-splitting. We defend against
 * these by:
 *
...
#define SCL_HTTP_CLIENT_MAX_HEADER_BUF (64 * 1024)  /* 64 KB for response headers */
#define SCL_HTTP_CLIENT_MAX_BODY_BUF  (4 * 1024 * 1024)  /* 4 MB default max body */
...
static inline scl_error_t scl_http_client_get(scl_http_client_t *c,
                                              const char *url,
                                              const char *headers,
                                              scl_http_client_response_t *resp) {
...
static inline scl_error_t scl_http_client_post(scl_http_client_t *c,
                                               const char *url,
                                               const char *headers,
                                               const void *body, size_t body_len,
                                               scl_http_client_response_t *resp) {
...
static inline scl_error_t scl_http_client_head(scl_http_client_t *c,
                                               const char *url,
                                               const char *headers,
                                               scl_http_client_response_t *resp) {
...
/* Low-level: feed raw bytes into the response parser. This is the
 * engine that incremental readers can call in a loop. For the common
 * case, use scl_http_client_read_response() instead. */
typedef struct {
    scl_http_client_state_t state;
    scl_http_client_response_t    *resp;
    size_t  content_length;       /* from Content-Length header (or 0) */
    size_t  parsed_up_to;         /* how far into rbuf we have consumed */
