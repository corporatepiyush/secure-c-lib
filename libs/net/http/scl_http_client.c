...
#include "scl_errno.h"   /* errno, EINTR, EAGAIN */
#include <fcntl.h>       /* fcntl, FD_CLOEXEC, F_GETFD, F_SETFD */
...
struct scl_http_client {
  ...
  /* Per-response timeout in ms. */
  int64_t timeout_ms;

  /* Absolute deadline computed once per request (issue 11). */
  int64_t deadline_ms;
};
  ...
  if (*p == '[') {
    ...
    *close_bracket = '\0';
    ...
    if (*p == ':') {
      ...
      if (!end || end == p + 1 || port == 0 || port > 65535)
        return SCL_ERR_INVALID_ARG;
    ...
    while (*host_end && *host_end != '/' && *host_end != '?' &&
      ...
      if (*host_end == ':' && !port_colon)
        port_colon = host_end;
      if ((unsigned char)*host_end < 0x20)
        return SCL_ERR_INVALID_ARG;
      host_end++;
    }
...
static bool conn_close_token(const char *val) {
  if (scl_unlikely(!val))
    return false;
  while (*val) {
    if ((*val == 'c' || *val == 'C') && (val[1] == 'l' || val[1] == 'L') &&
        (val[2] == 'o' || val[2] == 'O') && (val[3] == 's' || val[3] == 'S') &&
        (val[4] == 'e' || val[4] == 'E') &&
...
static scl_error_t tcp_connect_to(const char *host, uint16_t port,
                                  int *out_fd) {
  if (scl_unlikely(!host || !out_fd))
    return SCL_ERR_NULL_PTR;
  *out_fd = -1;

  char port_str[16];
  int pn = snprintf(port_str, sizeof(port_str), "%u", (unsigned)port);
  if (pn < 0 || (size_t)pn >= sizeof(port_str))
    return SCL_ERR_INVALID_ARG;
  port_str[15] = '\0';
...
scl_error_t scl_http_client_init(scl_allocator_t *alloc,
  ...
  *out = NULL;
  ...
  scl_http_client_t *c = (scl_http_client_t *)scl_calloc(
      alloc, 1, sizeof(scl_http_client_t), _Alignof(scl_http_client_t));
  if (!c)
  ...
  c->fd = -1;
  c->max_body_size =
      max_body_size > 0 ? max_body_size : SCL_HTTP_CLIENT_MAX_BODY_BUF;

  /* Pre-allocate read buffer. */
  c->rbuf_cap = 16384;
  c->rbuf =
      (unsigned char *)scl_alloc(alloc, c->rbuf_cap, _Alignof(max_align_t));
  if (!c->rbuf) {
    scl_free(alloc, c);
    return SCL_ERR_OUT_OF_MEMORY;
  }
...
scl_error_t scl_http_client_connect(scl_http_client_t *c, const char *host,
                                    uint16_t port) {
  if (scl_unlikely(!c || !host))
    return SCL_ERR_NULL_PTR;

  /* Reuse existing connection if viable. */
  if (c->fd >= 0 && c->conn_host && c->conn_port == port && !c->conn_close &&
      scl_strcmp(c->conn_host, host) == 0) {
    return SCL_OK;
  }
 ...
 */
static scl_error_t send_request(scl_http_client_t *c, const char *method,
  ...
  if (scl_unlikely(!method || method[0] == '\0'))
    return SCL_ERR_INVALID_ARG;
  for (const char *m = method; *m; m++) {
    if (scl_unlikely(*m < 'A' || *m > 'Z'))
      return SCL_ERR_INVALID_ARG;
  }
  ...
  if (c->conn_host) {
    n = snprintf(reqbuf + reqlen, sizeof(reqbuf) - reqlen, "Host: %s",
                 c->conn_host);
    if (n < 0 || reqlen + (size_t)n >= sizeof(reqbuf))
      return SCL_ERR_SIZE_OVERFLOW;
    reqlen += (size_t)n;
    if (c->conn_port != 80) {
      n = snprintf(reqbuf + reqlen, sizeof(reqbuf) - reqlen, ":%u\r\n",
                   (unsigned)c->conn_port);
      if (n < 0 || reqlen + (size_t)n >= sizeof(reqbuf))
        return SCL_ERR_SIZE_OVERFLOW;
      reqlen += (size_t)n;
    } else {
      if (reqlen + 3 > sizeof(reqbuf))
        return SCL_ERR_SIZE_OVERFLOW;
 ...
 * on success.
 */
static scl_error_t parse_status_line(const char *line, int *out_status,
                                     char *reason_buf, size_t reason_cap) {
  if (scl_unlikely(!line || !out_status))
    return SCL_ERR_NULL_PTR;

  /* Hand-rolled parse: skip "HTTP/1." */
  if (scl_strncmp(line, "HTTP/", 5) != 0)
    return SCL_ERR_PARSE;
  line += 5;
  /* Skip minor version "x" */
  while (*line && *line != ' ')
    line++;
  if (!*line)
    return SCL_ERR_PARSE;
  ...
  *out_status = st;
  ...
  if (*line == ' ') {
    line++; /* skip space */
    scl_strlcpy(reason_buf, line, reason_cap);
  } else {
    reason_buf[0] = '\0';
  }
...
static scl_error_t read_response(scl_http_client_t *c,
  ...
  /* Absolute deadline for per-response timeout (issue #11).
   * SO_RCVTIMEO is per-recv(), but a slow server can dribble 1 byte
   * per timeout window and hold the connection indefinitely. */
  int64_t deadline_ms = 0;
  if (c->timeout_ms > 0)
    deadline_ms = scl_now_ms() + c->timeout_ms;
  ...
  for (;;) {
    ...
    while ((hdr_end = find_hdrend(c->rbuf, c->rbuf_len)) < 0) {
      ...
      /* Grow rbuf on demand so recv always has space */
      if (c->rbuf_len == c->rbuf_cap) {
        size_t new_cap = c->rbuf_cap * 2;
        if (new_cap > SCL_HTTP_CLIENT_MAX_HEADER_BUF + 4096)
          new_cap = SCL_HTTP_CLIENT_MAX_HEADER_BUF + 4096;
        unsigned char *new_rbuf =
            scl_alloc(c->alloc, new_cap, alignof(max_align_t));
        if (scl_unlikely(!new_rbuf))
          return SCL_ERR_OUT_OF_MEMORY;
        scl_memcpy(new_rbuf, c->rbuf, c->rbuf_len);
        scl_free(c->alloc, c->rbuf);
        c->rbuf = new_rbuf;
        c->rbuf_cap = new_cap;
      }
      ...
      ssize_t n =
          recv(c->fd, c->rbuf + c->rbuf_len, c->rbuf_cap - c->rbuf_len, 0);
      if (scl_likely(n > 0)) {
        c->rbuf_len += (size_t)n;
        continue;
      }
      ...
      if (errno == EINTR)
        continue;
      if (scl_unlikely(errno == EAGAIN || errno == EWOULDBLOCK)) {
        /* Per-response deadline: only timeout if deadline reached.
         * Otherwise loop and retry recv(). */
        if (deadline_ms > 0 && scl_now_ms() >= deadline_ms)
    ...
    status = 0;
    err = parse_status_line((const char *)c->rbuf, &status, resp->status_text,
    ...
    if (hdr_area_len > 0) {
      ...
      resp->headers =
          (char *)scl_alloc(c->alloc, store_size, _Alignof(max_align_t));
      if (resp->headers) {
    ...
    if (status >= 100 && status < 200 && status != 101) {
      ...
      if (c->rbuf_len > header_block_len) {
        memmove(c->rbuf, c->rbuf + header_block_len,
   ...
   */
  bool no_body =
      head_response || status == 204 || status == 304 || status == 101;
  ...
  if (content_length > 0) {
    ...
    while (resp->body_len < content_length) {
      ...
      ssize_t n =
          recv(c->fd, (unsigned char *)resp->body + resp->body_len, to_read, 0);
      if (scl_likely(n > 0)) {
    ...
    size_t avail =
        c->rbuf_len > header_block_len ? c->rbuf_len - header_block_len : 0;
    if (avail > 0) {
    ...
    for (;;) {
      if (resp->body_len >= c->max_body_size)
        break;
      if (resp->body_len + 4096 > resp->body_cap) {
        size_t new_cap = resp->body_cap * 2;
        if (new_cap > c->max_body_size)
          new_cap = c->max_body_size;
        if (new_cap <= resp->body_cap)
          break;
        void *nb = scl_realloc(c->alloc, resp->body, resp->body_cap, new_cap,
      ...
      ssize_t n = recv(c->fd, (unsigned char *)resp->body + resp->body_len,
...
scl_error_t scl_http_client_request(scl_http_client_t *c, const char *method,
                                    ...
                                    scl_http_client_response_t *resp) {
  if (scl_unlikely(!c || !method || !url || !resp))
    return SCL_ERR_NULL_PTR;
  ...
  if (url_len >= SCL_HTTP_CLIENT_MAX_TARGET)
    return SCL_ERR_SIZE_OVERFLOW;
 ...
 *
 * This implements a full incremental HTTP/1.1 response parser. It
 * accumulates raw bytes in an internal buffer (ibuf) and advances
 * through states: STATUS → HEADERS → BODY (Content-Length or chunked)
 * → DONE.  The caller feeds bytes via scl_http_response_parser_feed();
 * the parser only consumes what it can fully parse and leaves the
 * remainder in the internal buffer for the next call.
 */

/* Append body bytes, growing the body buffer as needed. */
static bool scl_http__append_body(scl_http_response_parser_t *p,
                                   const void *data, size_t len) {
   if (len == 0)
     return true;
   size_t new_len = p->resp->body_len + len;
   if (new_len > p->resp->body_cap) {
     size_t new_cap = p->resp->body_cap == 0 ? 4096 : p->resp->body_cap;
     while (new_cap < new_len)
       new_cap *= 2;
     if (new_cap > p->max_body_size)
       new_cap = p->max_body_size;
     if (new_len > new_cap)
       return false; /* exceeds max body size */
     void *nb = scl_realloc(p->alloc, p->resp->body, p->resp->body_cap,
                            new_cap, _Alignof(max_align_t));
     if (!nb)
       return false;
     p->resp->body = nb;
     p->resp->body_cap = new_cap;
   }
   scl_memcpy((unsigned char *)p->resp->body + p->resp->body_len, data, len);
   p->resp->body_len += len;
   p->body_read += len;
   return true;
}

/* Append bytes to the internal buffer. Returns false on OOM. */
static bool scl_http__ibuf_append(scl_http__ibuf_t *ibuf,
                                   const unsigned char *data, size_t len,
                                   scl_allocator_t *alloc) {
   if (ibuf->len + len > ibuf->cap) {
     size_t new_cap = ibuf->cap == 0 ? 4096 : ibuf->cap;
     while (new_cap < ibuf->len + len)
       new_cap *= 2;
     /* Cap at the header limit for safety. */
     size_t hard_limit = SCL_HTTP_CLIENT_MAX_HEADER_BUF + 4096;
     if (new_cap > hard_limit)
       new_cap = hard_limit;
     unsigned char *new_data =
         (unsigned char *)scl_realloc(alloc, ibuf->data, ibuf->cap, new_cap,
                                      _Alignof(max_align_t));
     if (!new_data)
       return false;
     ibuf->data = new_data;
   ...
   scl_memcpy(ibuf->data + ibuf->len, data, len);
   ibuf->len += len;
   return true;
}

/* Consume n bytes from the front of ibuf. */
static void scl_http__ibuf_consume(scl_http__ibuf_t *ibuf, size_t n) {
   if (n >= ibuf->len) {
     ibuf->len = 0;
     return;
   }
   scl_memmove(ibuf->data, ibuf->data + n, ibuf->len - n);
   ibuf->len -= n;
}

/* Find "\r\n" in ibuf starting at offset. Returns offset or -1. */
static long scl_http__ibuf_find_crlf(const scl_http__ibuf_t *ibuf,
                                      size_t start) {
   for (size_t i = start; i + 1 < ibuf->len; i++) {
     if (ibuf->data[i] == '\r' && ibuf->data[i + 1] == '\n')
       return (long)i;
   }
   return -1;
}

/* Find "\r\n\r\n" in ibuf. Returns offset of first '\r' or -1. */
static long scl_http__ibuf_find_hdrend(const scl_http__ibuf_t *ibuf) {
   for (size_t i = 0; i + 3 < ibuf->len; i++) {
     if (ibuf->data[i] == '\r' && ibuf->data[i + 1] == '\n' &&
         ibuf->data[i + 2] == '\r' && ibuf->data[i + 3] == '\n')
       return (long)i;
   }
   return -1;
}
...
/*
 * Scan headers in ibuf for Content-Length, Transfer-Encoding,
 * Connection.  Returns header-area byte count including \r\n\r\n,
 * or 0 if headers are not yet complete.
 */
static size_t scl_http__ibuf_scan_headers(const scl_http__ibuf_t *ibuf,
                                          size_t *content_length,
                                          bool *chunked,
                                          bool *connection_close) {
   long eol_off = scl_http__ibuf_find_crlf(ibuf, 0);
   if (eol_off < 0)
     return 0; /* status line incomplete */

   size_t hdr_start = (size_t)eol_off + 2;
   long hdrend = scl_http__ibuf_find_hdrend(ibuf);
   if (hdrend < 0)
     return 0; /* header block incomplete */

   size_t hdr_area_len = (size_t)hdrend + 4;
   *content_length = 0;
   *chunked = false;
   *connection_close = false;

   size_t hp = hdr_start;
   while (hp + 1 < hdr_area_len - 2) {
     long line_crlf = scl_http__ibuf_find_crlf(ibuf, hp);
     if (line_crlf < 0)
       break;
     size_t line_start = hp;
     size_t line_end = hp + (size_t)line_crlf;
     ...
     unsigned char saved = ibuf->data[line_end];
     ibuf->data[line_end] = '\0';
     const char *line = (const char *)ibuf->data + line_start;
     const char *colon = scl_strchr(line, ':');
     ibuf->data[line_end] = saved;

     if (colon) {
       *((char *)colon) = '\0';
       const char *hdr_name = line;
       const char *hdr_val = colon + 1;
       while (*hdr_val == ' ' || *hdr_val == '\t')
         hdr_val++;

       if (scl_strcmp(hdr_name, "Content-Length") == 0) {
         char *endptr = NULL;
         unsigned long cl = scl_strtoul(hdr_val, &endptr, 10);
         if (endptr && *endptr == '\0' &&
             cl <= SCL_HTTP_CLIENT_MAX_BODY_BUF)
           *content_length = (size_t)cl;
       } else if (scl_strcmp(hdr_name, "Transfer-Encoding") == 0) {
         if (scl_strstr(hdr_val, "chunked"))
           *chunked = true;
       } else if (scl_strcmp(hdr_name, "Connection") == 0) {
         if (scl_strstr(hdr_val, "close"))
           *connection_close = true;
       }
       *((char *)colon) = ':';
     }
...
scl_error_t scl_http_response_parser_feed(scl_http_response_parser_t *p,
   ...
   /* Feed new data into internal buffer. */
   if (len > 0) {
     if (!scl_http__ibuf_append(&p->ibuf, buf, len, p->alloc))
       return SCL_ERR_OUT_OF_MEMORY;
   }

   while (p->state != SCL_HTTP_CS_DONE && p->state != SCL_HTTP_CS_ERROR) {
     switch (p->state) {

     case SCL_HTTP_CS_STATUS: {
       long eol = scl_http__ibuf_find_crlf(&p->ibuf, 0);
       if (eol < 0)
         return SCL_OK;
       unsigned char saved = p->ibuf.data[(size_t)eol];
       p->ibuf.data[(size_t)eol] = '\0';
       int status = 0;
       scl_error_t err = parse_status_line(
           (const char *)p->ibuf.data, &status,
           p->resp->status_text, sizeof(p->resp->status_text));
       p->ibuf.data[(size_t)eol] = saved;
       if (err != SCL_OK) {
         p->state = SCL_HTTP_CS_ERROR;
         return err;
       }
p->resp->status = status;
        p->state = SCL_HTTP_CS_HEADERS;
       /* fall through */
     }

     case SCL_HTTP_CS_HEADERS: {
       size_t cl = 0;
       bool chunked = false;
       bool conn_close = false;
       size_t hdr_area = scl_http__ibuf_scan_headers(
           &p->ibuf, &cl, &chunked, &conn_close);
       if (hdr_area == 0)
         return SCL_OK;

       p->content_length = cl;
       p->chunked = chunked;
       p->resp->connection_close = conn_close;

       /* Extract header block into response. */
       long hdrend = scl_http__ibuf_find_hdrend(&p->ibuf);
       size_t hdr_start = (size_t)((long)p->ibuf.len > hdrend + 4
                                   ? 0 : 0); /* always 0 after status line */
       (void)hdr_start;
       long eol_status = scl_http__ibuf_find_crlf(&p->ibuf, 0);
       hdr_start = (size_t)eol_status + 2;
       size_t hdr_area_len = (size_t)hdrend + 4;
       size_t hdr_data_len = hdr_area_len - hdr_start;

       if (hdr_data_len > 0 && hdr_data_len < hdr_area_len) {
         size_t max_hdrs = hdr_data_len / 2;
         if (max_hdrs > SCL_HTTP_CLIENT_MAX_HEADERS)
           max_hdrs = SCL_HTTP_CLIENT_MAX_HEADERS;
         size_t store_size = hdr_data_len + max_hdrs + 2;
         if (store_size > SCL_HTTP_CLIENT_MAX_HEADER_BUF + 1024)
           store_size = SCL_HTTP_CLIENT_MAX_HEADER_BUF + 1024;

         p->resp->headers = (char *)scl_alloc(p->alloc, store_size,
                                               _Alignof(max_align_t));
         if (p->resp->headers) {
           size_t store_pos = 0;
           size_t hp = hdr_start;
           while (hp + 1 < hdr_area_len - 2) {
             long lcrlf = scl_http__ibuf_find_crlf(&p->ibuf, hp);
             if (lcrlf < 0)
               break;
             size_t lstart = hp;
             size_t lend = hp + (size_t)lcrlf;
             if (lend == lstart) {
               hp = lend + 2;
               break;
             }
             unsigned char sc = p->ibuf.data[lend];
             p->ibuf.data[lend] = '\0';
             const char *line = (const char *)p->ibuf.data + lstart;
             const char *colon = scl_strchr(line, ':');
             if (colon) {
               *((char *)colon) = '\0';
               const char *val = colon + 1;
               while (*val == ' ' || *val == '\t')
                 val++;
               size_t nv_len = scl_strlen(line);
               if (store_pos + nv_len + 2 < store_size) {
                 scl_memcpy(p->resp->headers + store_pos, line, nv_len + 1);
                 store_pos += nv_len + 1;
                 p->resp->header_count++;
               }
             }
           ...
           p->resp->headers[store_pos] = '\0';
           p->resp->headers[store_pos + 1] = '\0';
           p->resp->headers_len = store_pos + 1;
         }
       }

/* Skip 1xx interim responses. */
        if (p->resp->status >= 100 && p->resp->status < 200 &&
            p->resp->status != 101) {
          scl_free(p->alloc, p->resp->headers);
          p->resp->headers = NULL;
          scl_http__ibuf_consume(&p->ibuf, hdr_area_len);
          scl_memset(p->resp, 0, sizeof(*p->resp));
...
scl_http__ibuf_consume(&p->ibuf, hdr_area_len);
        ...
        if (p->head_response) {
          ...
          p->state = SCL_HTTP_CS_CHUNK_SIZE;
        } else if (p->content_length > 0) {
          p->state = SCL_HTTP_CS_BODY_CL;
        } else {
          p->state = SCL_HTTP_CS_DONE;
        }
        break;
     ...
     case SCL_HTTP_CS_CHUNK_SIZE: {
       ...
       p->ibuf.data[(size_t)eol] = '\0';
       char *endptr = NULL;
       long chunk_sz = scl_strtoul((const char *)p->ibuf.data, &endptr, 16);
       p->ibuf.data[(size_t)eol] = saved;

       if (!endptr || endptr == (char *)p->ibuf.data || chunk_sz < 0) {
         p->state = SCL_HTTP_CS_ERROR;
         return SCL_ERR_PARSE;
       }

       p->chunk_remaining = (size_t)chunk_sz;
       scl_http__ibuf_consume(&p->ibuf, (size_t)eol + 2);

       if (p->chunk_remaining == 0) {
         p->state = SCL_HTTP_CS_CHUNK_TRAILER;
         break;
       }
       p->state = SCL_HTTP_CS_CHUNK_DATA;
       /* fall through */
     }

     case SCL_HTTP_CS_CHUNK_DATA: {
       if (p->ibuf.len < p->chunk_remaining)
         return SCL_OK;
       if (!scl_http__append_body(p, p->ibuf.data, p->chunk_remaining)) {
         p->state = SCL_HTTP_CS_ERROR;
         return SCL_ERR_OUT_OF_MEMORY;
       }
       scl_http__ibuf_consume(&p->ibuf, p->chunk_remaining);
       p->state = SCL_HTTP_CS_CHUNK_TRAILER;
       /* fall through */
     }

     case SCL_HTTP_CS_CHUNK_TRAILER: {
       long eol = scl_http__ibuf_find_crlf(&p->ibuf, 0);
       if (eol < 0)
         return SCL_OK;
       size_t consume = (size_t)(eol + 2);
       scl_http__ibuf_consume(&p->ibuf, consume);

       if (p->chunk_remaining == 0) {
         p->resp->body_len = p->body_read;
         p->state = SCL_HTTP_CS_DONE;
         return SCL_OK;
       }
       p->state = SCL_HTTP_CS_CHUNK_SIZE;
     ...
     case SCL_HTTP_CS_BODY_CL: {
       if (p->ibuf.len == 0)
         return SCL_OK;
       size_t remaining = p->content_length - p->body_read;
       size_t to_copy = p->ibuf.len < remaining ? p->ibuf.len : remaining;
       if (!scl_http__append_body(p, p->ibuf.data, to_copy)) {
         p->state = SCL_HTTP_CS_ERROR;
         return SCL_ERR_OUT_OF_MEMORY;
       }
       scl_http__ibuf_consume(&p->ibuf, to_copy);
       if (p->body_read >= p->content_length)
         p->state = SCL_HTTP_CS_DONE;
       break;
     }

     case SCL_HTTP_CS_DONE:
       return SCL_OK;
