...
/* Compare up to prefix_len bytes of 'full' against 'expected'
 * (case-insensitive) */
static int ci_prefix(const char *full, size_t prefix_len,
                     const char *expected) {
  for (size_t i = 0; i < prefix_len; i++) {
    if (expected[i] == '\0')
      return 0;
    if (scl_tolower((unsigned char)full[i]) !=
        scl_tolower((unsigned char)expected[i]))
      return 0;
  }
...
static int ci_equal(const char *a, const char *b) {
  while (scl_likely(*a && *b)) {
    if (scl_unlikely(scl_tolower((unsigned char)*a) !=
                     scl_tolower((unsigned char)*b)))
      return 0;
    a++;
    b++;
  }
...
static int ci_contains_token(const char *haystack, const char *token) {
  ...
  for (const char *p = haystack; *p; p++) {
    size_t i = 0;
    while (i < tl && p[i] &&
           scl_tolower((unsigned char)p[i]) ==
               scl_tolower((unsigned char)token[i]))
      i++;
    if (i == tl) {
      char c = p[i];
      if (c == '\0' || c == ' ' || c == ',' || c == '\t' || c == '\r' ||
...
static long url_decode_path(const char *src, size_t srclen, char *dst,
                            size_t dstcap) {
  size_t o = 0;
 ...
 */
static int resolve_in_docroot(const scl_http_server_t *srv,
                              const char *decoded_path, char *resolved,
                              size_t cap) {
  if (srv->docroot_real_len == 0)
    return 404;
  if (decoded_path[0] != '/')
    return 400;
  ...
  int n = snprintf(candidate, sizeof(candidate), "%s%s", srv->docroot_real,
                   decoded_path);
  ...
  if (rl < srv->docroot_real_len)
    return 403;
  if (scl_memcmp(real, srv->docroot_real, srv->docroot_real_len) != 0)
    return 403;
  if (rl > srv->docroot_real_len && real[srv->docroot_real_len] != '/')
    return 403;
...
static scl_error_t send_simple(const scl_http_server_t *srv, int fd, int status,
                               ...
                               size_t body_len, bool head_only,
                               bool close_conn) {
  char date[64];
...
static scl_error_t send_error(const scl_http_server_t *srv, int fd, int status,
  ...
  if (bn < 0)
    blen = 0;
  else if ((size_t)bn >= sizeof(body))
    blen = sizeof(body) - 1;
  else
    blen = (size_t)bn;
  return send_simple(srv, fd, status, "text/html; charset=utf-8", body, blen,
...
static scl_error_t serve_static(const scl_http_server_t *srv, int fd,
  ...
  if (rc != 0)
    return send_error(srv, fd, rc, head_only, close_conn);
  ...
  /* Open first, then re-stat the fd: the path-based stat() above chose
   * the branch, but the file could have been swapped since (TOCTOU).
   * Everything served — type check and Content-Length — must come from
   * the opened fd. O_CLOEXEC so worker fds never leak across exec. */
  int ffd = open(resolved, O_RDONLY | O_CLOEXEC);
  if (ffd < 0)
    return send_error(srv, fd, 403, head_only, close_conn);
  ...
  if (err == SCL_OK && !head_only) {
    char *buf = (char *)scl_alloc(srv->alloc, SCL_HTTP_SEND_CHUNK,
    ...
    for (;;) {
      ...
      if (r > 0) {
        err = scl_tcp_send_all(fd, buf, (size_t)r);
        if (err != SCL_OK)
          break;
      } else if (r == 0)
        break;
...
static int read_chunked_body(scl_allocator_t *alloc, int fd,
  ...
  size_t cap = 4096;
  unsigned char *buf =
      (unsigned char *)scl_alloc(alloc, cap, _Alignof(max_align_t));
  if (!buf)
    return 500;
  size_t len = 0;
  ...
  while (1) {
    ...
    if (i + 1 < *rbuf_len) {
      ...
      while (remaining > 0) {
        ...
        if (to_copy > 0) {
          if (len + to_copy > max_body) {
            scl_free(alloc, buf);
            return 413;
          }
          if (len + to_copy > cap) {
            size_t new_cap = cap * 2;
            while (new_cap < len + to_copy)
              new_cap *= 2;
            unsigned char *nb = (unsigned char *)scl_realloc(
                alloc, buf, cap, new_cap, _Alignof(max_align_t));
            if (!nb) {
              scl_free(alloc, buf);
              return 500;
            }
...
int scl_http_parse_multipart(const void *body, size_t body_len,
                             ...
                             size_t max_count) {
  if (!body || !content_type || !count || !uploads)
    return -1;
  ...
  while (remaining > 0 && *count < max_count) {
    ...
    if (scl_memcmp(ptr, end_delim, scl_strlen(end_delim)) == 0)
      break;
    if (scl_memcmp(ptr, delim, dlen) != 0)
      break;
    ...
    while (remaining > 0) {
      ...
      if (colon_pos == 0 || colon_pos >= hdr_end)
        goto next_hdr;
...
#define CI_HDR_EQ(expected) ci_prefix((const char *)ptr, colon_pos, expected)
      ...
      if (CI_HDR_EQ("Content-Disposition")) {
        ...
        for (char *p = (char *)hdr_value_p; p < vend; p++) {
          bool at_param = (p == (char *)hdr_value_p) || p[-1] == ' ' ||
                          p[-1] == ';' || p[-1] == '\t';
          if (!at_param)
            continue;
          if ((size_t)(vend - p) >= 10 &&
...
static int parse_request(char *buf, size_t hdr_len, scl_http_request_t *req) {
  ...
  *sp2 = '\0';
  ...
  if (scl_strcmp(version, "HTTP/1.1") == 0)
    req->version_minor = 1;
  else if (scl_strcmp(version, "HTTP/1.0") == 0)
    req->version_minor = 0;
  else
    return 505;
  ...
  if (conn_hdr) {
    if (ci_contains_token(conn_hdr, "close"))
      req->keep_alive = false;
    else if (ci_contains_token(conn_hdr, "keep-alive"))
      req->keep_alive = true;
  }
  return 0;
}
...
static int read_body_fixed(scl_allocator_t *alloc, int fd,
  ...
  if ((unsigned long long)cl > max_body)
    return 413;

  size_t need = (size_t)cl;
  unsigned char *buf =
      (unsigned char *)scl_alloc(alloc, need, _Alignof(max_align_t));
  if (!buf)
    return 500;
  size_t got = 0;
...
static void handle_connection(scl_http_server_t *srv, scl_tcp_conn_t *conn) {
  ...
  size_t max_body = srv->cfg.max_body_size > 0 ? srv->cfg.max_body_size
                                               : SCL_HTTP_MAX_BODY_DEFAULT;
  ...
  while (atomic_load_explicit(&srv->running, memory_order_acquire) &&
    ...
    const int64_t hdr_timeout =
        srv->cfg.recv_timeout_ms > 0 ? srv->cfg.recv_timeout_ms : 15000;
    ...
    while (scl_unlikely(hend == 0)) {
      ...
      if (scl_likely(n > 0)) {
        ...
        if (hdr_deadline == 0)
          hdr_deadline = scl_now_ms() + hdr_timeout;
    ...
    /* Validate Content-Length: reject duplicates with differing values (RFC
     * 7230 §3.3.2) */
    int cl_count = 0;
    ...
    if (is_chunked) {
      status = read_chunked_body(alloc, fd, conn->rbuf, &conn->rbuf_len,
                                 conn->rbuf_cap, hend, max_body, &body_buf,
                                 &body_len);
      if (status != 0) {
        send_error(srv, fd, status, false, true);
        return;
    ...
    if (srv->cfg.handler &&
        srv->cfg.handler(&req, &resp, srv->cfg.handler_user)) {
      if (resp.close)
        will_close = true;
      serr = send_simple(srv, fd, resp.status ? resp.status : 200,
                         resp.content_type, resp.body, resp.body_len, head_only,
                         will_close);
    ...
    if (body_allocated)
      scl_free(alloc, body_buf);

    if (serr != SCL_OK || will_close)
      return;
    ...
    if (!body_allocated) {
      ...
      if (conn->rbuf_len > hdr_sz) {
        scl_memmove(conn->rbuf, conn->rbuf + hdr_sz, conn->rbuf_len - hdr_sz);
...
static void *acceptor_main(void *arg) {
  ...
  while (atomic_load_explicit(&srv->running, memory_order_acquire)) {
    ...
    for (;;) {
      /* Re-check the stop flag inside the drain loop: a steady stream
       * of inbound connections must not be able to postpone shutdown
       * indefinitely. */
      if (scl_unlikely(
              !atomic_load_explicit(&srv->running, memory_order_acquire)))
        break;
      scl_tcp_conn_t tmp;
      tmp.fd = -1;
      scl_error_t aerr = scl_tcp_accept(srv->listen_fd, &tmp);
      if (aerr == SCL_ERR_TIMEOUT)
        break;
      if (aerr != SCL_OK)
        break;
      if (scl_unlikely(tmp.fd < 0))
        break; /* defensive: never treat an invalid fd as a connection */
...
static void apply_defaults(scl_http_config_t *c) {
  if (c->num_workers <= 0)
    c->num_workers = 4;
  if (c->pool_capacity == 0)
    c->pool_capacity = 256;
  if (c->conn_buf_cap == 0)
    c->conn_buf_cap = 64 * 1024;
  if (c->max_body_size == 0)
    c->max_body_size = SCL_HTTP_MAX_BODY_DEFAULT;
  if (c->recv_timeout_ms == 0)
    c->recv_timeout_ms = 15000;
  if (c->keep_alive_max <= 0)
    c->keep_alive_max = 100;
  if (c->backlog <= 0)
    c->backlog = 128;
  if (!c->server_name)
    c->server_name = "scl-httpd";
}

scl_error_t scl_http_server_init(scl_allocator_t *alloc,
                                 scl_http_server_t **out,
                                 const scl_http_config_t *cfg) {
  if (scl_unlikely(!alloc || !out || !cfg))
    return SCL_ERR_NULL_PTR;
  *out = NULL;

  scl_net_init();

  scl_http_server_t *srv = (scl_http_server_t *)scl_calloc(
      alloc, 1, sizeof(scl_http_server_t), _Alignof(scl_http_server_t));
  if (!srv)
    return SCL_ERR_OUT_OF_MEMORY;
  srv->alloc = alloc;
  srv->cfg = *cfg;
  ...
  scl_error_t err = scl_tcp_pool_init(alloc, &srv->pool, srv->cfg.pool_capacity,
                                      srv->cfg.conn_buf_cap);
  if (err != SCL_OK) {
    scl_free(alloc, srv);
    return err;
  }

  err = scl_tcp_listen(srv->cfg.host, srv->cfg.port, srv->cfg.backlog,
                       &srv->listen_fd);
  if (err != SCL_OK) {
  ...
  srv->workers =
      (scl_thread_t *)scl_calloc(alloc, (size_t)srv->num_workers,
...
scl_error_t scl_http_server_start(scl_http_server_t *srv) {
  if (scl_unlikely(!srv))
    return SCL_ERR_NULL_PTR;
  if (atomic_exchange(&srv->started, true))
  ...
  for (int i = 0; i < srv->num_workers; i++) {
    if (scl_thread_create(&srv->workers[i], worker_main, srv) != SCL_OK) {
      ...
      scl_mutex_unlock(&srv->sem_m);
      for (int j = 0; j < i; j++)
        scl_thread_join(srv->workers[j], NULL);
