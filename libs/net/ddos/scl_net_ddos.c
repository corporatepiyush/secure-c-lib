...
/* DDoS protection: token-bucket rate limiter, IP black/whitelist via Patricia
 * trie, LRU grey-list for adaptive scoring. */
...
static void ip_key(int family, const void *addr, unsigned char out[16],
                   size_t *klen) {
...
static unsigned int find_slot(scl_ddos_t *ddos, int family,
  ...
  for (unsigned int i = 0; i < ddos->slot_count; i++) {
    ...
    if (scl_unlikely(s->family < 0)) {
      if (scl_unlikely(!alloc_if_missing))
        return UINT32_MAX;
      slot_init(s, ddos, family, h, key, klen, now);
      return probe;
    }

    if (scl_likely(s->hash == h &&
                   (size_t)(family == AF_INET ? 4 : 16) == klen &&
...
scl_error_t scl_ddos_init(scl_allocator_t *alloc, scl_ddos_t **out,
  ...
  *out = NULL;
  ...
  scl_ddos_t *ddos = (scl_ddos_t *)scl_calloc(alloc, 1, sizeof(scl_ddos_t),
                                              _Alignof(scl_ddos_t));
  if (!ddos)
  ...
  ddos->slots = (scl_ddos_slot_t *)scl_calloc(
      alloc, pow2, sizeof(scl_ddos_slot_t), _Alignof(scl_ddos_slot_t));
  if (!ddos->slots) {
    scl_free(alloc, ddos);
    return SCL_ERR_OUT_OF_MEMORY;
  }
...
bool scl_ddos_check(scl_ddos_t *ddos, const struct sockaddr *addr) {
  ...
  if (scl_unlikely(family < 0))
    return true; /* unknown family: allow */
