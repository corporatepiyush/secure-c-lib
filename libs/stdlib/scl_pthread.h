...
/* Zero-overhead POSIX-thread wrappers. Eliminates attribute boilerplate; recursive mutex via dedicated init. Debug owner-tracking behind NDEBUG. */
...
static inline scl_error_t scl_mutex_init(scl_mutex_t *m) {
    if (scl_unlikely(!m)) return SCL_ERR_NULL_PTR;
#ifdef SCL_PTHREAD_DEBUG
...
static inline scl_error_t scl_mutex_lock(scl_mutex_t *m) {
    ...
    if (ret == EDEADLK) return SCL_ERR_DEADLOCK;
    if (ret == EINVAL)  return SCL_ERR_INVALID_ARG;
    if (ret != 0)       return SCL_ERR_LOCK;
#else
    int ret = pthread_mutex_lock(&m->impl);
    if (ret == EDEADLK) return SCL_ERR_DEADLOCK;
    if (ret != 0)       return SCL_ERR_LOCK;
...
static inline void scl_mutex_destroy(scl_mutex_t *m) {
    if (scl_likely(m)) pthread_mutex_destroy(&m->impl);
}
...
static inline scl_error_t scl_thread_create(scl_thread_t *t,
                                             void *(*func)(void *),
                                             void *arg) {
    if (scl_unlikely(!t || !func)) return SCL_ERR_NULL_PTR;
...
static inline scl_error_t scl_thread_create_named(scl_thread_t *t,
    ...
    if (name) pthread_setname_np(t->impl, name);
#else
    (void)name;
#endif
    return SCL_OK;
...
static inline void scl_thread_detach(scl_thread_t *t) {
    if (scl_likely(t)) pthread_detach(t->impl);
}
 ...
 *  scl_cond_t
 *
 *  Simple condvar wrapper.  Timed-wait timeout is in
 *  milliseconds (computed to absolute CLOCK_REALTIME).
 * =========================================================== */
typedef struct {
    pthread_cond_t impl;
} scl_cond_t;
...
static inline scl_error_t scl_cond_init(scl_cond_t *c) {
    if (scl_unlikely(!c)) return SCL_ERR_NULL_PTR;
...
#endif
    if (ret == 0) return SCL_OK;
    if (ret == ETIMEDOUT) return SCL_ERR_TIMEOUT;
    if (ret == EINVAL)    return SCL_ERR_INVALID_ARG;
    if (ret == EPERM)     return SCL_ERR_LOCK;
    return SCL_ERR_LOCK;
}
...
static inline void scl_cond_destroy(scl_cond_t *c) {
    if (scl_likely(c)) pthread_cond_destroy(&c->impl);
}
...
static inline scl_error_t scl_rwlock_init(scl_rwlock_t *rw) {
    if (scl_unlikely(!rw)) return SCL_ERR_NULL_PTR;
...
static inline void scl_rwlock_destroy(scl_rwlock_t *rw) {
    if (scl_likely(rw)) pthread_rwlock_destroy(&rw->impl);
}
...
static inline scl_error_t scl_barrier_init(scl_barrier_t *b,
                                            unsigned int count) {
    if (scl_unlikely(!b)) return SCL_ERR_NULL_PTR;
...
static inline scl_error_t scl_barrier_wait(scl_barrier_t *b) {
    ...
    if (ret == 0 || ret == PTHREAD_BARRIER_SERIAL_THREAD) return SCL_OK;
    if (ret == EINVAL) return SCL_ERR_INVALID_ARG;
    return SCL_ERR_LOCK;
}

static inline void scl_barrier_destroy(scl_barrier_t *b) {
    if (scl_likely(b)) pthread_barrier_destroy(&b->impl);
}
...
static inline scl_error_t scl_once(scl_once_t *once, void (*fn)(void)) {
    if (scl_unlikely(!once || !fn)) return SCL_ERR_NULL_PTR;
...
static inline scl_error_t scl_scoped_lock_acquire(scl_scoped_lock_t *sl,
                                                   scl_mutex_t *m) {
    if (scl_unlikely(!sl || !m)) return SCL_ERR_NULL_PTR;
    sl->mutex = m;
    return scl_mutex_lock(m);
}

static inline scl_error_t scl_scoped_lock_release(scl_scoped_lock_t *sl) {
    if (scl_unlikely(!sl || !sl->mutex)) return SCL_ERR_NULL_PTR;
...
#define SCL_LOCK(mutex) \
    for (int _scl_lk_ = 0; \
         _scl_lk_ == 0 \
             ? ((_scl_lk_ = 1), (scl_mutex_lock(mutex) == SCL_OK)) \
