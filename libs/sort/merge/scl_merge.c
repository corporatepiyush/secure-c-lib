...
scl_error_t scl_sort_merge_sort(scl_allocator_t *alloc, void *base,
      ...
      while (scl_likely(li < mid && ri < r)) {
        if (cmp((unsigned char *)base + li * element_size,
          ...
          scl_memcpy(tmp + ti++ * element_size,
                     (unsigned char *)base + li++ * element_size, element_size);
        else
          scl_memcpy(tmp + ti++ * element_size,
                     (unsigned char *)base + ri++ * element_size, element_size);
      }
      while (li < mid)
        scl_memcpy(tmp + ti++ * element_size,
                   (unsigned char *)base + li++ * element_size, element_size);
...
scl_error_t scl_sort_merge_sort_mt(scl_allocator_t *alloc, void *base,
                                   size_t count, size_t element_size,
                                   scl_cmp_func_t cmp,
  ...
  if (threads > SCL_SORT_MERGE_MAX_THREADS)
    threads = SCL_SORT_MERGE_MAX_THREADS;
  if (threads < 1)
    threads = 1;
  ...
  int depth = 0;
  for (unsigned int t = 1; t < threads; t <<= 1)
    depth++;
