 ...
 *                CPU count. Internally clamped to [1,
 * SCL_SORT_MERGE_MAX_THREADS]; a value of 1 (or a small array) runs fully
 * sequentially.
 *
 * Security / robustness:
 *   - Thread count is bounded (no unbounded thread creation regardless of
 * input).
...
scl_error_t scl_sort_merge_sort_mt(scl_allocator_t *alloc, void *base,
                                   size_t count, size_t element_size,
                                   scl_cmp_func_t cmp,
                                   unsigned int max_threads) SCL_WARN_UNUSED;
