...
static SCL_ALWAYS_INLINE void scl_sort_swap(unsigned char * a, unsigned char * b, size_t n)
{
...
static SCL_ALWAYS_INLINE void scl_sort_copy(unsigned char * dest, const unsigned char * src, size_t element_size)
{
...
scl_error_t scl_sort_insertion(void * base, size_t count, size_t element_size, scl_cmp_func_t cmp)
{
    ...
    for (size_t i = 1; i < count; i++) {
        size_t j = i;
        while (scl_likely(j > 0) && cmp((unsigned char *)base + j * element_size,
...
scl_error_t scl_sort_radix(scl_allocator_t *alloc, int32_t * base, size_t count)
{
    ...
    if (scl_unlikely(!output)) return SCL_ERR_OUT_OF_MEMORY;
