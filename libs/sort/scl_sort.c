...
static SCL_ALWAYS_INLINE void scl_sort_swap(unsigned char *a, unsigned char *b,
                                            size_t n) {
...
static SCL_ALWAYS_INLINE void scl_sort_copy(unsigned char *dest,
...
scl_error_t scl_sort_merge(scl_allocator_t *alloc, void *base, size_t count,
  ...
  unsigned char *tmp =
      (unsigned char *)scl_alloc(alloc, bytes, alignof(max_align_t));
  if (scl_unlikely(!tmp))
  ...
  for (size_t width = 1; scl_likely(width < count); width *= 2) {
    for (size_t i = 0; i < count; i += 2 * width) {
      ...
      scl_memcpy((unsigned char *)base + i * element_size, tmp,
                 ti * element_size);
    }
  }
...
static void scl_heap_sift_down(unsigned char *base, size_t start, size_t end,
  ...
  while (root * 2 + 1 <= end) {
    ...
    if (swp == root)
      break;
    scl_sort_swap(base + root * element_size, base + swp * element_size,
                  element_size);
    root = swp;
  }
}
...
scl_error_t scl_sort_counting(scl_allocator_t *alloc, int32_t *base,
                              size_t count) {
...
scl_error_t scl_sort_radix(scl_allocator_t *alloc, int32_t *base,
                           size_t count) {
  if (scl_unlikely(!base))
    return SCL_ERR_NULL_PTR;
  if (scl_unlikely(count < 2))
...
scl_error_t scl_sort_bucket(scl_allocator_t *alloc, void *base, size_t count,
                            size_t element_size, scl_cmp_func_t cmp) {
  (void)alloc;
  (void)base;
  (void)count;
  (void)element_size;
  (void)cmp;
  /* Bucket sort with a comparator cannot be O(n) — it requires a numeric
   * key extractor. Use scl_sort_bucket_sort() from scl_bucket.h instead,
   * which accepts a scl_bucket_key_func_t. This stub is retained for API
   * compatibility but always returns SCL_ERR_NOT_IMPLEMENTED. */
