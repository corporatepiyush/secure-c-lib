...
/* Bucket sort. O(N+k) avg. Distributes into buckets, sorts individually. Stable. */

#include "scl_bucket.h"
#include "scl_string.h"
#include "scl_bucket.h"

static SCL_ALWAYS_INLINE void insertion_sort(unsigned char *ptr, size_t count, size_t element_size, scl_cmp_func_t cmp)
{
    for (size_t i = 1; i < count; i++) {
        size_t j = i;
        while (scl_likely(j > 0) && cmp(ptr + j * element_size, ptr + (j - 1) * element_size) < 0) {
            size_t k = 0;
