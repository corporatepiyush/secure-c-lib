...
/* Safe string/memory ops: bounded scl_strcpy/scl_memset/scl_memcpy, scl_strdup
 * via allocator, scl_secure_zero (volatile-barrier wipe immune to dead-store
 * elimination). */
...
char *scl_strstr(const char *haystack, const char *needle) {
  if (scl_unlikely(!haystack || !needle))
    return NULL;
  return (char *)strstr(haystack, needle);
}

char *scl_strpbrk(const char *s, const char *accept) {
  if (scl_unlikely(!s || !accept))
    return NULL;
...
char *scl_strtok_r(char *restrict str, const char *restrict delim,
                   char **restrict saveptr) {
  if (scl_unlikely(!delim || !saveptr))
    return NULL;
...
void *scl_memset(void *s, int c, size_t n) { return memset(s, c, n); }

void scl_memzero(void *s, size_t n) {
   if (scl_unlikely(!s))
     return;
   /* Delegate to the secure zero which uses a volatile function pointer
    * to prevent dead-store elimination.  Unlike __builtin_memset, this
    * is guaranteed not to be elided at -O2 / -O3. */
   scl_secure_zero(s, n);
 }
