...
/* Hardened socket wrappers. EINTR retry on recv/send/accept, SIGPIPE
 * suppression (MSG_NOSIGNAL/SO_NOSIGPIPE), forced TCP_NODELAY, non-blocking
 * connect helper. */
...
scl_error_t scl_socket_listen(const char *host, uint16_t port, int backlog,
                              int *out_fd);
