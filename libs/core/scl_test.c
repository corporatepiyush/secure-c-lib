...
static bool record(scl_test_runner_t *tr, bool ok, const char *file, int line,
                   const char *msg) {
  tr->asserts++;
  const char *ts = scl_test_timestamp();
  if (ok) {
    tr->passed++;
...
bool scl_expect_true(scl_test_runner_t *tr, bool cond, const char *file,
  ...
  if (cond)
    return record(tr, true, file, line, expr);
  char buf[1024];
...
bool scl_expect_eq_ptr(scl_test_runner_t *tr, const void *a, const void *b,
  ...
  if (a == b)
    return record(tr, true, file, line, "");
  char buf[512];
  snprintf(buf, sizeof(buf), "expected ptr %p == %p", (const void *)a,
           (const void *)b);
  return record(tr, false, file, line, buf);
}
...
bool scl_expect_eq_str(scl_test_runner_t *tr, const char *a, const char *b,
  ...
  if ((a == NULL && b == NULL) || (a && b && strcmp(a, b) == 0))
    return record(tr, true, file, line, "");
  char buf[1024];
  snprintf(buf, sizeof(buf), "expected \"%s\" == \"%s\"", a ? a : "NULL",
           b ? b : "NULL");
  return record(tr, false, file, line, buf);
}
...
bool scl_expect_error(scl_test_runner_t *tr, scl_error_t err,
  ...
  if (err == expected)
    return record(tr, true, file, line, "");
  char buf[512];
  snprintf(buf, sizeof(buf), "expected %s, got %s", scl_error_string(expected),
