...
scl_error_t scl_cringbuf_init(scl_allocator_t *alloc, scl_concurrent_ringbuf_t *rb, size_t element_size, size_t capacity)
{
...
scl_error_t scl_cringbuf_peek(const scl_concurrent_ringbuf_t *rb, size_t index, void  *SCL_RESTRICT out)
{
    if (scl_unlikely(!rb || !out)) return SCL_ERR_NULL_PTR;
    size_t cnt = atomic_load_explicit(&rb->count, memory_order_acquire);
