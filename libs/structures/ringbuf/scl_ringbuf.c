...
scl_error_t scl_ringbuf_init(scl_allocator_t *alloc, scl_ringbuf_t *rb,
...
scl_error_t scl_ringbuf_push(scl_ringbuf_t *rb, const void *element) {
  ...
  scl_memcpy(rb->data + tail * es, element, es);
  rb->count = cnt + 1;
  return SCL_OK;
}
...
scl_error_t scl_ringbuf_pop(scl_ringbuf_t *rb, void *out) {
  if (scl_unlikely(!rb || !out))
    return SCL_ERR_NULL_PTR;
  if (scl_unlikely(rb->count == 0))
    return SCL_ERR_EMPTY;

  size_t es = rb->element_size;
  scl_memcpy(out, rb->data + rb->head * es, es);
  rb->head = (rb->head + 1) & rb->mask;
  rb->count--;
  return SCL_OK;
}

scl_error_t scl_ringbuf_peek(const scl_ringbuf_t *rb, size_t index,
                             void *SCL_RESTRICT out) {
  if (scl_unlikely(!rb || !out))
    return SCL_ERR_NULL_PTR;
  if (scl_unlikely(index >= rb->count))
...
scl_error_t scl_ringbuf_search(const scl_ringbuf_t *restrict rb,
