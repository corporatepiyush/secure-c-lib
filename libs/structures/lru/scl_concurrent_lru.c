...
scl_error_t scl_clru_put(scl_allocator_t *alloc, scl_concurrent_lru_t *cache, const void  *SCL_RESTRICT key, const void *value)
{
    ...
    scl_concurrent_lru_node_t *n = scl_alloc(alloc, sizeof(scl_concurrent_lru_node_t), alignof(max_align_t));
    ...
    scl_memcpy(n->key, key, cache->key_size);
    scl_memcpy(n->value, value, cache->value_size);
    n->prev = n->next = NULL;
