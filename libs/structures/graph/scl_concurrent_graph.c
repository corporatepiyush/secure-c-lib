...
scl_error_t scl_cgraph_init_ex(scl_allocator_t *alloc,
                               scl_concurrent_graph_t *g, size_t vertex_count,
                               size_t shard_len) {
  if (scl_unlikely(!g))
...
scl_error_t scl_cgraph_init(scl_allocator_t *alloc, scl_concurrent_graph_t *g,
                            size_t vertex_count) {
  return scl_cgraph_init_ex(alloc, g, vertex_count,
                            SCL_GRAPH_DEFAULT_SHARD_LEN);
}
...
scl_error_t scl_cgraph_add_edge(scl_allocator_t *alloc,
                                scl_concurrent_graph_t *g, size_t from,
...
scl_error_t scl_cgraph_remove_edge(scl_allocator_t *alloc,
                                   scl_concurrent_graph_t *g, size_t from,
                                   size_t to) {
  if (scl_unlikely(!g))
    return SCL_ERR_NULL_PTR;
...
bool scl_cgraph_has_edge(const scl_concurrent_graph_t *g, size_t from,
                         size_t to) {
  if (scl_unlikely(!g))
    return false;
  scl_spinlock_t *lock =
      (scl_spinlock_t *)&g->lock; /* logical, not bitwise, const */
...
scl_error_t scl_cgraph_bfs(scl_allocator_t *alloc, scl_concurrent_graph_t *g,
  ...
  scl_spinlock_lock(&g->lock);
  scl_error_t r =
      scl_search_breadth_first_search(alloc, &g->core, start, visited);
  scl_spinlock_unlock(&g->lock);
  return r;
}
...
scl_error_t scl_cgraph_dfs(scl_allocator_t *alloc, scl_concurrent_graph_t *g,
  ...
  scl_spinlock_lock(&g->lock);
  scl_error_t r =
      scl_search_depth_first_search(alloc, &g->core, start, visited);
  scl_spinlock_unlock(&g->lock);
  return r;
}
