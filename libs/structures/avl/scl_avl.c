...
static SCL_ALWAYS_INLINE void scl_avl_update_height(scl_avl_node_t *n) {
  n->height =
      1 + scl_avl_max2(scl_avl_height(n->left), scl_avl_height(n->right));
}
...
scl_error_t scl_avl_init(scl_allocator_t *alloc, scl_avl_t *tree,
...
static scl_avl_node_t *scl_avl_create_node(scl_allocator_t *alloc,
                                           const void *data,
                                           size_t element_size) {
  scl_avl_node_t *n =
      scl_alloc(alloc, sizeof(scl_avl_node_t), alignof(max_align_t));
  if (scl_unlikely(!n))
    return NULL;
  n->data = scl_alloc(alloc, element_size, alignof(max_align_t));
  if (scl_unlikely(!n->data)) {
    scl_free(alloc, n);
    return NULL;
  }
...
static __attribute__((unused)) scl_avl_node_t *
scl_avl_balance_node(scl_avl_node_t *n, const void *data, scl_cmp_func_t cmp) {
  (void)data;
  (void)cmp;
...
scl_error_t scl_avl_insert(scl_allocator_t *alloc, scl_avl_t *tree,
                           const void *SCL_RESTRICT element) {
  if (scl_unlikely(!tree || !element))
    return SCL_ERR_NULL_PTR;

  scl_avl_node_t *node =
      scl_avl_create_node(alloc, element, tree->element_size);
...
static __attribute__((unused)) SCL_ALWAYS_INLINE scl_avl_node_t *
scl_avl_min_node(scl_avl_node_t *n) {
  while (n && n->left)
    n = n->left;
  return n;
}

scl_error_t scl_avl_remove(scl_allocator_t *alloc, scl_avl_t *tree,
  ...
  if (!node->left || !node->right) {
    ...
    while (scl_likely(succ->left)) {
      if (scl_unlikely(succ_idx >= 255))
        return SCL_ERR_INVALID_STATE;
      if (succ_idx == path_len)
        path[succ_idx] = node;
      path[succ_idx++] = succ;
      succ = succ->left;
    }
...
scl_error_t scl_avl_find(const scl_avl_t *tree, const void *key,
...
scl_error_t scl_avl_min(const scl_avl_t *tree, void *out) {
  if (scl_unlikely(!tree || !out))
    return SCL_ERR_NULL_PTR;
  if (scl_unlikely(!tree->root))
    return SCL_ERR_EMPTY;
  scl_avl_node_t *cur = tree->root;
  while (scl_likely(cur->left))
    cur = cur->left;
  scl_memcpy(out, cur->data, tree->element_size);
  return SCL_OK;
}
...
size_t scl_avl_count(const scl_avl_t *tree) {
  return scl_likely(tree) ? tree->count : 0;
}
bool scl_avl_empty(const scl_avl_t *tree) {
  return scl_likely(tree) ? tree->count == 0 : true;
}

scl_error_t scl_avl_inorder(const scl_avl_t *tree, scl_visit_func_t visit,
                            void *SCL_RESTRICT ctx) {
  if (scl_unlikely(!tree || !visit))
    return SCL_ERR_NULL_PTR;
