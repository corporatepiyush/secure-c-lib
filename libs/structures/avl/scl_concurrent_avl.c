...
/* Thread-safe avl data structure. Guarded by scl_spinlock_t. */
...
static int balance(scl_concurrent_avl_node_t *n) {
...
static scl_concurrent_avl_node_t *
create_node(scl_allocator_t *alloc, const void *data, size_t element_size) {
  scl_concurrent_avl_node_t *n =
      scl_alloc(alloc, sizeof(scl_concurrent_avl_node_t), alignof(max_align_t));
  if (!n)
    return NULL;
...
void scl_cavl_destroy(scl_allocator_t *alloc, scl_concurrent_avl_t *tree) {
  ...
  if (tree->root) {
    size_t cap = 64;
    scl_concurrent_avl_node_t **stack =
...
scl_error_t scl_cavl_init(scl_allocator_t *alloc, scl_concurrent_avl_t *tree,
...
scl_error_t scl_cavl_insert(scl_allocator_t *alloc, scl_concurrent_avl_t *tree,
                            const void *SCL_RESTRICT element) {
  if (scl_unlikely(!tree || !element))
    return SCL_ERR_NULL_PTR;
  scl_spinlock_lock(&tree->lock);

  scl_concurrent_avl_node_t *node =
...
scl_error_t scl_cavl_remove(scl_allocator_t *alloc, scl_concurrent_avl_t *tree,
...
scl_error_t scl_cavl_find(scl_concurrent_avl_t *tree, const void *key,
