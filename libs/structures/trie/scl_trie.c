...
scl_error_t scl_trie_init(scl_allocator_t *alloc, scl_trie_t *trie,
                          size_t value_size) {
  if (scl_unlikely(!trie))
    return SCL_ERR_NULL_PTR;

  trie->root =
      scl_calloc(alloc, 1, sizeof(scl_trie_node_t), alignof(max_align_t));
...
scl_error_t scl_trie_insert(scl_allocator_t *alloc, scl_trie_t *trie,
...
scl_error_t scl_trie_get(const scl_trie_t *trie, const unsigned char *key,
  ...
  if (scl_unlikely(!node->terminal || !node->value))
    return SCL_ERR_NOT_FOUND;
  scl_memcpy(out_value, node->value, trie->value_size);
  return SCL_OK;
}

bool scl_trie_contains(const scl_trie_t *trie, const unsigned char *key,
                       size_t key_len) {
...
scl_error_t scl_trie_remove(scl_allocator_t *alloc, scl_trie_t *trie,
                            const unsigned char *SCL_RESTRICT key,
                            size_t key_len) {
  if (scl_unlikely(!trie || !key))
    return SCL_ERR_NULL_PTR;
  if (scl_unlikely(!scl_trie_contains(trie, key, key_len)))
    return SCL_ERR_NOT_FOUND;
  ...
  for (int i = path_len - 1; i > 0; i--) {
    ...
    if (n->terminal || scl_trie_has_children(n))
      break;
...
size_t scl_trie_count(const scl_trie_t *trie) { return trie ? trie->count : 0; }
