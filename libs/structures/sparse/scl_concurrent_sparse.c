...
scl_error_t scl_csparse_init(scl_allocator_t *alloc,
                             scl_concurrent_sparse_t *st, size_t n,
                             size_t element_size, const void *data,
                             void (*combine)(void *out, const void *a,
                                             const void *SCL_RESTRICT b)) {
  if (scl_unlikely(!st || !combine || !data))
    return SCL_ERR_NULL_PTR;
  if (scl_unlikely(n == 0 || element_size == 0))
    return SCL_ERR_INVALID_ARG;

  size_t levels_count = 0;
  while ((1UL << levels_count) <= n)
    levels_count++;

  st->levels = scl_calloc(alloc, levels_count, sizeof(unsigned char *),
                          alignof(max_align_t));
...
scl_error_t scl_csparse_query(const scl_concurrent_sparse_t *st, size_t l,
                              size_t r, void *SCL_RESTRICT out) {
  if (scl_unlikely(!st || !out))
    return SCL_ERR_NULL_PTR;
  if (scl_unlikely(l > r || r >= st->n))
    return SCL_ERR_INVALID_ARG;
  ...
  st->combine(st->scratch, st->levels[k] + l * esize,
