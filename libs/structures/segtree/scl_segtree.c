...
scl_error_t scl_segtree_init(scl_allocator_t *alloc, scl_segtree_t *tree,
  ...
  size_t size = 1;
  while (size < n) {
    if (scl_unlikely(size > SIZE_MAX / 2))
...
scl_error_t scl_segtree_update(scl_segtree_t *tree, size_t idx,
                               const void *SCL_RESTRICT val) {
...
scl_error_t scl_segtree_query(const scl_segtree_t *tree, size_t l, size_t r,
