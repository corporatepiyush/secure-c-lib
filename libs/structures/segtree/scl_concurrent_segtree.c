...
scl_error_t scl_csegtree_init(scl_allocator_t *alloc,
                              scl_concurrent_segtree_t *tree, size_t n,
                              size_t element_size, const void *data,
                              void (*combine)(void *out, const void *a,
                                              const void *SCL_RESTRICT b)) {
  if (scl_unlikely(!tree || !combine || !data))
  ...
  for (size_t i = 0; i < n; i++)
    scl_memcpy(tree->data + (size + i) * element_size, src + i * element_size,
               element_size);

  for (size_t i = size - 1; i >= 1; i--)
    combine(tree->data + i * element_size, tree->data + (2 * i) * element_size,
            tree->data + (2 * i + 1) * element_size);

  return SCL_OK;
}

void scl_csegtree_destroy(scl_allocator_t *alloc,
                          scl_concurrent_segtree_t *tree) {
  if (scl_unlikely(!tree || !tree->data))
    return;
...
scl_error_t scl_csegtree_update(scl_allocator_t *alloc,
                                ...
                                const void *val) {
  (void)alloc;
  if (scl_unlikely(!tree || !val))
    return SCL_ERR_NULL_PTR;
  if (scl_unlikely(idx >= tree->n))
  ...
  for (p >>= 1; p >= 1; p >>= 1)
    tree->combine(tree->data + p * esize, tree->data + (2 * p) * esize,
...
scl_error_t scl_csegtree_query(const scl_concurrent_segtree_t *tree, size_t l,
                               size_t r, void *SCL_RESTRICT out) {
  if (scl_unlikely(!tree || !out))
    return SCL_ERR_NULL_PTR;
  if (scl_unlikely(l >= r || r > tree->n))
    return SCL_ERR_INVALID_ARG;
