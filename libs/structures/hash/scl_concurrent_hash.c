...
scl_error_t scl_chash_init(scl_allocator_t *alloc, scl_concurrent_hash_t *ht,
                           size_t key_size, size_t value_size,
                           size_t bucket_count, scl_hash_func_t hf) {
  if (scl_unlikely(!ht))
    return SCL_ERR_NULL_PTR;
  if (scl_unlikely(key_size == 0 || value_size == 0 || bucket_count == 0))
    return SCL_ERR_INVALID_ARG;
  size_t nb = scl_bit_ceil_sz(bucket_count);
  ht->entries = scl_calloc(alloc, nb, sizeof(scl_concurrent_hash_entry_t *),
                           alignof(max_align_t));
  if (scl_unlikely(!ht->entries))
    return SCL_ERR_OUT_OF_MEMORY;
  ht->buckets = scl_calloc(alloc, nb, sizeof(scl_concurrent_hash_bucket_t),
                           alignof(max_align_t));
...
scl_error_t scl_chash_insert(scl_allocator_t *alloc, scl_concurrent_hash_t *ht,
  ...
  scl_concurrent_hash_entry_t *n = scl_alloc(
      alloc, sizeof(scl_concurrent_hash_entry_t), alignof(max_align_t));
  if (!n) {
    scl_spinlock_unlock(&buckets[idx].lock);
    return SCL_ERR_OUT_OF_MEMORY;
  }
  ...
  if (scl_unlikely(!n->key || !n->value)) {
    scl_free(alloc, n->key);
    scl_free(alloc, n->value);
    scl_free(alloc, n);
    scl_spinlock_unlock(&buckets[idx].lock);
    return SCL_ERR_OUT_OF_MEMORY;
  }
...
scl_error_t scl_chash_get(const scl_concurrent_hash_t *ht, const void *key,
...
scl_error_t scl_chash_remove(scl_allocator_t *alloc, scl_concurrent_hash_t *ht,
