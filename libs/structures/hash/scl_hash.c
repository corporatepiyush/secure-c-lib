...
bool scl_hash_eq_mem(const void *a, const void *b, size_t key_size) {
  return scl_memcmp(a, b, key_size) == 0;
}

static SCL_COLD_PATH scl_error_t scl_hash_grow(scl_allocator_t *alloc,
                                               scl_hash_t *ht) {
  if (scl_unlikely(ht->capacity > SIZE_MAX / 2))
  ...
  scl_hash_slot_state_t *new_states =
      scl_alloc(alloc, ssz_total, alignof(max_align_t));
  ...
  scl_memset(new_states, 0, ssz_total);
  ...
  for (size_t i = 0; i < ht->capacity; i++) {
    if (ht->states[i] == SCL_HASH_OCCUPIED) {
      ...
      scl_memcpy(new_vals + idx * vsz, ht->values + i * vsz, vsz);
...
scl_error_t scl_hash_init(scl_allocator_t *alloc, scl_hash_t *ht,
  ...
  ht->states = scl_calloc(alloc, cap, sizeof(scl_hash_slot_state_t),
                          alignof(max_align_t));
  ...
  ht->seed = ((uint64_t)(unsigned int)scl_rand() << 32) |
             (uint64_t)(unsigned int)scl_rand();
  ht->key_size = ksz;
  ht->value_size = vsz;
  return SCL_OK;
}
...
void scl_hash_destroy(scl_allocator_t *alloc, scl_hash_t *ht) {
  if (scl_unlikely(!ht))
    return;
  if (ht->keys)
    scl_secure_zero(ht->keys, ht->capacity * ht->key_size);
  if (ht->values)
    scl_secure_zero(ht->values, ht->capacity * ht->value_size);
  scl_free(alloc, ht->keys);
  scl_free(alloc, ht->values);
  scl_free(alloc, ht->states);
  ht->keys = NULL;
...
scl_error_t scl_hash_insert(scl_allocator_t *alloc, scl_hash_t *ht,
                            const void *SCL_RESTRICT key, const void *value) {
  if (scl_unlikely(!ht || !key || !value))
    return SCL_ERR_NULL_PTR;
  ...
  for (;;) {
    ...
    if (st == SCL_HASH_EMPTY || st == SCL_HASH_TOMBSTONE) {
      scl_memcpy(ht->keys + idx * ksz, key, ksz);
      scl_memcpy(ht->values + idx * vsz, value, vsz);
...
scl_error_t scl_hash_get(const scl_hash_t *ht, const void *key,
                         void *SCL_RESTRICT out_value) {
  if (scl_unlikely(!ht || !key || !out_value))
    return SCL_ERR_NULL_PTR;
  ...
  for (;;) {
    scl_hash_slot_state_t st = ht->states[idx];
    if (st == SCL_HASH_EMPTY)
      break;
    if (st == SCL_HASH_OCCUPIED && eq(ht->keys + idx * ksz, key, ksz)) {
      scl_memcpy(out_value, ht->values + idx * ht->value_size, ht->value_size);
      return SCL_OK;
    }
    idx = (idx + 1) & mask;
    if (idx == start)
...
scl_error_t scl_hash_remove(scl_allocator_t *alloc, scl_hash_t *ht,
                            const void *SCL_RESTRICT key) {
  (void)alloc;
  if (scl_unlikely(!ht || !key))
    return SCL_ERR_NULL_PTR;
...
size_t scl_hash_count(const scl_hash_t *ht) { return ht ? ht->count : 0; }
