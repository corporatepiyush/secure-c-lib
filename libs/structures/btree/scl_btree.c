                                   ...
                                   size_t ksz, size_t vsz)
{
    ...
    for (size_t j = parent->count; j > i; j--) {
        scl_memcpy(pk + j * ksz, pk + (j - 1) * ksz, ksz);
        scl_memcpy(pv + j * vsz, pv + (j - 1) * vsz, vsz);
    }
    ...
    for (;;) {
        ...
        if (node->leaf) {
            ...
            while (i >= 0 && cmp(key, nk + (size_t)i * ksz) < 0) {
                scl_memcpy(nk + (size_t)(i + 1) * ksz, nk + (size_t)i * ksz, ksz);
                scl_memcpy(nv + (size_t)(i + 1) * vsz, nv + (size_t)i * vsz, vsz);
                i--;
            }
            i++;
            if ((size_t)i < node->count && cmp(key, nk + (size_t)i * ksz) == 0) {
                scl_memcpy(nv + (size_t)i * vsz, value, vsz);
                return SCL_OK;
            }
            scl_memcpy(nk + (size_t)i * ksz, key, ksz);
            scl_memcpy(nv + (size_t)i * vsz, value, vsz);
            node->count++;
            break;
        } else {
