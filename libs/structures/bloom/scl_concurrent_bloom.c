...
scl_error_t scl_cbloom_init(scl_allocator_t *alloc, scl_concurrent_bloom_t *bf,
  ...
  if (num_hashes > 64)
    num_hashes = 64;
  bf->bits =
      scl_calloc(alloc, bytes, sizeof(atomic_uchar), alignof(max_align_t));
...
scl_error_t scl_cbloom_insert(scl_concurrent_bloom_t *bf, const void *data,
                              size_t len) {
  ...
  for (size_t i = 0; i < bf->num_hashes; i++) {
    ...
    atomic_fetch_or_explicit(&bf->bits[byte_idx], bit_mask,
...
bool scl_cbloom_maybe_contains(const scl_concurrent_bloom_t *bf,
  ...
  for (size_t i = 0; i < bf->num_hashes; i++) {
    ...
    unsigned char bit_mask = (unsigned char)(1 << (h % 8));
    unsigned char val =
        atomic_load_explicit(&bf->bits[byte_idx], memory_order_relaxed);
    if (!(val & bit_mask))
      return false;
  }
  return true;
