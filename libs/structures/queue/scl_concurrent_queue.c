...
scl_error_t scl_cqueue_init(scl_allocator_t *alloc,
                            scl_concurrent_queue_t *queue,
                            size_t element_size) {
  if (scl_unlikely(!queue))
    return SCL_ERR_NULL_PTR;
  if (scl_unlikely(element_size == 0))
    return SCL_ERR_INVALID_ARG;
  scl_concurrent_queue_node_t *dummy = scl_alloc(
      alloc, sizeof(scl_concurrent_queue_node_t), alignof(max_align_t));
  if (scl_unlikely(!dummy))
...
void scl_cqueue_destroy(scl_allocator_t *alloc, scl_concurrent_queue_t *queue) {
  ...
  scl_concurrent_queue_node_t *cur =
      (scl_concurrent_queue_node_t *)atomic_load_explicit(&queue->head,
                                                          memory_order_acquire);
  while (scl_likely(cur)) {
    scl_concurrent_queue_node_t *next =
        (scl_concurrent_queue_node_t *)atomic_load_explicit(
            &cur->next, memory_order_relaxed);
    if (cur->data)
      scl_secure_zero(cur->data, esz);
...
scl_error_t scl_cqueue_enqueue(scl_allocator_t *alloc,
                               scl_concurrent_queue_t *queue,
                               const void *SCL_RESTRICT element) {
  if (scl_unlikely(!queue || !element))
    return SCL_ERR_NULL_PTR;
  scl_concurrent_queue_node_t *node = scl_alloc(
      alloc, sizeof(scl_concurrent_queue_node_t), alignof(max_align_t));
  ...
  while (1) {
    scl_concurrent_queue_node_t *tail =
        (scl_concurrent_queue_node_t *)atomic_load_explicit(
            &queue->tail, memory_order_acquire);
    scl_concurrent_queue_node_t *next =
        (scl_concurrent_queue_node_t *)atomic_load_explicit(
            &tail->next, memory_order_acquire);
    if (tail != (scl_concurrent_queue_node_t *)atomic_load_explicit(
                    &queue->tail, memory_order_acquire))
      continue;
    if (next == NULL) {
      uintptr_t expected = (uintptr_t)NULL;
      if (atomic_compare_exchange_weak_explicit(
              &tail->next, &expected, (uintptr_t)node, memory_order_release,
              memory_order_relaxed))
        break;
    } else {
      atomic_compare_exchange_weak_explicit(
          &queue->tail, (uintptr_t *)&tail, (uintptr_t)next,
          memory_order_release, memory_order_relaxed);
    }
  }
  scl_concurrent_queue_node_t *old_tail =
      (scl_concurrent_queue_node_t *)atomic_load_explicit(&queue->tail,
                                                          memory_order_relaxed);
  atomic_compare_exchange_weak_explicit(&queue->tail, (uintptr_t *)&old_tail,
                                        (uintptr_t)node, memory_order_release,
                                        memory_order_relaxed);
  atomic_fetch_add_explicit(&queue->count, 1, memory_order_relaxed);
  return SCL_OK;
}

scl_error_t scl_cqueue_dequeue(scl_allocator_t *alloc,
                               scl_concurrent_queue_t *queue,
                               void *SCL_RESTRICT out) {
  if (scl_unlikely(!queue || !out))
    return SCL_ERR_NULL_PTR;
  while (1) {
    scl_concurrent_queue_node_t *head =
        (scl_concurrent_queue_node_t *)atomic_load_explicit(
    ...
    if (head != (scl_concurrent_queue_node_t *)atomic_load_explicit(
                    &queue->head, memory_order_acquire))
      continue;
    ...
    if (head == tail) {
      ...
      if (atomic_compare_exchange_weak_explicit(
              &queue->head, &old_head, (uintptr_t)next, memory_order_acq_rel,
              memory_order_relaxed)) {
...
bool scl_cqueue_empty(const scl_concurrent_queue_t *queue) {
  ...
  scl_concurrent_queue_node_t *head =
      (scl_concurrent_queue_node_t *)atomic_load_explicit(&queue->head,
                                                          memory_order_acquire);
  scl_concurrent_queue_node_t *next =
      (scl_concurrent_queue_node_t *)atomic_load_explicit(&head->next,
                                                          memory_order_acquire);
  return next == NULL;
}
