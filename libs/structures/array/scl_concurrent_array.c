...
scl_error_t scl_carray_init(scl_allocator_t *alloc, scl_concurrent_array_t *arr,
...
scl_error_t scl_carray_push(scl_allocator_t *alloc, scl_concurrent_array_t *arr,
  ...
  while (1) {
    if (scl_unlikely(old >= arr->capacity))
      return SCL_ERR_FULL;
...
scl_error_t scl_carray_get(const scl_concurrent_array_t *arr, size_t index,
...
scl_error_t scl_carray_set(scl_concurrent_array_t *arr, size_t index,
                           const void *SCL_RESTRICT element) {
  if (scl_unlikely(!arr || !element))
    return SCL_ERR_NULL_PTR;
  size_t cnt = atomic_load_explicit(&arr->count, memory_order_acquire);
