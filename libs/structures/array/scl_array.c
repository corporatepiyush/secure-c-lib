...
scl_error_t scl_array_init(scl_allocator_t *alloc, scl_array_t *arr,
...
SCL_COLD_PATH scl_error_t scl_array_reserve(scl_allocator_t *alloc,
                                            scl_array_t *arr,
                                            size_t new_capacity) {
  if (scl_unlikely(!arr))
    return SCL_ERR_NULL_PTR;
  ...
  unsigned char *tmp =
      scl_realloc(alloc, arr->data, old_bytes, new_bytes, alignof(max_align_t));
  if (scl_unlikely(!tmp))
    return SCL_ERR_OUT_OF_MEMORY;
...
scl_error_t scl_array_push(scl_allocator_t *alloc, scl_array_t *arr,
                           const void *SCL_RESTRICT element) {
  if (scl_unlikely(!arr || !element))
    return SCL_ERR_NULL_PTR;
...
scl_error_t scl_array_pop(scl_array_t *arr, void *out) {
  if (scl_unlikely(!arr || !out))
    return SCL_ERR_NULL_PTR;
  if (scl_unlikely(arr->count == 0))
...
scl_error_t scl_array_get(const scl_array_t *arr, size_t index,
                          void *SCL_RESTRICT out) {
  if (scl_unlikely(!arr || !out))
    return SCL_ERR_NULL_PTR;
  if (scl_unlikely(index >= arr->count))
...
scl_error_t scl_array_set(scl_array_t *arr, size_t index,
                          const void *SCL_RESTRICT element) {
  if (scl_unlikely(!arr || !element))
    return SCL_ERR_NULL_PTR;
  if (scl_unlikely(index >= arr->count))
...
scl_error_t scl_array_insert(scl_allocator_t *alloc, scl_array_t *arr,
                             size_t index, const void *SCL_RESTRICT element) {
  if (scl_unlikely(!arr || !element))
    return SCL_ERR_NULL_PTR;
  if (scl_unlikely(index > arr->count))
    return SCL_ERR_INVALID_INDEX;
  ...
  if (tail > 0) {
    scl_memmove(arr->data + (index + 1) * es, arr->data + index * es,
                tail * es);
  }
...
scl_error_t scl_array_remove(scl_array_t *arr, size_t index,
                             void *SCL_RESTRICT out) {
  if (scl_unlikely(!arr || !out))
    return SCL_ERR_NULL_PTR;
  if (scl_unlikely(index >= arr->count))
  ...
  if (tail > 0) {
    scl_memmove(arr->data + index * es, arr->data + (index + 1) * es,
                tail * es);
  }
  arr->count--;
  return SCL_OK;
...
size_t scl_array_count(const scl_array_t *arr) { return arr ? arr->count : 0; }
...
scl_error_t scl_array_linear_search(const scl_array_t *restrict arr,
...
scl_error_t scl_array_binary_search(const scl_array_t *restrict arr,
                                    const void *restrict key,
                                    int (*cmp)(const void *, const void *),
                                    size_t *restrict out_index) {
  if (scl_unlikely(!arr || !key || !cmp || !out_index))
