...
scl_error_t scl_sharded_array_init(scl_allocator_t *alloc, scl_sharded_array_t *sa,
                                   size_t elem_size, size_t shard_len)
{
    if (scl_unlikely(!sa)) return SCL_ERR_NULL_PTR;
    if (scl_unlikely(elem_size == 0)) return SCL_ERR_INVALID_ARG;
    if (shard_len == 0) shard_len = SCL_SHARDED_ARRAY_DEFAULT_SHARD;
...
scl_error_t scl_sharded_array_append(scl_sharded_array_t *sa, const void *elem, size_t *out_index)
{
    if (scl_unlikely(!sa || !elem)) return SCL_ERR_NULL_PTR;
    ...
    if (shard >= sa->shard_count) {
        ...
        if (sa->shard_count >= sa->dir_cap) {
            ...
            size_t nbytes;
            if (scl_unlikely(scl_mul_overflow(ndir, sizeof(unsigned char *), &nbytes)))
                return SCL_ERR_SIZE_OVERFLOW;
            unsigned char **nd = (unsigned char **)scl_realloc(sa->alloc, sa->shards,
                                     sa->dir_cap * sizeof(unsigned char *), nbytes,
                                     alignof(max_align_t));
            if (scl_unlikely(!nd)) return SCL_ERR_OUT_OF_MEMORY;
            sa->shards = nd;
            sa->dir_cap = ndir;
        }
        size_t sbytes = sa->shard_len * sa->elem_size;   /* overflow checked in init */
        unsigned char *blk = (unsigned char *)scl_alloc(sa->alloc, sbytes, alignof(max_align_t));
        if (scl_unlikely(!blk)) return SCL_ERR_OUT_OF_MEMORY;
        sa->shards[sa->shard_count++] = blk;
    }

    unsigned char *slot = sa->shards[shard] + off * sa->elem_size;
    scl_memcpy(slot, elem, sa->elem_size);
    if (out_index) *out_index = sa->count;
    sa->count++;
    return SCL_OK;
}
