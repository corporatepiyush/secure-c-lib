...
static inline void *scl_sharded_array_get(const scl_sharded_array_t *sa,
                                          size_t index) {
  if (scl_unlikely(!sa || index >= sa->count))
    return NULL;
  return sa->shards[index / sa->shard_len] +
         (index % sa->shard_len) * sa->elem_size;
}
