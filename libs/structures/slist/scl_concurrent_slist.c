...
scl_error_t scl_cslist_init(scl_allocator_t *alloc,
                            scl_concurrent_slist_t *list, size_t element_size) {
  (void)alloc;
...
void scl_cslist_destroy(scl_allocator_t *alloc, scl_concurrent_slist_t *list) {
  ...
  atomic_store_explicit(&list->head, scl_tagged_make(NULL, 0),
                        memory_order_relaxed);
  atomic_store_explicit(&list->count, 0, memory_order_relaxed);
}

scl_error_t scl_cslist_push_front(scl_allocator_t *alloc,
                                  scl_concurrent_slist_t *list,
                                  const void *SCL_RESTRICT element) {
  if (scl_unlikely(!list || !element))
    return SCL_ERR_NULL_PTR;
  scl_concurrent_slist_node_t *node = scl_alloc(
      alloc, sizeof(scl_concurrent_slist_node_t), alignof(max_align_t));
  ...
  do {
    node->next = old.ptr;
      ...
      &list->head, &old, scl_tagged_make(node, old.tag + 1),
...
scl_error_t scl_cslist_pop_front(scl_allocator_t *alloc,
                                 scl_concurrent_slist_t *list,
  ...
  while (1) {
    ...
    scl_concurrent_slist_node_t *next =
        ((scl_concurrent_slist_node_t *)old.ptr)->next;
    if (atomic_compare_exchange_weak_explicit(
            &list->head, &old, scl_tagged_make(next, old.tag + 1),
            memory_order_acquire, memory_order_relaxed))
      break;
  }
  scl_memcpy(out, ((scl_concurrent_slist_node_t *)old.ptr)->data,
             list->element_size);
