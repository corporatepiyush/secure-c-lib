...
scl_error_t scl_skiplist_init(scl_allocator_t *alloc, scl_skiplist_t *sl,
  ...
  if (scl_unlikely(!sl || !cmp))
    return SCL_ERR_NULL_PTR;
  if (scl_unlikely(element_size == 0))
    return SCL_ERR_INVALID_ARG;

  sl->head =
      scl_alloc(alloc, sizeof(scl_skiplist_node_t), alignof(max_align_t));
  if (scl_unlikely(!sl->head))
    return SCL_ERR_OUT_OF_MEMORY;

  sl->head->data = NULL;
  sl->head->forward =
      scl_calloc(alloc, SCL_SKIPLIST_MAX_LEVEL, sizeof(scl_skiplist_node_t *),
                 alignof(max_align_t));
...
scl_error_t scl_skiplist_insert(scl_allocator_t *alloc, scl_skiplist_t *sl,
  ...
  node->forward = scl_calloc(alloc, new_level, sizeof(scl_skiplist_node_t *),
                             alignof(max_align_t));
  if (!node->forward) {
...
scl_error_t scl_skiplist_remove(scl_allocator_t *alloc, scl_skiplist_t *sl,
...
scl_error_t scl_skiplist_find(const scl_skiplist_t *sl, const void *key,
  ...
  cur = cur->forward[0];
  if (scl_unlikely(!cur || sl->cmp(cur->data, key) != 0))
    return SCL_ERR_NOT_FOUND;
