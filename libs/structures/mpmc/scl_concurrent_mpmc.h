...
/* Vyukov bounded MPMC queue. Lock-free, power-of-2 sized. Cache-line padded
 * enqueue/dequeue. O(1) bounded capacity. */
...
static inline scl_error_t scl_mpmc_init(scl_allocator_t *alloc,
  ...
  if (scl_unlikely(!q || !alloc))
    return SCL_ERR_NULL_PTR;
  if (capacity < 2)
    capacity = 2;
  capacity = scl_bit_ceil_sz(capacity); /* round up to power of two */
  q->buffer = (scl_mpmc_cell_t *)scl_calloc(
      alloc, capacity, sizeof(scl_mpmc_cell_t), _Alignof(scl_mpmc_cell_t));
  if (!q->buffer)
