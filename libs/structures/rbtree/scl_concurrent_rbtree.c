...
static scl_concurrent_rbtree_node_t *
create_node(scl_allocator_t *alloc, const void *data, size_t element_size) {
  scl_concurrent_rbtree_node_t *n = scl_alloc(
      alloc, sizeof(scl_concurrent_rbtree_node_t), alignof(max_align_t));
  if (!n)
    return NULL;
...
void scl_crbtree_destroy(scl_allocator_t *alloc,
  ...
  if (tree->root) {
    size_t cap = 64;
    scl_concurrent_rbtree_node_t **stack =
        scl_alloc(alloc, cap * sizeof(stack[0]), alignof(max_align_t));
    if (!stack) {
    ...
    while (cur || sp >= 0) {
      while (scl_likely(cur)) {
        if ((size_t)(sp + 1) >= cap) {
          ...
          scl_concurrent_rbtree_node_t **ns =
              scl_realloc(alloc, stack, cap * sizeof(stack[0]),
                          new_cap * sizeof(stack[0]), alignof(max_align_t));
          if (!ns) {
...
static void rotate_left(scl_concurrent_rbtree_node_t **root,
                        scl_concurrent_rbtree_node_t *x) {
...
static void rotate_right(scl_concurrent_rbtree_node_t **root,
                         scl_concurrent_rbtree_node_t *x) {
...
static void insert_fixup(scl_concurrent_rbtree_node_t **root,
                         scl_concurrent_rbtree_node_t *z) {
...
static void transplant(scl_concurrent_rbtree_node_t **root,
                       scl_concurrent_rbtree_node_t *u,
                       scl_concurrent_rbtree_node_t *v) {
  if (!u->parent)
    *root = v;
...
static void remove_fixup(scl_concurrent_rbtree_node_t **root,
                         scl_concurrent_rbtree_node_t *x,
                         scl_concurrent_rbtree_node_t *x_parent) {
...
scl_error_t scl_crbtree_insert(scl_allocator_t *alloc,
                               scl_concurrent_rbtree_t *tree,
                               const void *SCL_RESTRICT element) {
  if (scl_unlikely(!tree || !element))
    return SCL_ERR_NULL_PTR;
  scl_concurrent_rbtree_node_t *z =
      create_node(alloc, element, tree->element_size);
  if (scl_unlikely(!z))
  ...
  if (!y)
    tree->root = z;
  else if (tree->cmp(element, y->data) < 0)
    y->left = z;
  else
    y->right = z;
...
scl_error_t scl_crbtree_remove(scl_allocator_t *alloc,
                               scl_concurrent_rbtree_t *tree,
...
scl_error_t scl_crbtree_find(scl_concurrent_rbtree_t *tree, const void *key,
                             void *SCL_RESTRICT out) {
  if (scl_unlikely(!tree || !key || !out))
    return SCL_ERR_NULL_PTR;
  scl_spinlock_lock(&tree->lock);
  scl_concurrent_rbtree_node_t *cur = tree->root;
  while (scl_likely(cur)) {
