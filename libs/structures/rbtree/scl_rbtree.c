...
static SCL_ALWAYS_INLINE bool scl_rb_is_red(const scl_rbtree_node_t *n) {
  return n && n->color == SCL_RB_RED;
}

scl_error_t scl_rbtree_init(scl_allocator_t *alloc, scl_rbtree_t *tree,
...
static scl_rbtree_node_t *scl_rbtree_create_node(scl_allocator_t *alloc,
                                                 const void *data,
                                                 size_t element_size) {
  scl_rbtree_node_t *n =
      scl_alloc(alloc, sizeof(scl_rbtree_node_t), alignof(max_align_t));
  if (!n)
    return NULL;
...
scl_error_t scl_rbtree_insert(scl_allocator_t *alloc, scl_rbtree_t *tree,
                              const void *SCL_RESTRICT element) {
  if (scl_unlikely(!tree || !element))
    return SCL_ERR_NULL_PTR;

  scl_rbtree_node_t *z =
      scl_rbtree_create_node(alloc, element, tree->element_size);
  if (scl_unlikely(!z))
...
static SCL_ALWAYS_INLINE bool
scl_rb_is_black_or_null(const scl_rbtree_node_t *n) {
  return !n || n->color == SCL_RB_BLACK;
}

static void scl_rbtree_remove_fixup(scl_rbtree_t *tree, scl_rbtree_node_t *x,
  ...
  while (x != tree->root && scl_rb_is_black_or_null(x)) {
    ...
    if (!w || (scl_rb_is_black_or_null(w->left) &&
               scl_rb_is_black_or_null(w->right))) {
      if (w)
        w->color = SCL_RB_RED;
      x = x_parent;
...
scl_error_t scl_rbtree_remove(scl_allocator_t *alloc, scl_rbtree_t *tree,
...
scl_error_t scl_rbtree_find(const scl_rbtree_t *tree, const void *key,
...
scl_error_t scl_rbtree_inorder(const scl_rbtree_t *tree, scl_visit_func_t visit,
