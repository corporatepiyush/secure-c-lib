...
scl_error_t scl_heap_init(scl_allocator_t *alloc, scl_heap_t *heap,
                          size_t element_size, size_t initial_capacity,
                          scl_cmp_func_t cmp) {
  if (scl_unlikely(!heap || !cmp))
...
static SCL_ALWAYS_INLINE size_t scl_heap_parent(size_t i) {
  return (i - 1) / 2;
}
...
scl_error_t scl_heap_push(scl_allocator_t *alloc, scl_heap_t *heap,
  ...
  if (scl_unlikely(cnt == heap->capacity)) {
    ...
    if (scl_mul_overflow(new_cap, es, &new_bytes))
      return SCL_ERR_SIZE_OVERFLOW;
    unsigned char *tmp = scl_realloc(alloc, heap->data, old_bytes, new_bytes,
                                     alignof(max_align_t));
    if (scl_unlikely(!tmp))
      return SCL_ERR_OUT_OF_MEMORY;
    heap->data = tmp;
...
bool scl_heap_empty(const scl_heap_t *heap) {
  return heap ? heap->count == 0 : true;
}

scl_error_t scl_heap_search(const scl_heap_t *restrict heap,
                            const void *restrict key, scl_cmp_func_t cmp,
                            size_t *restrict out_index) {
  if (scl_unlikely(!heap || !key || !cmp || !out_index))
    return SCL_ERR_NULL_PTR;
