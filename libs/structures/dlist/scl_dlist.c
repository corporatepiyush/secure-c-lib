...
static scl_error_t scl_dlist_create_node(scl_allocator_t *alloc,
                                         scl_dlist_node_t **out,
                                         const void *data,
                                         size_t element_size) {
  size_t node_sz;
...
scl_error_t scl_dlist_push_front(scl_allocator_t *alloc, scl_dlist_t *list,
...
scl_error_t scl_dlist_push_back(scl_allocator_t *alloc, scl_dlist_t *list,
                                const void *SCL_RESTRICT element) {
  if (scl_unlikely(!list || !element))
    return SCL_ERR_NULL_PTR;
  size_t esz = list->element_size;
...
scl_error_t scl_dlist_pop_front(scl_allocator_t *alloc, scl_dlist_t *list,
...
scl_error_t scl_dlist_pop_back(scl_allocator_t *alloc, scl_dlist_t *list,
                               void *SCL_RESTRICT out) {
  if (scl_unlikely(!list || !out))
    return SCL_ERR_NULL_PTR;
...
scl_error_t scl_dlist_insert_at(scl_allocator_t *alloc, scl_dlist_t *list,
                                size_t index,
                                const void *SCL_RESTRICT element) {
  if (scl_unlikely(!list || !element))
    return SCL_ERR_NULL_PTR;
  if (scl_unlikely(index > list->count))
    return SCL_ERR_INVALID_INDEX;
  if (index == 0)
    return scl_dlist_push_front(alloc, list, element);
  size_t cnt = list->count;
  if (index == cnt)
    return scl_dlist_push_back(alloc, list, element);
  size_t esz = list->element_size;
...
scl_error_t scl_dlist_remove_at(scl_allocator_t *alloc, scl_dlist_t *list,
                                size_t index, void *SCL_RESTRICT out) {
  if (scl_unlikely(!list || !out))
    return SCL_ERR_NULL_PTR;
  if (scl_unlikely(index >= list->count))
    return SCL_ERR_INVALID_INDEX;
  if (index == 0)
    return scl_dlist_pop_front(alloc, list, out);
  size_t cnt = list->count;
  if (index == cnt - 1)
    return scl_dlist_pop_back(alloc, list, out);
  size_t esz = list->element_size;
  scl_dlist_node_t *cur;
  if (index <= cnt / 2) {
    cur = list->head;
...
scl_error_t scl_dlist_remove(scl_allocator_t *alloc, scl_dlist_t *list,
                             const void *element,
                             int (*cmp)(const void *, const void *)) {
  if (scl_unlikely(!list || !element || !cmp))
    return SCL_ERR_NULL_PTR;
  scl_dlist_node_t *cur = list->head;
  while (scl_likely(cur)) {
...
scl_error_t scl_dlist_search(const scl_dlist_t *restrict list,
