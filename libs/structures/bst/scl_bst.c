...
scl_error_t scl_bst_init(scl_allocator_t *alloc, scl_bst_t *tree,
...
static scl_error_t scl_bst_create_node(scl_allocator_t *alloc,
                                       scl_bst_node_t **out, const void *data,
                                       size_t element_size) {
  scl_bst_node_t *node = scl_alloc(alloc, sizeof(scl_bst_node_t) + element_size,
                                   alignof(max_align_t));
...
scl_error_t scl_bst_insert(scl_allocator_t *alloc, scl_bst_t *tree,
  ...
  scl_bst_node_t *node;
  scl_error_t err =
      scl_bst_create_node(alloc, &node, element, tree->element_size);
  if (err != SCL_OK)
    return err;
...
scl_error_t scl_bst_remove(scl_allocator_t *alloc, scl_bst_t *tree,
...
scl_error_t scl_bst_find(const scl_bst_t *tree, const void *key,
...
size_t scl_bst_count(const scl_bst_t *tree) { return tree ? tree->count : 0; }

bool scl_bst_empty(const scl_bst_t *tree) {
  return tree ? tree->count == 0 : true;
}

scl_error_t scl_bst_inorder(const scl_bst_t *tree, scl_visit_func_t visit,
