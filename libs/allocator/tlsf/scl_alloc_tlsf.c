...
static SCL_ALWAYS_INLINE void tlsf_mapping(size_t size, int *fl, int *sl) {
  if (scl_likely(size < TLSF_SMALL_BLOCK)) {
    ...
    *sl = (int)((size >> (s - TLSF_SL_LOG2)) ^ (1 << TLSF_SL_LOG2));
    if (scl_unlikely(*sl >= TLSF_SL_COUNT))
      *sl = TLSF_SL_COUNT - 1;
  }
}

static SCL_ALWAYS_INLINE void tlsf_mapping_insert(size_t size, int *fl,
                                                  int *sl) {
  if (scl_likely(size < TLSF_SMALL_BLOCK)) {
    *fl = 0;
    *sl = (int)(size / (TLSF_SMALL_BLOCK / TLSF_SL_COUNT));
...
static int tlsf_search_and_remove(tlsf_state_t *tlsf, size_t size,
                                  tlsf_block_hdr_t **out) {
  int fl, sl;
  ...
  if (scl_likely(remaining >= TLSF_MIN_BLOCK_SIZE)) {
    tlsf_block_hdr_t *new_block =
        (tlsf_block_hdr_t *)((unsigned char *)block + size);
    ...
    tlsf_block_hdr_t *next_phys =
        (tlsf_block_hdr_t *)((unsigned char *)new_block + remaining);
...
static void *tlsf_malloc_fn(void *state, size_t size, size_t alignment) {
   ...
   if (scl_unlikely(!t || size == 0))
     return NULL;

   /* Honour the requested alignment: the pool was aligned at creation time.
    * For over-aligned requests, verify the pool meets the requirement. */
   if (scl_unlikely(alignment > t->pool_alignment))
     return NULL;

   size_t aligned = size;
   size_t align = alignment > alignof(max_align_t) ? alignment : alignof(max_align_t);
   size_t mod = aligned % align;
   if (mod)
     aligned += align - mod;

  size_t req = TLSF_BLOCK_HDR_SZ + aligned;
  if (req < TLSF_MIN_BLOCK_SIZE)
    req = TLSF_MIN_BLOCK_SIZE;

  tlsf_block_hdr_t *block;
  if (scl_unlikely(!tlsf_search_and_remove(t, req, &block)))
    return NULL;

  return (void *)((unsigned char *)block + TLSF_BLOCK_HDR_SZ);
}

static void *tlsf_calloc_fn(void *state, size_t count, size_t size,
                            size_t alignment) {
  size_t total;
...
static void *tlsf_realloc_fn(void *state, void *ptr, size_t old_size,
                             size_t new_size, size_t alignment) {
  if (scl_unlikely(!ptr))
    return tlsf_malloc_fn(state, new_size, alignment);
...
static void tlsf_free_fn(void *state, void *ptr) {
  tlsf_state_t *t = (tlsf_state_t *)state;
  if (scl_unlikely(!t || !ptr))
    return;

  tlsf_block_hdr_t *block =
      (tlsf_block_hdr_t *)((unsigned char *)ptr - TLSF_BLOCK_HDR_SZ);
  size_t block_size = block->size & ~3UL;
  ...
  tlsf_block_hdr_t *next =
      (tlsf_block_hdr_t *)((unsigned char *)block + block_size);
...
scl_allocator_t *scl_alloc_tlsf_create(scl_allocator_t *backing,
                                        size_t memory_size,
                                        size_t alignment) {
   if (scl_unlikely(!backing || memory_size < 4096))
     return NULL;
   if (alignment == 0)
   ...
   tlsf_state_t *state = (tlsf_state_t *)backing->malloc_fn(
       backing->state, sizeof(tlsf_state_t), alignof(max_align_t));
...
void scl_alloc_tlsf_destroy(scl_allocator_t *alloc) {
  ...
  if (s->pool)
    backing->free_fn(backing->state, s->pool);
