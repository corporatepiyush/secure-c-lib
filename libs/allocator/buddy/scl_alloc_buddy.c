...
/* Binary buddy allocator (max order 20, min block 16 B). Buddy coalescing
 * eliminates external fragmentation. O(log N) alloc/free. Page-backed. */
...
static void *buddy_malloc_fn(void *state, size_t size, size_t alignment) {
   ...
   /* Honour the requested alignment: the buddy pool is always aligned to
    * the maximum power-of-two size, which is at least alignof(max_align_t).
    * For over-aligned requests, verify the pool meets the requirement. */
   if (scl_unlikely(alignment > b->pool_alignment))
     return NULL;
  ...
  if (scl_unlikely(current > b->max_order))
    return NULL;
...
static void *buddy_calloc_fn(void *state, size_t count, size_t size,
                             size_t alignment) {
  size_t total;
...
static void *buddy_realloc_fn(void *state, void *ptr, size_t old_size,
                              size_t new_size, size_t alignment) {
  if (scl_unlikely(!ptr))
    return buddy_malloc_fn(state, new_size, alignment);
...
static void buddy_free_fn(void *state, void *ptr) {
  ...
  scl_buddy_node_t *node =
      (scl_buddy_node_t *)((unsigned char *)ptr - BUDDY_HDR_SZ);
  unsigned int order = node->order;

  /* Security: wipe the block contents (header + payload) before returning
   * to the free list so no stale data leaks between users. */
  scl_secure_zero(node, (size_t)1 << order);
...
scl_allocator_t *scl_alloc_buddy_create(scl_allocator_t *backing,
                                         size_t total_size,
                                         size_t alignment) {
   if (scl_unlikely(!backing || total_size == 0))
     return NULL;
   if (alignment == 0)
   ...
   if (pool_sz < BUDDY_MIN_BLOCK)
     pool_sz = BUDDY_MIN_BLOCK;

   unsigned int max_order = (unsigned int)scl_log2_sz(pool_sz);
   if (scl_unlikely(max_order > BUDDY_MAX_ORDER))
     return NULL;

   buddy_state_t *state = (buddy_state_t *)backing->malloc_fn(
       backing->state, sizeof(buddy_state_t), alignof(max_align_t));
...
state->pool = (unsigned char *)backing->malloc_fn(backing->state, pool_sz,
                                                      alignment);
...
void scl_alloc_buddy_destroy(scl_allocator_t *alloc) {
  ...
  if (b->pool)
    backing->free_fn(backing->state, b->pool);
