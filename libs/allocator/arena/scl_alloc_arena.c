...
static SCL_ALWAYS_INLINE SCL_PURE size_t arena_align_up(size_t offset,
                                                        size_t align) {
  size_t mask = align - 1;
...
static arena_node_t *arena_node_create(arena_state_t *s, size_t capacity,
   ...
   arena_node_t *node = (arena_node_t *)b->malloc_fn(
       b->state, sizeof(arena_node_t), alignof(max_align_t));
   if (scl_unlikely(!node))
     return NULL;

   node->buffer = (char *)b->malloc_fn(b->state, capacity, alignment);
...
static void *arena_malloc_fn(void *state, size_t size, size_t alignment) {
  ...
  if (scl_unlikely(aligned > cur->capacity || size > cur->capacity - aligned)) {
    ...
    if (scl_unlikely(s->max_capacity > 0 &&
                     s->total_allocated + new_cap > s->max_capacity))
      return NULL;

    arena_node_t *node = arena_node_create(s, new_cap, alignment);
    if (scl_unlikely(!node))
      return NULL;
...
static void *arena_calloc_fn(void *state, size_t count, size_t size,
                             size_t alignment) {
  size_t total;
...
static void *arena_realloc_fn(void *state, void *ptr, size_t old_size,
                              size_t new_size, size_t alignment) {
  if (scl_unlikely(!ptr))
    return arena_malloc_fn(state, new_size, alignment);
...
scl_allocator_t *scl_alloc_arena_create(scl_allocator_t *backing,
   ...
   arena_state_t *state = (arena_state_t *)backing->malloc_fn(
       backing->state, sizeof(arena_state_t), alignof(max_align_t));
   ...
   state->head = arena_node_create(state, capacity, alignment);
