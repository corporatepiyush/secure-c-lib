...
/* Procedural noise: value/Perlin (1D/2D/3D), white noise, FBM. Permutation
 * table from PRNG. Multiple octaves with lacunarity scaling. */
...
static SCL_ALWAYS_INLINE SCL_PURE double grad3d(int hash, double x, double y,
                                                double z) {
  int h = hash & 15;
  double u = (h < 8) ? x : y;
...
double scl_rand_noise_perlin2d(scl_rand_noise_t *noise, double x, double y) {
  ...
  double x2 = lerp(grad2d(ab, fracx, fracy - 1.0),
                   grad2d(bb, fracx - 1.0, fracy - 1.0), u);
  return lerp(x1, x2, v);
}

double scl_rand_noise_perlin3d(scl_rand_noise_t *noise, double x, double y,
                               double z) {
  if (scl_unlikely(!noise))
    return 0.0;
  ...
  double v = fade(fracy);
  double w = fade(fracz);
  /* Use noise->perm directly instead of copying 2 KiB to the stack */
  int *p = noise->perm;
  ...
  double x11 = lerp(grad3d(aaa, fracx, fracy, fracz),
                    grad3d(baa, fracx - 1.0, fracy, fracz), u);
  double x12 = lerp(grad3d(aba, fracx, fracy - 1.0, fracz),
                    grad3d(bba, fracx - 1.0, fracy - 1.0, fracz), u);
  double x21 = lerp(grad3d(aab, fracx, fracy, fracz - 1.0),
                    grad3d(bab, fracx - 1.0, fracy, fracz - 1.0), u);
  double x22 = lerp(grad3d(abb, fracx, fracy - 1.0, fracz - 1.0),
                    grad3d(bbb, fracx - 1.0, fracy - 1.0, fracz - 1.0), u);
...
double scl_rand_noise_white(scl_rand_noise_t *noise, double x, double y) {
  if (scl_unlikely(!noise))
    return 0.0;
  /* Hash (x,y) into the PRNG state so different positions produce
   * different values (true white noise across a field).
   * Use multiplicative mixing to avoid correlated IEEE-754 bit patterns. */
  uint64_t sm = noise->seed;
  uint64_t hx;
  scl_memcpy(&hx, &x, sizeof(hx));
  uint64_t hy;
  scl_memcpy(&hy, &y, sizeof(hy));
  sm ^= hx * 0x9e3779b97f4a7c15ULL;
  sm ^= hy * 0xbf58476d1ce4e5b9ULL;
  uint64_t r = splitmix64(&sm);
  return (double)(r & 0x1FFFFFFFFFFFFFULL) / 9007199254740991.0 * 2.0 - 1.0;
}

double scl_rand_noise_fbm(scl_rand_noise_t *noise, double x, double y,
                          int octaves, double lacunarity, double gain) {
  if (scl_unlikely(!noise || octaves < 1))
    return 0.0;
  double total = 0.0;
  double amplitude = 1.0;
  double max_amplitude = 0.0;
  double px = x * noise->freq, py = y * noise->freq;
  for (int i = 0; i < octaves; i++) {
    total += amplitude * scl_rand_noise_perlin2d(noise, px, py);
