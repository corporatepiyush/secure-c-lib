...
static SCL_ALWAYS_INLINE void swap_elements(void *a, void *b,
                                            size_t elem_size) {
  unsigned char tmp[64];
  size_t offset = 0;
  while (offset < elem_size) {
    size_t chunk =
        (elem_size - offset < sizeof(tmp)) ? (elem_size - offset) : sizeof(tmp);
...
scl_error_t scl_rand_dist_sample(scl_allocator_t *alloc, scl_rand_dist_t *dist,
                                 size_t n, size_t k, size_t *out_indices) {
  if (scl_unlikely(!dist || !out_indices))
    return SCL_ERR_NULL_PTR;
  if (k > n)
  ...
  /* Reservoir sampling (Algorithm R) — O(n) time, O(k) memory. */
  size_t *reservoir =
      (size_t *)scl_calloc(alloc, k, sizeof(size_t), _Alignof(max_align_t));
  if (scl_unlikely(!reservoir))
  ...
  scl_memcpy(out_indices, reservoir, k * sizeof(size_t));
  scl_free(alloc, reservoir);
  return SCL_OK;
}
