...
static int docx_zip_find_file(scl_parse_docx_t *parser, const char *name,
  ...
  while (pos + ZIP_LOCAL_HDR_SZ <= sz) {
    ...
    if ((size_t)name_len + (size_t)extra_len > sz)
      break;
    if (pos + ZIP_LOCAL_HDR_SZ > sz - ((size_t)name_len + extra_len))
      break;
...
static int docx_append_text(scl_parse_docx_t *parser, const char *text,
                             size_t len) {
  if (len == 0)
    return 0;

  /* Enforce maximum extracted text length to prevent memory exhaustion
   * from a maliciously large document.xml. */
  if (parser->max_text_len && parser->text_len + len > parser->max_text_len) {
    if (parser->text_len >= parser->max_text_len)
      return 0; /* already at cap, discard further text */
    len = parser->max_text_len - parser->text_len;
  }

  while (parser->text_len + len + 1 > parser->text_cap) {
    size_t nc = parser->text_cap ? parser->text_cap * 2 : 4096;
    /* Cap capacity growth at max_text_len to avoid over-allocation */
    if (parser->max_text_len && nc > parser->max_text_len + 1)
      nc = parser->max_text_len + 1;
    char *nb = (char *)scl_realloc(parser->alloc, parser->text_buf,
                                    parser->text_cap, nc, _Alignof(max_align_t));
    if (!nb)
      return -1;
    parser->text_buf = nb;
    parser->text_cap = nc;
...
static void docx_extract_text_from_xml(scl_parse_docx_t *parser,
  ...
  /* Bound iterations on malformed input */
  size_t max_iters = xml_len;

  while (p < end && max_iters-- > 0) {
    if (*p == '<') {
      in_tag = 1;
      p++;
      continue;
    ...
    if (!in_tag) {
      ...
      while (p < end && *p != '<')
        p++;
      size_t text_len = (size_t)(p - start);
      /* Skip writing if we're already at the text cap */
      if (parser->max_text_len && parser->text_len >= parser->max_text_len)
        break;
      docx_append_text(parser, (const char *)start, text_len);
      if (p < end && *p == '<') {
...
scl_error_t scl_parse_docx_open(scl_allocator_t *alloc,
  ...
  parser->alloc = alloc;
  parser->max_text_len = SCL_DOCX_MAX_TEXT_LEN;
  ...
  /* Enforce maximum ZIP file size to prevent memory exhaustion. */
  if ((unsigned long long)sz > SCL_DOCX_MAX_ZIP_SIZE) {
    fclose(parser->fp);
    scl_free(alloc, parser->filename);
  ...
  parser->zip_buf = (unsigned char *)scl_alloc(alloc, parser->zip_size,
  ...
  if (fread(parser->zip_buf, 1, parser->zip_size, parser->fp) !=
      parser->zip_size) {
    fclose(parser->fp);
    scl_free(alloc, parser->filename);
    scl_free(alloc, parser->zip_buf);
    return SCL_ERR_ALLOC;
  }
  ...
  size_t xml_len = 0;
  if (docx_zip_find_file(parser, "word/document.xml", &xml_data, &xml_len) !=
      0) {
    if (docx_zip_find_file(parser, "/word/document.xml", &xml_data, &xml_len) !=
        0) {
    }
  ...
  if (xml_data) {
    ...
    if (parser->text_buf) {
      ...
      parser->text_cap =
          0; /* keep cap consistent so append() won't deref NULL */
    }
  }
