...
#define PARQUET_MAX_PATH_DEPTH 64
...
static int parquet_parse_footer(scl_parse_parquet_t *parser) {
  ...
  uint32_t footer_len = (uint32_t)buf[sz - 8] | ((uint32_t)buf[sz - 7] << 8) |
                         ((uint32_t)buf[sz - 6] << 16) |
                         ((uint32_t)buf[sz - 5] << 24);

  if (footer_len == 0 || footer_len > sz - 8)
    return -1;
  ...
  /* First pass: count total number of key-value items to validate schema
   * complexity before allocating anything. */
  int total_kv = 0;
  const unsigned char *scan = fp;
  while (scan < fend) {
    unsigned char ft = *scan++;
    if (ft == 0x00)
      break;
    ft = ft & 0x03;
    switch (ft) {
    case 0:
      parquet_read_varint(&scan, fend);
      break;
    case 1:
      scan = parquet_clamp(scan, 8, fend);
      break;
    case 2: {
      uint64_t len = parquet_read_varint(&scan, fend);
      scan = parquet_clamp(scan, len, fend);
      break;
    }
    case 3: {
      int depth = 1;
      while (depth > 0 && scan < fend) {
        unsigned char bt = *scan++;
        if (bt == 0x00)
          depth--;
        else if ((bt & 3) == 3)
          depth++;
  ...
  /* Enforce maximum schema complexity. */
  if (total_kv > (int)SCL_PARQUET_MAX_SCHEMA_ITEMS)
    return -1;
  ...
  while (fp < fend) {
    ...
    case 2: {
      ...
      if (id == 2) {
        ...
        if (elem_count > (uint64_t)(list_end - fp))
          elem_count = (uint64_t)(list_end - fp);
        parser->num_columns = (int)elem_count;
        parser->column_names =
            (char **)scl_calloc(parser->alloc, (size_t)elem_count,
                                sizeof(char *), _Alignof(max_align_t));
        parser->column_types =
            (int *)scl_calloc(parser->alloc, (size_t)elem_count, sizeof(int),
                              _Alignof(max_align_t));
        if (elem_count > 0 &&
            (!parser->column_names || !parser->column_types)) {
          ...
          while (fp < list_end) {
            ...
            if (stype == 2 && sid == 1 && ci < elem_count) {
              ...
              if (nlen > (uint64_t)(list_end - fp))
                nlen = (uint64_t)(list_end - fp);
              if (nlen > SCL_PARQUET_MAX_COLUMN_NAME)
                nlen = SCL_PARQUET_MAX_COLUMN_NAME;
              if (nlen > 0 && ci < elem_count) {
                parser->column_names[ci] = (char *)scl_alloc(
                    parser->alloc, (size_t)nlen + 1, _Alignof(max_align_t));
...
scl_error_t scl_parse_parquet_open(scl_allocator_t *alloc,
  ...
  long sz = ftell(fp);
  if (sz < 0) {
    scl_parse_parquet_close(parser);
    return SCL_ERR_ALLOC;
  }

  /* Enforce maximum Parquet file size. */
  if ((unsigned long long)sz > SCL_PARQUET_MAX_FILE_SIZE) {
    scl_parse_parquet_close(parser);
...
scl_error_t scl_parse_parquet_get_column_name(scl_parse_parquet_t *parser,
  ...
  if (scl_unlikely(!out))
    return SCL_ERR_NULL_PTR;
  if (index < 0 || index >= parser->num_columns)
    return SCL_ERR_INVALID_INDEX;
  if (!parser->column_names[index])
    return SCL_ERR_NOT_FOUND;
  *out = parser->column_names[index];
  if (out_len)
    *out_len = scl_strlen(parser->column_names[index]);
  return SCL_OK;
}
