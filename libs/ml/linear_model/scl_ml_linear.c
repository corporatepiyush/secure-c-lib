...
static scl_error_t scl_ml_linear_fit_sgd(scl_ml_linear_t *model,
  ...
  for (size_t epoch = 0; epoch < p->max_iter; epoch++) {
    /* Shuffle indices — we use simple sequential scan (faster, same
     * convergence) Random shuffling gives faster convergence but costs O(n)
     * shuffle per epoch */
    double epoch_loss = 0.0;

    for (size_t batch_start = 0; batch_start < n; batch_start += bs) {
      size_t bsize = bs;
      if (batch_start + bsize > n)
        bsize = n - batch_start;
      ...
      for (size_t i = 0; i < bsize; i++) {
        ...
        for (size_t j = 0; j < d; j++)
          pred += (double)ds->data[row * ds->row_stride + j] *
    ...
    if (epoch > 0) {
      ...
      if (fabsf(prev_loss - loss) < tol * (fabsf(prev_loss) + 1.0f)) {
        has_converged = 1;
        if (p->verbose)
          fprintf(stderr, "  SGD converged at epoch %zu, loss=%g\n", epoch,
                  loss);
        break;
      }
      prev_loss = loss;
...
static scl_error_t scl_ml_linear_fit_normal(scl_ml_linear_t *model,
  ...
  double *XtX = (double *)scl_calloc(model->scratch, d * d, sizeof(double),
                                     alignof(max_align_t));
  double *Xty = (double *)scl_calloc(model->scratch, d, sizeof(double),
                                     alignof(max_align_t));
  double *xc =
      (double *)scl_calloc(model->scratch, d, sizeof(double),
                           alignof(max_align_t)); /* centered row scratch */
  if (!XtX || !Xty || !xc) {
    return SCL_ERR_OUT_OF_MEMORY;
  }
  ...
  double *L = (double *)scl_calloc(model->scratch, d * d, sizeof(double),
                                   alignof(max_align_t));
  if (!L) {
    return SCL_ERR_OUT_OF_MEMORY;
  }
  ...
  double *z = (double *)scl_calloc(model->scratch, d, sizeof(double),
                                   alignof(max_align_t));
  if (!z) {
    return SCL_ERR_OUT_OF_MEMORY;
  }
...
static scl_error_t scl_ml_linear_fit_cd(scl_ml_linear_t *model,
  ...
  float *col_norms = (float *)scl_calloc(model->scratch, d, sizeof(float),
                                         alignof(max_align_t));
  if (!col_norms)
    return SCL_ERR_OUT_OF_MEMORY;
  for (size_t j = 0; j < d; j++)
    for (size_t i = 0; i < n; i++)
      col_norms[j] +=
          ds->data[i * ds->row_stride + j] * ds->data[i * ds->row_stride + j];
  for (size_t j = 0; j < d; j++)
    col_norms[j] += 2.0f * alpha_l2;
  /* For features with zero variance, leave them at 0 */
  for (size_t j = 0; j < d; j++)
    if (col_norms[j] < FLT_MIN)
      col_norms[j] = 1.0f;

  /* Initial predictions */
  float *pred = (float *)scl_calloc(model->scratch, n, sizeof(float),
                                    alignof(max_align_t));
  if (!pred) {
    return SCL_ERR_OUT_OF_MEMORY;
  }
  for (size_t i = 0; i < n; i++)
    pred[i] = model->intercept;
  ...
  for (size_t epoch = 0; epoch < model->params.max_iter; epoch++) {
    ...
    for (size_t j = 0; j < d; j++) {
      ...
      for (size_t i = 0; i < n; i++)
        rho += ds->data[i * ds->row_stride + j] *
               (ds->targets[i] - pred[i] +
                model->weights[j] * ds->data[i * ds->row_stride + j]);
    ...
    if (epoch > 0) {
      double rel_change =
          fabs(epoch_loss - prev_loss) / (fabs(prev_loss) + 1.0);
  ...
  float *resid = (float *)scl_calloc(model->scratch, n, sizeof(float),
                                     alignof(max_align_t));
  if (!resid)
    return SCL_ERR_OUT_OF_MEMORY;
  /* Compute intercept */
...
SCL_WARN_UNUSED scl_error_t scl_ml_linear_fit(scl_ml_linear_t *model,
  ...
  model->weights = (SCL_ML_FLOAT *)scl_calloc(la, d, sizeof(SCL_ML_FLOAT),
                                              alignof(max_align_t));
  if (scl_unlikely(!model->weights))
    return SCL_ERR_OUT_OF_MEMORY;

  /* Allocate working buffers */
  size_t bs =
      model->params.batch_size > 0 ? model->params.batch_size : ds->n_rows;
  scl_free(la, model->gradient);
  scl_free(la, model->pred_buffer);
  scl_free(la, model->resid_buffer);
  model->gradient = (SCL_ML_FLOAT *)scl_calloc(la, d, sizeof(SCL_ML_FLOAT),
                                               alignof(max_align_t));
  model->pred_buffer = (SCL_ML_FLOAT *)scl_calloc(la, bs, sizeof(SCL_ML_FLOAT),
                                                  alignof(max_align_t));
  model->resid_buffer = (SCL_ML_FLOAT *)scl_calloc(la, bs, sizeof(SCL_ML_FLOAT),
                                                   alignof(max_align_t));
...
SCL_WARN_UNUSED scl_error_t scl_ml_linear_predict(scl_ml_linear_t *model,
  ...
  for (size_t i = 0; i < n; i++) {
    ...
    double pred = (double)scl_ml_simd.dot_f(&ds->data[i * ds->row_stride],
                                            model->weights, d);
...
SCL_WARN_UNUSED scl_error_t scl_ml_linear_save(const scl_ml_linear_t *model,
  ...
  off += sizeof(size_t);
  memcpy(buffer + off, &model->intercept, sizeof(SCL_ML_FLOAT));
  off += sizeof(SCL_ML_FLOAT);
  memcpy(buffer + off, model->weights, weights_bytes);
  off += weights_bytes;
...
SCL_WARN_UNUSED scl_error_t scl_ml_linear_load(scl_ml_linear_t **model,
  ...
  m->weights = (SCL_ML_FLOAT *)scl_calloc(
      m->alloc, n_features, sizeof(SCL_ML_FLOAT), alignof(max_align_t));
  if (!m->weights) {
    scl_ml_linear_free(m);
    return SCL_ERR_OUT_OF_MEMORY;
  }

  memcpy(&m->intercept, buf + off, sizeof(SCL_ML_FLOAT));
  off += sizeof(SCL_ML_FLOAT);
