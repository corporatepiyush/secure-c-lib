...
static scl_error_t scl_ml_logistic_fit_sgd(scl_ml_logistic_t *model,
  ...
  for (size_t epoch = 0; epoch < p->max_iter; epoch++) {
    ...
    for (size_t batch_start = 0; batch_start < n; batch_start += bs) {
      ...
      scl_ml_simd.gemv(model->pred_buffer,
                       ds->data + batch_start * ds->row_stride, model->weights,
                       bsize, d, 0.0f);

      for (size_t i = 0; i < bsize; i++)
        model->pred_buffer[i] =
            scl_ml_clamp_logit(model->pred_buffer[i] + model->intercept);
      ...
      for (size_t j = 0; j < d; j++) {
        float g =
            model->gradient[j] * scale + 2.0f * alpha_l2 * model->weights[j];
        model->gradient[j] = g;
...
SCL_WARN_UNUSED scl_error_t scl_ml_logistic_new(
    scl_ml_logistic_t **model, scl_ml_logistic_params_t params) {
  if (scl_unlikely(!model))
    return SCL_ERR_NULL_PTR;
  if (!params.alloc)
    return SCL_ERR_INVALID_ARG;
  scl_allocator_t *alloc = params.alloc;
  scl_ml_logistic_t *m = (scl_ml_logistic_t *)scl_calloc(
      alloc, 1, sizeof(scl_ml_logistic_t), alignof(max_align_t));
  if (scl_unlikely(!m))
...
void scl_ml_logistic_free(scl_ml_logistic_t *model) {
  if (scl_unlikely(!model))
    return;
...
SCL_WARN_UNUSED scl_error_t scl_ml_logistic_fit(scl_ml_logistic_t *model,
                                                const scl_ml_dataset_t *ds) {
  ...
  scl_free(a, model->weights);
  model->weights = (SCL_ML_FLOAT *)scl_calloc(a, d, sizeof(SCL_ML_FLOAT),
                                              alignof(max_align_t));
  ...
  scl_free(a, model->gradient);
  scl_free(a, model->pred_buffer);
  model->gradient = (SCL_ML_FLOAT *)scl_calloc(a, d, sizeof(SCL_ML_FLOAT),
                                               alignof(max_align_t));
  model->pred_buffer = (SCL_ML_FLOAT *)scl_calloc(a, bs, sizeof(SCL_ML_FLOAT),
                                                  alignof(max_align_t));
  if (!model->gradient || !model->pred_buffer)
    return SCL_ERR_OUT_OF_MEMORY;
...
SCL_WARN_UNUSED scl_error_t scl_ml_logistic_predict(scl_ml_logistic_t *model,
                                                    const scl_ml_dataset_t *ds,
                                                    SCL_ML_FLOAT *y_out) {
...
SCL_WARN_UNUSED scl_error_t scl_ml_logistic_predict_proba(
    scl_ml_logistic_t *model, const scl_ml_dataset_t *ds,
...
SCL_PURE size_t scl_ml_logistic_get_n_features(const scl_ml_logistic_t *model) {
  return model ? model->n_features : 0;
}

SCL_WARN_UNUSED scl_error_t scl_ml_logistic_save(const scl_ml_logistic_t *model,
...
scl_ml_logistic_load(scl_ml_logistic_t **model, const uint8_t *buf, size_t len,
                     scl_ml_logistic_params_t params) {
  ...
  if (n_features == 0 || n_features > len)
    return SCL_ERR_INVALID_ARG;
  ...
  m->weights = (SCL_ML_FLOAT *)scl_calloc(a, n_features, sizeof(SCL_ML_FLOAT),
                                          alignof(max_align_t));
  if (!m->weights) {
    scl_ml_logistic_free(m);
    return SCL_ERR_OUT_OF_MEMORY;
