...
static size_t scl_ml_dbscan_region_query(const SCL_ML_FLOAT *data, size_t n,
                                         size_t d, size_t row_stride, size_t p,
                                         float eps_sq, size_t *neighbors) {
  size_t count = 0;
  for (size_t j = 0; j < n; j++) {
    if (j == p)
      continue;
    float dist =
        scl_ml_simd.dist_l2_sq((const float *)(data + p * row_stride),
                               (const float *)(data + j * row_stride), d);
    if (dist <= eps_sq)
      neighbors[count++] = j;
  }
  return count;
...
SCL_WARN_UNUSED scl_error_t scl_ml_dbscan_fit(scl_ml_dbscan_t *model,
  ...
  size_t *neighbors = (size_t *)scl_alloc(model->scratch, n * sizeof(size_t),
                                          alignof(max_align_t));
  size_t *queue = (size_t *)scl_alloc(model->scratch, n * sizeof(size_t),
                                      alignof(max_align_t));
  if (!neighbors || !queue) {
    return SCL_ERR_OUT_OF_MEMORY;
  }
  ...
  for (size_t i = 0; i < n; i++) {
    ...
    while (qhead < qtail) {
      ...
      size_t nn = scl_ml_dbscan_region_query(ds->data, n, d, ds->row_stride, q,
                                             eps_sq, neighbors);
...
SCL_WARN_UNUSED scl_error_t scl_ml_dbscan_predict(scl_ml_dbscan_t *model,
  ...
  if (scl_unlikely(ds->n_cols != model->n_features))
    return SCL_ERR_INVALID_ARG;
  if (scl_unlikely(ds->n_rows != model->n_samples))
    return SCL_ERR_INVALID_ARG;
