 ...
 * per-instance. */
static inline uint32_t scl_ml_gmm_next(uint32_t *st) {
  uint32_t x = *st;
  x ^= x << 13;
  x ^= x >> 17;
  x ^= x << 5;
...
static inline double scl_ml_gmm_uniform(uint32_t *st) {
  return (double)(scl_ml_gmm_next(st) >> 8) / (double)(1u << 24);
}

static scl_error_t scl_ml_gmm_init_kmeans(scl_ml_gmm_t *model,
  ...
  SCL_ML_FLOAT *min_dist = (SCL_ML_FLOAT *)scl_alloc(
      model->scratch, n * sizeof(SCL_ML_FLOAT), SCL_ML_ALIGNMENT);
  if (!min_dist)
  ...
  SCL_ML_FLOAT *global_var = (SCL_ML_FLOAT *)scl_calloc(
  ...
  for (size_t k = 0; k < K; k++) {
    ...
    for (size_t j = 0; j < d; j++) {
      ...
      model->precisions[k * d + j] = 1.0f / model->covariances[k * d + j];
...
SCL_WARN_UNUSED scl_error_t scl_ml_gmm_new(scl_ml_gmm_t **model,
                                           scl_ml_gmm_params_t params) {
  ...
  scl_ml_gmm_t *m = (scl_ml_gmm_t *)scl_calloc(alloc, 1, sizeof(scl_ml_gmm_t),
                                               SCL_ML_ALIGNMENT);
  if (scl_unlikely(!m))
    return SCL_ERR_OUT_OF_MEMORY;
...
void scl_ml_gmm_free(scl_ml_gmm_t *model) {
  if (scl_unlikely(!model))
    return;
...
SCL_WARN_UNUSED scl_error_t scl_ml_gmm_fit(scl_ml_gmm_t *model,
                                           const scl_ml_dataset_t *ds) {
  ...
  model->weights =
      (SCL_ML_FLOAT *)scl_calloc(a, K, sizeof(SCL_ML_FLOAT), SCL_ML_ALIGNMENT);
  model->means = (SCL_ML_FLOAT *)scl_calloc(a, K * d, sizeof(SCL_ML_FLOAT),
                                            SCL_ML_ALIGNMENT);
  model->covariances = (SCL_ML_FLOAT *)scl_calloc(
      a, K * d, sizeof(SCL_ML_FLOAT), SCL_ML_ALIGNMENT);
  model->precisions = (SCL_ML_FLOAT *)scl_calloc(a, K * d, sizeof(SCL_ML_FLOAT),
                                                 SCL_ML_ALIGNMENT);
  model->log_covar_det =
      (SCL_ML_FLOAT *)scl_calloc(a, K, sizeof(SCL_ML_FLOAT), SCL_ML_ALIGNMENT);
  model->responsibilities = (SCL_ML_FLOAT *)scl_calloc(
      a, resp_sz, sizeof(SCL_ML_FLOAT), SCL_ML_ALIGNMENT);
  ...
  SCL_ML_FLOAT *sum_x = (SCL_ML_FLOAT *)scl_calloc(
      model->scratch, K * d, sizeof(SCL_ML_FLOAT), SCL_ML_ALIGNMENT);
  double *N_k =
      (double *)scl_calloc(model->scratch, K, sizeof(double), SCL_ML_ALIGNMENT);
  if (!sum_x || !N_k) {
    return SCL_ERR_OUT_OF_MEMORY;
  }
  ...
  for (size_t iter = 0; iter < max_iter; iter++) {
    ...
    for (size_t i = 0; i < n; i++) {
      ...
      for (size_t k = 0; k < K; k++) {
        ...
        if (log_resp_val > max_log)
          max_log = log_resp_val;
      }
...
SCL_WARN_UNUSED scl_error_t scl_ml_gmm_predict(scl_ml_gmm_t *model,
                                               const scl_ml_dataset_t *ds,
                                               size_t *labels_out) {
  if (scl_unlikely(!model || !ds || !labels_out))
    return SCL_ERR_NULL_PTR;
  ...
  for (size_t i = 0; i < n; i++) {
    ...
    for (size_t k = 0; k < K; k++) {
      ...
      for (size_t j = 0; j < d; j++) {
        double diff =
            (double)ds->data[i * stride + j] - (double)model->means[k * d + j];
        quad += diff * diff * (double)model->precisions[k * d + j];
      }
...
SCL_WARN_UNUSED scl_error_t scl_ml_gmm_predict_proba(scl_ml_gmm_t *model,
                                                     const scl_ml_dataset_t *ds,
...
SCL_WARN_UNUSED scl_error_t scl_ml_gmm_save(const scl_ml_gmm_t *model,
  ...
  size_t payload_sz =
      sizeof(size_t) + sizeof(size_t) + nc * sizeof(SCL_ML_FLOAT) +
      nc * d * sizeof(SCL_ML_FLOAT) + nc * d * sizeof(SCL_ML_FLOAT);
...
SCL_WARN_UNUSED scl_error_t scl_ml_gmm_load(scl_ml_gmm_t **model,
  ...
  m->means = (SCL_ML_FLOAT *)scl_calloc(a, nc * d, sizeof(SCL_ML_FLOAT),
                                        SCL_ML_ALIGNMENT);
  m->covariances = (SCL_ML_FLOAT *)scl_calloc(a, nc * d, sizeof(SCL_ML_FLOAT),
                                              SCL_ML_ALIGNMENT);
  m->precisions = (SCL_ML_FLOAT *)scl_calloc(a, nc * d, sizeof(SCL_ML_FLOAT),
                                             SCL_ML_ALIGNMENT);
  m->log_covar_det =
      (SCL_ML_FLOAT *)scl_calloc(a, nc, sizeof(SCL_ML_FLOAT), SCL_ML_ALIGNMENT);
  if (!m->weights || !m->means || !m->covariances || !m->precisions ||
      !m->log_covar_det) {
    scl_ml_gmm_free(m);
    return SCL_ERR_OUT_OF_MEMORY;
  }
