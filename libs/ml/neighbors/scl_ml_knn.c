...
typedef struct scl_ml_knn {
  ...
  /* Pre-allocated buffers for predict (zero-alloc hot path) */
  float *dist_row;
  uint32_t *indices;
  float *dists_k;
...
static SCL_COLD_PATH scl_error_t scl_ml_knn_topk_min(const float *vals,
                                                     uint32_t *indices_out,
                                                     float *dists_out, size_t n,
                                                     size_t k) {
  if (k == 0 || n == 0)
    return SCL_OK;
  if (k > n)
...
SCL_WARN_UNUSED scl_error_t scl_ml_knn_new(scl_ml_knn_t **model,
  ...
  scl_ml_knn_t *m = (scl_ml_knn_t *)scl_calloc(alloc, 1, sizeof(scl_ml_knn_t),
                                               SCL_ML_ALIGNMENT);
  if (scl_unlikely(!m))
...
void scl_ml_knn_free(scl_ml_knn_t *model) {
  ...
  scl_free(a, model->dist_row);
  scl_free(a, model->indices);
  scl_free(a, model->dists_k);
  if (model->scratch)
...
SCL_WARN_UNUSED scl_error_t scl_ml_knn_fit(scl_ml_knn_t *model,
  ...
  model->train_data = (SCL_ML_FLOAT *)scl_calloc(a, n * d, sizeof(SCL_ML_FLOAT),
                                                 SCL_ML_ALIGNMENT);
  if (scl_unlikely(!model->train_data))
    return SCL_ERR_OUT_OF_MEMORY;

  model->train_targets =
      (SCL_ML_FLOAT *)scl_calloc(a, n, sizeof(SCL_ML_FLOAT), SCL_ML_ALIGNMENT);
  ...
  /* Pre-allocate predict-time buffers (zero-alloc hot path) */
  model->dist_row = (float *)scl_alloc(a, n * sizeof(float), SCL_ML_ALIGNMENT);
  model->indices =
      (uint32_t *)scl_alloc(a, k * sizeof(uint32_t), SCL_ML_ALIGNMENT);
  model->dists_k = (float *)scl_alloc(a, k * sizeof(float), SCL_ML_ALIGNMENT);
  if (scl_unlikely(!model->dist_row || !model->indices || !model->dists_k)) {
    scl_free(a, model->train_data);
    scl_free(a, model->train_targets);
    scl_free(a, model->dist_row);
    scl_free(a, model->indices);
...
SCL_WARN_UNUSED scl_error_t scl_ml_knn_predict(scl_ml_knn_t *model,
  ...
  if (k > nt)
    k = nt;
  if (k == 0) {
    memset(y_out, 0, nq * sizeof(SCL_ML_FLOAT));
    return SCL_OK;
  }

  /* Use pre-allocated buffers (zero-alloc hot path) */
  float *dist_row = model->dist_row;
  uint32_t *indices = model->indices;
  float *dists_k = model->dists_k;
  float seen_labels[32];
  float seen_weights[32];
  int seen_counts[32];
  if (scl_unlikely(!dist_row || !indices || !dists_k)) {
    return SCL_ERR_INVALID_STATE;
  }
  ...
  for (size_t i = 0; i < nq; i++) {
    ...
    for (size_t j = 0; j < nt; j++) {
      dist_row[j] =
          scl_ml_simd.dist_l2_sq((const float *)(ds->data + q_base),
                                 (const float *)(model->train_data + j * d), d);
    }
...
SCL_WARN_UNUSED scl_error_t scl_ml_knn_save(const scl_ml_knn_t *model,
  ...
  size_t data_bytes =
      model->n_samples * model->n_features * sizeof(SCL_ML_FLOAT);
  ...
  off += sizeof(size_t);
  memcpy(buffer + off, model->train_data, data_bytes);
  off += data_bytes;
  memcpy(buffer + off, model->train_targets, tgt_bytes);
  off += tgt_bytes;
...
SCL_WARN_UNUSED scl_error_t scl_ml_knn_load(scl_ml_knn_t **model,
  ...
  /* n_samples/n_features are untrusted (the CRC only detects corruption,
   * not a crafted file): the byte counts must be computed overflow-safe
   * and verified to lie inside the serialized buffer before any copy. */
  size_t data_bytes, tgt_bytes, need;
  if (scl_mul_overflow(n_samples, n_features, &data_bytes) ||
      scl_mul_overflow(data_bytes, sizeof(SCL_ML_FLOAT), &data_bytes) ||
      scl_mul_overflow(n_samples, sizeof(SCL_ML_FLOAT), &tgt_bytes) ||
      scl_add_overflow(data_bytes, tgt_bytes, &need) ||
  ...
  m->train_data =
      (SCL_ML_FLOAT *)scl_calloc(a, 1, data_bytes, SCL_ML_ALIGNMENT);
  m->train_targets =
      (SCL_ML_FLOAT *)scl_calloc(a, 1, tgt_bytes, SCL_ML_ALIGNMENT);

  /* Predict-time scratch buffers: fit() allocates these, and predict()
   * refuses to run without them — a loaded model must match. */
  size_t k = m->params.k;
  m->dist_row =
      (float *)scl_alloc(a, n_samples * sizeof(float), SCL_ML_ALIGNMENT);
  m->indices = (uint32_t *)scl_alloc(a, k * sizeof(uint32_t), SCL_ML_ALIGNMENT);
  m->dists_k = (float *)scl_alloc(a, k * sizeof(float), SCL_ML_ALIGNMENT);
  if (!m->train_data || !m->train_targets || !m->dist_row || !m->indices ||
      !m->dists_k) {
    scl_ml_knn_free(m);
    return SCL_ERR_OUT_OF_MEMORY;
  }

  memcpy(m->train_data, buf + off, data_bytes);
  off += data_bytes;
