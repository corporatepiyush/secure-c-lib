...
static scl_error_t scl_ml_pca_jacobi(SCL_ML_FLOAT *A, SCL_ML_FLOAT *V, size_t d,
  ...
  for (size_t sweep = 0; sweep < max_sweeps; sweep++) {
    ...
    float theta =
        0.5f * atan2f(2.0f * A[p * d + q], A[q * d + q] - A[p * d + p]);
    float c = cosf(theta);
    float s = sinf(theta);
...
SCL_WARN_UNUSED scl_error_t scl_ml_pca_new(scl_ml_pca_t **model,
                                           scl_ml_pca_params_t params) {
  ...
  scl_ml_pca_t *m = (scl_ml_pca_t *)scl_calloc(alloc, 1, sizeof(scl_ml_pca_t),
                                               SCL_ML_ALIGNMENT);
  if (scl_unlikely(!m))
...
void scl_ml_pca_free(scl_ml_pca_t *model) {
  if (scl_unlikely(!model))
    return;
...
SCL_WARN_UNUSED scl_error_t scl_ml_pca_fit(scl_ml_pca_t *model,
                                           const scl_ml_dataset_t *ds) {
  if (scl_unlikely(!model || !ds))
    return SCL_ERR_NULL_PTR;
  if (scl_unlikely(!ds->data || ds->n_rows < 2))
    return SCL_ERR_INVALID_ARG;
  ...
  model->mean =
      (SCL_ML_FLOAT *)scl_calloc(a, d, sizeof(SCL_ML_FLOAT), SCL_ML_ALIGNMENT);
  model->components = (SCL_ML_FLOAT *)scl_calloc(
      a, n_comp * d, sizeof(SCL_ML_FLOAT), SCL_ML_ALIGNMENT);
  model->explained_variance = (SCL_ML_FLOAT *)scl_calloc(
      a, n_comp, sizeof(SCL_ML_FLOAT), SCL_ML_ALIGNMENT);
  model->explained_variance_ratio = (SCL_ML_FLOAT *)scl_calloc(
      a, n_comp, sizeof(SCL_ML_FLOAT), SCL_ML_ALIGNMENT);
  if (!model->mean || !model->components || !model->explained_variance ||
      !model->explained_variance_ratio) {
    return SCL_ERR_OUT_OF_MEMORY;
  }

  SCL_ML_FLOAT *cov = (SCL_ML_FLOAT *)scl_calloc(
      model->scratch, d * d, sizeof(SCL_ML_FLOAT), SCL_ML_ALIGNMENT);
  SCL_ML_FLOAT *V = (SCL_ML_FLOAT *)scl_calloc(
      model->scratch, d * d, sizeof(SCL_ML_FLOAT), SCL_ML_ALIGNMENT);
  SCL_ML_FLOAT *xc = (SCL_ML_FLOAT *)scl_calloc(
      model->scratch, d, sizeof(SCL_ML_FLOAT), SCL_ML_ALIGNMENT);
  if (!cov || !V || !xc) {
    return SCL_ERR_OUT_OF_MEMORY;
  }
  ...
  scl_error_t err =
      scl_ml_pca_jacobi(cov, V, d, model->params.tol, model->params.max_sweeps);
  if (scl_unlikely(err != SCL_OK)) {
    return err;
  }

  scl_pca_eigen_t *eigens = (scl_pca_eigen_t *)scl_alloc(
      model->scratch, d * sizeof(scl_pca_eigen_t), SCL_ML_ALIGNMENT);
  if (!eigens) {
    return SCL_ERR_OUT_OF_MEMORY;
  }
...
SCL_WARN_UNUSED scl_error_t scl_ml_pca_transform(scl_ml_pca_t *model,
                                                 const scl_ml_dataset_t *ds,
                                                 SCL_ML_FLOAT *out) {
  if (scl_unlikely(!model || !ds || !out))
    return SCL_ERR_NULL_PTR;
  ...
  SCL_ML_FLOAT *mean_dot = (SCL_ML_FLOAT *)scl_calloc(
      model->scratch, n_comp, sizeof(SCL_ML_FLOAT), SCL_ML_ALIGNMENT);
  if (!mean_dot)
    return SCL_ERR_OUT_OF_MEMORY;
  for (size_t k = 0; k < n_comp; k++)
    mean_dot[k] = scl_ml_simd.dot_f(model->mean, &model->components[k * d], d);

  for (size_t i = 0; i < n; i++) {
    const SCL_ML_FLOAT *row = &ds->data[i * stride];
    for (size_t k = 0; k < n_comp; k++)
      out[i * n_comp + k] =
          (SCL_ML_FLOAT)((double)scl_ml_simd.dot_f(
                             row, &model->components[k * d], d) -
                         (double)mean_dot[k]);
  }

  if (model->params.whiten) {
    for (size_t k = 0; k < n_comp; k++) {
      SCL_ML_FLOAT inv_std =
          1.0f / sqrtf(model->explained_variance[k] + SCL_ML_EPSILON);
      for (size_t i = 0; i < n; i++)
...
SCL_WARN_UNUSED scl_error_t scl_ml_pca_fit_transform(scl_ml_pca_t *model,
                                                     const scl_ml_dataset_t *ds,
                                                     SCL_ML_FLOAT *out) {
...
scl_ml_pca_inverse_transform(scl_ml_pca_t *model, const SCL_ML_FLOAT *X_proj,
                             size_t n_samples, SCL_ML_FLOAT *out) {
  if (scl_unlikely(!model || !X_proj || !out))
    return SCL_ERR_NULL_PTR;
  if (scl_unlikely(!model->fitted))
    return SCL_ERR_INVALID_STATE;
  ...
  scl_ml_simd.gemm(out, X_proj, model->components, n_samples, d, n_comp, 1.0f,
                   0.0f);
...
SCL_PURE size_t scl_ml_pca_get_n_components(const scl_ml_pca_t *model) {
...
SCL_WARN_UNUSED scl_error_t scl_ml_pca_save(const scl_ml_pca_t *model,
  ...
  size_t payload_sz = sizeof(size_t) + sizeof(size_t) +
                      d * sizeof(SCL_ML_FLOAT) + nc * d * sizeof(SCL_ML_FLOAT) +
                      nc * sizeof(SCL_ML_FLOAT) + nc * sizeof(SCL_ML_FLOAT);
  ...
  memcpy(buffer + off, &d, sizeof(size_t));
  off += sizeof(size_t);
  memcpy(buffer + off, &nc, sizeof(size_t));
  off += sizeof(size_t);
  memcpy(buffer + off, model->mean, d * sizeof(SCL_ML_FLOAT));
  ...
  memcpy(buffer + off, model->explained_variance_ratio,
...
SCL_WARN_UNUSED scl_error_t scl_ml_pca_load(scl_ml_pca_t **model,
  ...
  size_t off = sizeof(*hdr);
  size_t d = 0, nc = 0;
  memcpy(&d, buf + off, sizeof(size_t));
  off += sizeof(size_t);
  memcpy(&nc, buf + off, sizeof(size_t));
  off += sizeof(size_t);
  ...
  m->mean =
      (SCL_ML_FLOAT *)scl_calloc(a, d, sizeof(SCL_ML_FLOAT), SCL_ML_ALIGNMENT);
  m->components = (SCL_ML_FLOAT *)scl_calloc(a, nc * d, sizeof(SCL_ML_FLOAT),
                                             SCL_ML_ALIGNMENT);
  m->explained_variance =
      (SCL_ML_FLOAT *)scl_calloc(a, nc, sizeof(SCL_ML_FLOAT), SCL_ML_ALIGNMENT);
  m->explained_variance_ratio =
      (SCL_ML_FLOAT *)scl_calloc(a, nc, sizeof(SCL_ML_FLOAT), SCL_ML_ALIGNMENT);
  if (!m->mean || !m->components || !m->explained_variance ||
      !m->explained_variance_ratio) {
    scl_ml_pca_free(m);
    return SCL_ERR_OUT_OF_MEMORY;
  }
