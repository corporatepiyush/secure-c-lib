...
/* ML module master header: Dataset type, common enums, config parameter structs, serialization helpers. */
...
typedef struct {
    SCL_ML_FLOAT *data;          /* Row-major: [n_rows * row_stride] floats. 32-byte aligned. */
    ...
    SCL_ML_FLOAT *data_col;      /* Column-major view: [n_cols * col_stride], built lazily */
...
SCL_WARN_UNUSED scl_error_t
scl_ml_dataset_wrap(scl_ml_dataset_t *ds, SCL_ML_FLOAT *data, SCL_ML_FLOAT *targets,
                     size_t n_rows, size_t n_cols);
 ...
 *   scl_error_t scl_ml_<algo>_save(const model_t *, uint8_t **buf, size_t *len, scl_allocator_t *);
 *   scl_error_t scl_ml_<algo>_load(model_t **, const uint8_t *buf, size_t len, params_t);
 *
 * Buffer format:
...
/* ── Inline helpers ──────────────────────────────────────────── */
static inline int
scl_ml_float_is_finite(SCL_ML_FLOAT x) {
    return isfinite(x);
}
...
static inline SCL_ML_FLOAT
scl_ml_clamp_logit(SCL_ML_FLOAT x) {
    if (scl_unlikely(x < -30.0f)) return -30.0f;
    if (scl_unlikely(x >  30.0f)) return  30.0f;
    return x;
}
