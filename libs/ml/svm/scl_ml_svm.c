...
static SCL_ML_FLOAT scl_ml_svm_kernel_linear(const SCL_ML_FLOAT *x,
                                             const SCL_ML_FLOAT *y, size_t d) {
  /* SIMD dot with f64 accumulator (numerical stability for large d) */
  return scl_ml_simd.dot_f(x, y, d);
}

static SCL_ML_FLOAT scl_ml_svm_kernel_rbf(const SCL_ML_FLOAT *x,
                                          const SCL_ML_FLOAT *y, size_t d,
                                          SCL_ML_FLOAT gamma) {
...
static SCL_ML_FLOAT scl_ml_svm_kernel_poly(const SCL_ML_FLOAT *x,
                                           const SCL_ML_FLOAT *y, size_t d,
                                           SCL_ML_FLOAT gamma,
                                           SCL_ML_FLOAT coef0, int degree) {
...
static SCL_ML_FLOAT scl_ml_svm_kernel(const scl_ml_svm_t *model,
                                      const SCL_ML_FLOAT *x,
                                      const SCL_ML_FLOAT *y) {
  ...
  case SCL_ML_KERNEL_POLY: {
    ...
    return scl_ml_svm_kernel_poly(x, y, model->n_features, gamma, coef0,
                                  model->params.degree);
  }
  default:
...
static int scl_ml_svm_take_step(scl_ml_svm_t *model, const scl_ml_dataset_t *ds,
  ...
  if (eta <= 0.0f) {
    ...
    SCL_ML_FLOAT rest_i =
        Ei + yi - alpha_i_old * yi * kii - alpha_j_old * yj * kij;
    SCL_ML_FLOAT rest_j =
        Ej + yj - alpha_i_old * yi * kij - alpha_j_old * yj * kjj;

    SCL_ML_FLOAT delta_aj_L = L - alpha_j_old;
    SCL_ML_FLOAT ai_L = alpha_i_old - s * delta_aj_L;
    SCL_ML_FLOAT obj_L = ai_L + L - 0.5f * kii * ai_L * ai_L -
                         0.5f * kjj * L * L - s * kij * ai_L * L -
                         ai_L * yi * rest_i - L * yj * rest_j;

    SCL_ML_FLOAT delta_aj_H = H - alpha_j_old;
    SCL_ML_FLOAT ai_H = alpha_i_old - s * delta_aj_H;
    SCL_ML_FLOAT obj_H = ai_H + H - 0.5f * kii * ai_H * ai_H -
                         0.5f * kjj * H * H - s * kij * ai_H * H -
                         ai_H * yi * rest_i - H * yj * rest_j;
...
static int scl_ml_svm_examine_example(scl_ml_svm_t *model,
  ...
  if ((r1 < -tol && alphas[i] < C) || (r1 > tol && alphas[i] > 0.0f)) {
    ...
    if (max_j >= 0) {
      if (scl_ml_svm_take_step(model, ds, alphas, b, E, tol, C, i, max_j))
        return 1;
    }
    ...
    for (size_t j = 0; j < n; j++) {
      if (j == (size_t)i)
        continue;
      if (scl_ml_svm_take_step(model, ds, alphas, b, E, tol, C, i, (int)j))
        return 1;
    }
  }
...
SCL_WARN_UNUSED scl_error_t scl_ml_svm_new(scl_ml_svm_t **model,
                                           scl_ml_svm_params_t params) {
  ...
  scl_ml_svm_t *m = (scl_ml_svm_t *)scl_calloc(alloc, 1, sizeof(scl_ml_svm_t),
                                               alignof(max_align_t));
  if (scl_unlikely(!m))
...
void scl_ml_svm_free(scl_ml_svm_t *model) {
  if (scl_unlikely(!model))
    return;
...
SCL_WARN_UNUSED scl_error_t scl_ml_svm_fit(scl_ml_svm_t *model,
                                           const scl_ml_dataset_t *ds) {
  ...
  scl_allocator_t *a = model->alloc;
  SCL_ML_FLOAT *alphas = (SCL_ML_FLOAT *)scl_calloc(
      model->scratch, n, sizeof(SCL_ML_FLOAT), alignof(max_align_t));
  SCL_ML_FLOAT *E = (SCL_ML_FLOAT *)scl_calloc(
      model->scratch, n, sizeof(SCL_ML_FLOAT), alignof(max_align_t));
  if (!alphas || !E) {
    return SCL_ERR_OUT_OF_MEMORY;
  }
  ...
  while (iteration < max_iter) {
    ...
    if (examine_all) {
      for (size_t i = 0; i < n; i++) {
        if (scl_ml_svm_examine_example(model, ds, alphas, &b, E, tol, C,
                                       (int)i))
          num_changed++;
      }
    } else {
  ...
  if (sv_count > 0) {
    ...
    model->alpha = (SCL_ML_FLOAT *)scl_calloc(a, sv_count, sizeof(SCL_ML_FLOAT),
                                              alignof(max_align_t));
    ...
    for (size_t i = 0; i < n; i++) {
      if (alphas[i] > 1e-6f) {
        memcpy(&model->support_vectors[idx * d], &ds->data[i * ds->row_stride],
...
SCL_WARN_UNUSED scl_error_t scl_ml_svm_predict(scl_ml_svm_t *model,
                                               const scl_ml_dataset_t *ds,
                                               SCL_ML_FLOAT *y_out) {
  ...
  const size_t n_rows = ds->n_rows;
  const size_t row_stride = ds->row_stride;
  const size_t n_sv = model->n_sv;
  const size_t d = model->n_features;

  /* Use double-precision accumulators to match the original
   * scl_ml_svm_decision_function's numerics while keeping the
   * cache-friendly SVs-outer, rows-inner loop order. */
  double *acc = (double *)scl_calloc(model->scratch, n_rows, sizeof(double),
                                     alignof(max_align_t));
  if (!acc)
    return SCL_ERR_OUT_OF_MEMORY;

  for (size_t i = 0; i < n_rows; i++)
    acc[i] = (double)model->b;

  for (size_t j = 0; j < n_sv; j++) {
    const double coeff = (double)model->alpha[j] * (double)model->sv_labels[j];
    const SCL_ML_FLOAT *sv = &model->support_vectors[j * d];

    switch (model->params.kernel) {
    case SCL_ML_KERNEL_LINEAR:
      for (size_t i = 0; i < n_rows; i++)
        acc[i] += coeff * (double)scl_ml_svm_kernel_linear(
                              sv, ds->data + i * row_stride, d);
      break;
    case SCL_ML_KERNEL_RBF: {
      const double gamma = (double)model->params.gamma;
      for (size_t i = 0; i < n_rows; i++)
        acc[i] +=
            coeff * (double)scl_ml_svm_kernel_rbf(sv, ds->data + i * row_stride,
                                                  d, (SCL_ML_FLOAT)gamma);
      break;
    }
    case SCL_ML_KERNEL_POLY: {
      const double gamma = (double)model->params.gamma;
      const double coef0 = (double)model->params.coef0;
      const int degree = model->params.degree;
      for (size_t i = 0; i < n_rows; i++)
        acc[i] += coeff * (double)scl_ml_svm_kernel_poly(
                              sv, ds->data + i * row_stride, d,
                              (SCL_ML_FLOAT)gamma, (SCL_ML_FLOAT)coef0, degree);
      break;
    }
    default: {
      const double gamma = (double)model->params.gamma;
  ...
  /* Convert raw decision values to binary labels. */
  for (size_t i = 0; i < n_rows; i++)
    y_out[i] = (SCL_ML_FLOAT)(acc[i] >= 0.0 ? 1.0 : -1.0);

  return SCL_OK;
}

SCL_PURE size_t scl_ml_svm_get_n_features(const scl_ml_svm_t *model) {
  return model ? model->n_features : 0;
}

SCL_PURE size_t scl_ml_svm_get_n_support(const scl_ml_svm_t *model) {
  return model ? model->n_sv : 0;
}

SCL_WARN_UNUSED scl_error_t scl_ml_svm_save(const scl_ml_svm_t *model,
  ...
  /* The kernel and its hyperparameters are part of the decision
   * function, so they must travel with the model — otherwise a loaded
   * model silently predicts with whatever kernel the caller's params
   * happen to contain. gamma is the *effective* value (fit() replaces
   * gamma <= 0 with 1/d). */
  int32_t kern = (int32_t)model->params.kernel;
  int32_t degree = (int32_t)model->params.degree;
  double gamma = model->params.gamma;
  double coef0 = model->params.coef0;

  size_t payload_sz = sizeof(size_t) * 2 + sizeof(SCL_ML_FLOAT) +
                      sizeof(int32_t) * 2 + sizeof(double) * 2 + sv_bytes +
                      alpha_bytes + labels_bytes;
  size_t total = sizeof(hdr) + payload_sz + sizeof(uint32_t);

  uint8_t *buffer =
      (uint8_t *)scl_calloc(alloc, 1, total, alignof(max_align_t));
  if (!buffer)
    return SCL_ERR_OUT_OF_MEMORY;
  ...
  memcpy(buffer + off, &model->n_sv, sizeof(size_t));
  off += sizeof(size_t);
  memcpy(buffer + off, &model->n_features, sizeof(size_t));
  off += sizeof(size_t);
  memcpy(buffer + off, &model->b, sizeof(SCL_ML_FLOAT));
  off += sizeof(SCL_ML_FLOAT);
  memcpy(buffer + off, &kern, sizeof(int32_t));
  off += sizeof(int32_t);
  memcpy(buffer + off, &degree, sizeof(int32_t));
  off += sizeof(int32_t);
  memcpy(buffer + off, &gamma, sizeof(double));
  off += sizeof(double);
  memcpy(buffer + off, &coef0, sizeof(double));
  off += sizeof(double);
  memcpy(buffer + off, model->support_vectors, sv_bytes);
  off += sv_bytes;
  memcpy(buffer + off, model->alpha, alpha_bytes);
  off += alpha_bytes;
  memcpy(buffer + off, model->sv_labels, labels_bytes);
  off += labels_bytes;
...
SCL_WARN_UNUSED scl_error_t scl_ml_svm_load(scl_ml_svm_t **model,
  ...
  memcpy(&m->n_sv, buf + off, sizeof(size_t));
  off += sizeof(size_t);
  memcpy(&m->n_features, buf + off, sizeof(size_t));
  off += sizeof(size_t);
  memcpy(&m->b, buf + off, sizeof(SCL_ML_FLOAT));
  off += sizeof(SCL_ML_FLOAT);

  if (len - off < sizeof(int32_t) * 2 + sizeof(double) * 2 + sizeof(uint32_t)) {
    scl_ml_svm_free(m);
    return SCL_ERR_INVALID_ARG;
  }
  /* Restore the kernel the model was trained with — the caller's params
   * only control allocator/solver knobs, not the decision function. */
  int32_t kern = 0, degree = 0;
  double gamma = 0.0, coef0 = 0.0;
  memcpy(&kern, buf + off, sizeof(int32_t));
  off += sizeof(int32_t);
  memcpy(&degree, buf + off, sizeof(int32_t));
  off += sizeof(int32_t);
  memcpy(&gamma, buf + off, sizeof(double));
  off += sizeof(double);
  memcpy(&coef0, buf + off, sizeof(double));
  off += sizeof(double);
  if (kern < 0 || kern > SCL_ML_KERNEL_SIGMOID) {
    scl_ml_svm_free(m);
    return SCL_ERR_INVALID_ARG;
  }
  m->params.kernel = (scl_ml_kernel_t)kern;
  m->params.degree = (int)degree;
  m->params.gamma = gamma;
  m->params.coef0 = coef0;

  /* n_sv/n_features are untrusted: compute byte counts overflow-safe and
   * verify they fit inside the buffer (excluding the trailing CRC). */
  size_t sv_bytes, alpha_bytes, labels_bytes, need;
  if (m->n_features == 0 ||
      scl_mul_overflow(m->n_sv, m->n_features, &sv_bytes) ||
      scl_mul_overflow(sv_bytes, sizeof(SCL_ML_FLOAT), &sv_bytes) ||
      scl_mul_overflow(m->n_sv, sizeof(SCL_ML_FLOAT), &alpha_bytes) ||
      scl_add_overflow(sv_bytes, alpha_bytes * 2, &need) ||
      !scl_range_in_bounds(len - sizeof(uint32_t), off, need)) {
    scl_ml_svm_free(m);
    return SCL_ERR_INVALID_ARG;
  }
  labels_bytes = alpha_bytes;

  m->support_vectors = (SCL_ML_FLOAT *)scl_calloc(
      a, m->n_sv * m->n_features, sizeof(SCL_ML_FLOAT), alignof(max_align_t));
  m->alpha = (SCL_ML_FLOAT *)scl_calloc(a, m->n_sv, sizeof(SCL_ML_FLOAT),
                                        alignof(max_align_t));
  m->sv_labels = (SCL_ML_FLOAT *)scl_calloc(a, m->n_sv, sizeof(SCL_ML_FLOAT),
                                            alignof(max_align_t));
  ...
  memcpy(m->support_vectors, buf + off, sv_bytes);
  off += sv_bytes;
  memcpy(m->alpha, buf + off, alpha_bytes);
  off += alpha_bytes;
  ...
  uint32_t expected_crc =
      scl_ml_crc32c(buf + sizeof(*hdr), len - sizeof(*hdr) - sizeof(uint32_t));
