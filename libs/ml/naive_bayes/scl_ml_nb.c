...
static int scl_ml_nb_find_class(const int *labels, size_t n_classes,
                                int label) {
...
static void scl_ml_nb_precompute_gaussian(scl_ml_nb_t *model) {
...
SCL_WARN_UNUSED scl_error_t scl_ml_nb_new(scl_ml_nb_t **model,
                                          scl_ml_nb_params_t params) {
  ...
  scl_ml_nb_t *m = (scl_ml_nb_t *)scl_calloc(alloc, 1, sizeof(scl_ml_nb_t),
                                             SCL_ML_ALIGNMENT);
  if (scl_unlikely(!m))
...
void scl_ml_nb_free(scl_ml_nb_t *model) {
  if (scl_unlikely(!model))
    return;
...
SCL_WARN_UNUSED scl_error_t scl_ml_nb_fit(scl_ml_nb_t *model,
  ...
  int n_classes = 0;
  int *labels =
      (int *)scl_calloc(model->scratch, n, sizeof(int), SCL_ML_ALIGNMENT);
  if (!labels)
    return SCL_ERR_OUT_OF_MEMORY;
  ...
  int *counts = (int *)scl_calloc(model->scratch, (size_t)n_classes,
                                  sizeof(int), SCL_ML_ALIGNMENT);
  if (!counts) {
    return SCL_ERR_OUT_OF_MEMORY;
  }
  ...
  model->means = (SCL_ML_FLOAT *)scl_calloc(a, cv_sz, sizeof(SCL_ML_FLOAT),
                                            SCL_ML_ALIGNMENT);
  model->vars = (SCL_ML_FLOAT *)scl_calloc(a, cv_sz, sizeof(SCL_ML_FLOAT),
                                           SCL_ML_ALIGNMENT);
  model->class_log_prior = (SCL_ML_FLOAT *)scl_calloc(
      a, (size_t)n_classes, sizeof(SCL_ML_FLOAT), SCL_ML_ALIGNMENT);
  model->class_labels =
      (int *)scl_calloc(a, (size_t)n_classes, sizeof(int), SCL_ML_ALIGNMENT);
  model->lp_buffer = (SCL_ML_FLOAT *)scl_calloc(
      a, (size_t)n_classes, sizeof(SCL_ML_FLOAT), SCL_ML_ALIGNMENT);
  model->log_term = (SCL_ML_FLOAT *)scl_calloc(a, cv_sz, sizeof(SCL_ML_FLOAT),
                                               SCL_ML_ALIGNMENT);
  model->inv_2var = (SCL_ML_FLOAT *)scl_calloc(a, cv_sz, sizeof(SCL_ML_FLOAT),
                                               SCL_ML_ALIGNMENT);
  if (!model->means || !model->vars || !model->class_log_prior ||
      !model->class_labels || !model->lp_buffer || !model->log_term ||
      !model->inv_2var) {
    return SCL_ERR_OUT_OF_MEMORY;
  }
  ...
  for (size_t i = 0; i < n; i++) {
    int c =
        scl_ml_nb_find_class(labels, (size_t)n_classes, (int)ds->targets[i]);
    for (size_t f = 0; f < nf; f++)
...
SCL_WARN_UNUSED scl_error_t scl_ml_nb_predict(scl_ml_nb_t *model,
                                              const scl_ml_dataset_t *ds,
                                              SCL_ML_FLOAT *y_out) {
...
SCL_WARN_UNUSED scl_error_t scl_ml_nb_predict_proba(scl_ml_nb_t *model,
                                                    const scl_ml_dataset_t *ds,
                                                    SCL_ML_FLOAT *proba_out) {
  if (scl_unlikely(!model || !ds || !proba_out))
    return SCL_ERR_NULL_PTR;
  if (scl_unlikely(!model->fitted))
    return SCL_ERR_INVALID_STATE;
  if (scl_unlikely(ds->n_cols != model->n_features))
...
SCL_PURE size_t scl_ml_nb_get_n_classes(const scl_ml_nb_t *model) {
  return model ? model->n_classes : 0;
}

SCL_WARN_UNUSED scl_error_t scl_ml_nb_save(const scl_ml_nb_t *model,
  ...
  size_t payload_sz = sizeof(size_t) * 2 + cv_sz * sizeof(SCL_ML_FLOAT) * 2 +
  ...
  off += sizeof(size_t);
  memcpy(buffer + off, &nf, sizeof(size_t));
  off += sizeof(size_t);
  memcpy(buffer + off, model->means, cv_sz * sizeof(SCL_ML_FLOAT));
  off += cv_sz * sizeof(SCL_ML_FLOAT);
  memcpy(buffer + off, model->vars, cv_sz * sizeof(SCL_ML_FLOAT));
  off += cv_sz * sizeof(SCL_ML_FLOAT);
...
SCL_WARN_UNUSED scl_error_t scl_ml_nb_load(scl_ml_nb_t **model,
  ...
  off += sizeof(size_t);
  memcpy(&nf, buf + off, sizeof(size_t));
  off += sizeof(size_t);
  ...
  m->means = (SCL_ML_FLOAT *)scl_calloc(a, cv_sz, sizeof(SCL_ML_FLOAT),
                                        SCL_ML_ALIGNMENT);
  m->vars = (SCL_ML_FLOAT *)scl_calloc(a, cv_sz, sizeof(SCL_ML_FLOAT),
                                       SCL_ML_ALIGNMENT);
  m->class_log_prior =
      (SCL_ML_FLOAT *)scl_calloc(a, nc, sizeof(SCL_ML_FLOAT), SCL_ML_ALIGNMENT);
  m->class_labels = (int *)scl_calloc(a, nc, sizeof(int), SCL_ML_ALIGNMENT);
  m->lp_buffer =
      (SCL_ML_FLOAT *)scl_calloc(a, nc, sizeof(SCL_ML_FLOAT), SCL_ML_ALIGNMENT);
  m->log_term = (SCL_ML_FLOAT *)scl_calloc(a, cv_sz, sizeof(SCL_ML_FLOAT),
                                           SCL_ML_ALIGNMENT);
  m->inv_2var = (SCL_ML_FLOAT *)scl_calloc(a, cv_sz, sizeof(SCL_ML_FLOAT),
                                           SCL_ML_ALIGNMENT);
  if (!m->means || !m->vars || !m->class_log_prior || !m->class_labels ||
      !m->lp_buffer || !m->log_term || !m->inv_2var) {
    scl_ml_nb_free(m);
    return SCL_ERR_OUT_OF_MEMORY;
  }
  ...
  memcpy(m->means, buf + off, cv_sz * sizeof(SCL_ML_FLOAT));
  off += cv_sz * sizeof(SCL_ML_FLOAT);
  memcpy(m->vars, buf + off, cv_sz * sizeof(SCL_ML_FLOAT));
  off += cv_sz * sizeof(SCL_ML_FLOAT);
