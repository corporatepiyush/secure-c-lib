...
SCL_WARN_UNUSED scl_error_t scl_ml_distance_matrix(SCL_ML_FLOAT *out,
                                                   const SCL_ML_FLOAT *a,
                                                   const SCL_ML_FLOAT *b,
                                                   size_t n, size_t m, size_t d,
                                                   scl_ml_distance_t metric) {
  if (scl_unlikely(!out || !a || !b))
...
float scl_ml_kernel(const SCL_ML_FLOAT *x, const SCL_ML_FLOAT *y, size_t d,
                    scl_ml_kernel_t type, float gamma, float coef0,
                    int degree) {
  scl_ml_simd_init();
...
SCL_WARN_UNUSED scl_error_t scl_ml_kernel_matrix(
    SCL_ML_FLOAT *k_out, const SCL_ML_FLOAT *a, const SCL_ML_FLOAT *b, size_t n,
    size_t m, size_t d, scl_ml_kernel_t type, float gamma, float coef0,
    int degree) {
  if (scl_unlikely(!k_out || !a || !b))
    return SCL_ERR_NULL_PTR;

  for (size_t i = 0; i < n; i++) {
    for (size_t j = 0; j < m; j++) {
      k_out[i * m + j] =
          scl_ml_kernel(&a[i * d], &b[j * d], d, type, gamma, coef0, degree);
    }
  }
  return SCL_OK;
}
...
float scl_ml_mean_squared_error(const SCL_ML_FLOAT *y_true,
                                const SCL_ML_FLOAT *y_pred, size_t n) {
  if (scl_unlikely(!y_true || !y_pred || n == 0))
    return -1.0f;
  scl_ml_simd_init();
...
float scl_ml_mean_absolute_error(const SCL_ML_FLOAT *y_true,
                                 const SCL_ML_FLOAT *y_pred, size_t n) {
  if (scl_unlikely(!y_true || !y_pred || n == 0))
    return -1.0f;
  double sum = 0.0;
...
float scl_ml_r2_score(const SCL_ML_FLOAT *y_true, const SCL_ML_FLOAT *y_pred,
                      size_t n) {
  if (scl_unlikely(!y_true || !y_pred || n == 0))
    return -1.0f;
...
float scl_ml_log_loss(const SCL_ML_FLOAT *y_true,
                      const SCL_ML_FLOAT *y_pred_proba, size_t n) {
  if (scl_unlikely(!y_true || !y_pred_proba || n == 0))
    return -1.0f;
  ...
  for (size_t i = 0; i < n; i++) {
    ...
    if (p > 1.0f)
      p = 1.0f;
    loss += (double)y_true[i] * logf(p) +
            (double)(1.0f - y_true[i]) * logf(1.0f - p);
  }
  return (float)(-loss / (double)n);
}

float scl_ml_accuracy(const float *y_true, const float *y_pred, size_t n) {
  if (scl_unlikely(!y_true || !y_pred || n == 0))
    return 0.0f;
  size_t correct = 0;
  for (size_t i = 0; i < n; i++)
