...
static uint32_t scl_ml_gbdt_rand(uint32_t *state) {
  *state = *state * 1103515245u + 12345u;
  return *state;
}

static double scl_ml_gbdt_rand_uniform(uint32_t *state) {
  return (double)scl_ml_gbdt_rand(state) / 4294967296.0;
}

SCL_WARN_UNUSED scl_error_t scl_ml_gbdt_new(scl_ml_gbdt_t **model,
                                            scl_ml_gbdt_params_t params) {
...
void scl_ml_gbdt_free(scl_ml_gbdt_t *model) {
  if (scl_unlikely(!model))
    return;
...
static int scl_ml_gbdt_train_regression_tree(
    scl_ml_gbdt_internal_tree_t *tree_out, const scl_ml_dataset_t *ds,
    const SCL_ML_FLOAT *targets_override, size_t n_features_total,
    scl_ml_gbdt_params_t *params, scl_allocator_t *persist_alloc) {

  scl_allocator_t *tree_arena = scl_alloc_arena_create(persist_alloc, 8192, 0, 0);
  if (!tree_arena)
    return -1;
...
SCL_WARN_UNUSED scl_error_t scl_ml_gbdt_fit(scl_ml_gbdt_t *model,
  ...
  model->trees = (scl_ml_gbdt_internal_tree_t *)scl_calloc(
      a, model->n_estimators, sizeof(scl_ml_gbdt_internal_tree_t),
      alignof(max_align_t));
  if (!model->trees)
    return SCL_ERR_OUT_OF_MEMORY;
  ...
  if (subsample_n < 1)
    subsample_n = 1;
  if (subsample_n > ds->n_rows)
    subsample_n = ds->n_rows;
  ...
  for (size_t round = 0; round < model->n_estimators; round++) {
    ...
    if (model->params.subsample < 1.0 - 1e-6) {
      size_t *sub_idx = (size_t *)scl_alloc(s, subsample_n * sizeof(size_t),
                                            alignof(max_align_t));
      if (!sub_idx) {
      ...
      for (size_t i = 0; i < subsample_n; i++) {
        sub_idx[i] =
            (size_t)(scl_ml_gbdt_rand_uniform(&ls) * (double)ds->n_rows);
        if (sub_idx[i] >= ds->n_rows)
          sub_idx[i] = ds->n_rows - 1;
      }

      SCL_ML_FLOAT *sub_data = (SCL_ML_FLOAT *)scl_alloc(
          s, subsample_n * ds->n_cols * sizeof(SCL_ML_FLOAT),
          alignof(max_align_t));
      ...
      for (size_t i = 0; i < subsample_n; i++) {
        ...
        for (size_t j = 0; j < ds->n_cols; j++)
          sub_data[i * ds->n_cols + j] = ds->data[src * ds->row_stride + j];
      ...
      int ret = scl_ml_gbdt_train_regression_tree(
          &model->trees[round], &subset_ds, sub_residuals, model->n_features,
          &model->params, a);
      ...
      int ret = scl_ml_gbdt_train_regression_tree(
          &model->trees[round], &subset_ds, residuals, model->n_features,
          &model->params, a);
    ...
    /* Batch predict this tree across all rows (trees outer, rows inner). */
    const scl_ml_gbdt_internal_tree_t *tr = &model->trees[round];
    ...
    for (size_t i = 0; i < n_rows; i++) {
      ...
      int node_idx = 0;
      while (1) {
        scl_ml_gbdt_node_t *n = &tr->nodes[node_idx];
        if (n->feature_idx < 0) {
          current_pred[i] += (SCL_ML_FLOAT)(lr * (double)n->value);
          break;
        }
...
SCL_WARN_UNUSED scl_error_t scl_ml_gbdt_predict(scl_ml_gbdt_t *model,
                                                const scl_ml_dataset_t *ds,
                                                SCL_ML_FLOAT *y_out) {
  ...
  for (size_t i = 0; i < n_rows; i++)
    y_out[i] = model->base_score;

  for (size_t t = 0; t < model->n_estimators; t++) {
    const scl_ml_gbdt_internal_tree_t *tr = &model->trees[t];
    for (size_t i = 0; i < n_rows; i++) {
      int node_idx = 0;
      const SCL_ML_FLOAT *row = ds->data + i * row_stride;
      while (1) {
        scl_ml_gbdt_node_t *n = &tr->nodes[node_idx];
        if (n->feature_idx < 0) {
          y_out[i] += (SCL_ML_FLOAT)(lr * (double)n->value);
          break;
        }
        if (row[(size_t)n->feature_idx] <= n->threshold)
          node_idx = n->left_child;
...
SCL_PURE size_t scl_ml_gbdt_get_n_features(const scl_ml_gbdt_t *model) {
  return model ? model->n_features : 0;
}

SCL_WARN_UNUSED scl_error_t scl_ml_gbdt_save(const scl_ml_gbdt_t *model,
  ...
  double lr = model->params.learning_rate;
  memcpy(buffer + off, &lr, sizeof(double));
  off += sizeof(double);
...
SCL_WARN_UNUSED scl_error_t scl_ml_gbdt_load(scl_ml_gbdt_t **model,
  ...
  m->trees = (scl_ml_gbdt_internal_tree_t *)scl_calloc(
      a, m->n_estimators, sizeof(scl_ml_gbdt_internal_tree_t),
      alignof(max_align_t));
  if (!m->trees) {
    scl_ml_gbdt_free(m);
    return SCL_ERR_OUT_OF_MEMORY;
  ...
  for (size_t i = 0; i < m->n_estimators; i++) {
    ...
    m->trees[i].nodes = (scl_ml_gbdt_node_t *)scl_calloc(
        a, m->trees[i].n_nodes, sizeof(scl_ml_gbdt_node_t),
        alignof(max_align_t));
    if (!m->trees[i].nodes) {
