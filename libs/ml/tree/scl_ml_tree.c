...
static int scl_ml_tree_ensure_capacity(scl_ml_tree_t *tree, size_t needed) {
  if (needed <= tree->n_capacity)
    return 0;
  size_t new_cap =
      tree->n_capacity ? tree->n_capacity * 2 : SCL_TREE_INIT_CAPACITY;
  while (new_cap < needed)
    new_cap *= 2;
  scl_ml_tree_node_t *p = (scl_ml_tree_node_t *)scl_realloc(
      tree->alloc, tree->nodes, tree->n_capacity * sizeof(scl_ml_tree_node_t),
      new_cap * sizeof(scl_ml_tree_node_t), SCL_ML_ALIGNMENT);
  if (!p)
    return -1;
...
static int scl_ml_tree_add_node(scl_ml_tree_t *tree, int *node_idx) {
...
static SCL_ML_FLOAT scl_ml_compute_gini(const SCL_ML_FLOAT *targets,
                                        const size_t *indices, size_t n,
                                        size_t n_classes, scl_allocator_t *a) {
  if (n == 0)
    return 0.0f;
  size_t *counts =
      (size_t *)scl_calloc(a, n_classes, sizeof(size_t), SCL_ML_ALIGNMENT);
  if (!counts)
    return 1.0f;
...
static SCL_ML_FLOAT scl_ml_compute_mse(const SCL_ML_FLOAT *targets,
                                       const size_t *indices, size_t n) {
  if (n == 0)
    return 0.0f;
  double mean = 0.0;
...
static SCL_ML_FLOAT scl_ml_compute_impurity(const SCL_ML_FLOAT *targets,
                                            const size_t *indices, size_t n,
...
static SCL_ML_FLOAT scl_ml_compute_leaf_value(const SCL_ML_FLOAT *targets,
                                              const size_t *indices, size_t n,
                                              int is_classifier,
                                              size_t n_classes,
                                              scl_allocator_t *a) {
...
static int scl_ml_sort_pair_cmp(const void *a, const void *b) {
...
static int scl_ml_tree_find_best_split(
    const scl_ml_dataset_t *ds, const size_t *indices, size_t n,
    int is_classifier, size_t n_classes, const size_t *feature_subset,
    size_t n_subset, size_t min_samples_leaf, double min_impurity_decrease,
    SCL_ML_FLOAT current_impurity, int *best_feature,
    SCL_ML_FLOAT *best_threshold, SCL_ML_FLOAT *best_impurity_reduction,
    scl_allocator_t *a) {
  ...
  *best_impurity_reduction = 0.0f;
  ...
  scl_ml_sort_pair_t *pairs = (scl_ml_sort_pair_t *)scl_alloc(
      a, n * sizeof(scl_ml_sort_pair_t), SCL_ML_ALIGNMENT);
  if (!pairs)
    return -1;
  ...
  if (is_classifier) {
    ...
    prefix_sum =
        (double *)scl_alloc(a, (n + 1) * sizeof(double), SCL_ML_ALIGNMENT);
    prefix_sqsum =
        (double *)scl_alloc(a, (n + 1) * sizeof(double), SCL_ML_ALIGNMENT);
  ...
  for (size_t f = 0; f < n_subset; f++) {
    ...
    if (is_classifier) {
      ...
      for (size_t split_i = 0; split_i < n - 1; split_i++) {
        ...
        double left_gini =
            1.0 - sum_c2_left / ((double)n_left * (double)n_left);
        double right_gini =
            1.0 - sum_c2_right / ((double)n_right * (double)n_right);
        ...
        if (reduction > best_feat_reduction) {
          ...
          best_feat_threshold =
              (pairs[split_i].value + pairs[split_i + 1].value) / 2.0f;
        }
      }
    } else {
...
static int scl_ml_tree_grow(scl_ml_tree_t *tree, const scl_ml_dataset_t *ds,
                            size_t *indices, size_t n, size_t depth,
                            const size_t *feature_subset, size_t n_subset) {

  int node_idx;
  if (scl_ml_tree_add_node(tree, &node_idx) != 0)
    return -1;
  ...
  node->impurity =
      scl_ml_compute_impurity(ds->targets, indices, n, tree->is_classifier,
                              tree->n_classes, tree->scratch);
  ...
  if (is_leaf) {
    node->value =
        scl_ml_compute_leaf_value(ds->targets, indices, n, tree->is_classifier,
                                  tree->n_classes, tree->scratch);
    tree->n_leaves++;
    return 0;
  }
  ...
  int ret = scl_ml_tree_find_best_split(
      ds, indices, n, tree->is_classifier, tree->n_classes, feature_subset,
      n_subset, tree->params.min_samples_leaf,
      tree->params.min_impurity_decrease, node->impurity, &best_feature,
      &best_threshold, &best_reduction, tree->scratch);
  ...
  size_t *left_idx = (size_t *)scl_alloc(tree->scratch, n_left * sizeof(size_t),
                                         SCL_ML_ALIGNMENT);
  size_t *right_idx = (size_t *)scl_alloc(
      tree->scratch, n_right * sizeof(size_t), SCL_ML_ALIGNMENT);
  ...
  if (scl_ml_tree_grow(tree, ds, left_idx, n_left, depth + 1, feature_subset,
                       n_subset) != 0) {
    return -1;
  }
  node = &tree->nodes[node_idx];
  ...
  if (scl_ml_tree_grow(tree, ds, right_idx, n_right, depth + 1, feature_subset,
                       n_subset) != 0) {
    return -1;
  }
  node = &tree->nodes[node_idx];
...
SCL_WARN_UNUSED scl_error_t scl_ml_tree_new(scl_ml_tree_t **model,
                                            scl_ml_tree_params_t params) {
  ...
  scl_ml_tree_t *t = (scl_ml_tree_t *)scl_calloc(
      alloc, 1, sizeof(scl_ml_tree_t), SCL_ML_ALIGNMENT);
  if (scl_unlikely(!t))
    return SCL_ERR_OUT_OF_MEMORY;
  t->params = params;
  t->alloc = alloc;
  t->scratch = scl_alloc_arena_create(alloc, 8192, 0, 0);
  if (!t->scratch) {
    scl_free(alloc, t);
    return SCL_ERR_OUT_OF_MEMORY;
  }
...
void scl_ml_tree_free(scl_ml_tree_t *model) {
  if (scl_unlikely(!model))
    return;
...
SCL_WARN_UNUSED scl_error_t scl_ml_tree_fit(scl_ml_tree_t *model,
  ...
  for (size_t i = 1; i < ds->n_rows; i++) {
    if (ds->targets[i] < min_target)
      min_target = ds->targets[i];
    if (ds->targets[i] > max_target)
      max_target = ds->targets[i];
  }
  ...
  size_t *indices = (size_t *)scl_alloc(t->scratch, ds->n_rows * sizeof(size_t),
                                        SCL_ML_ALIGNMENT);
  if (!indices)
    return SCL_ERR_OUT_OF_MEMORY;
  ...
  size_t n_features_use =
      t->params.max_features > 0 ? t->params.max_features : t->n_features;
  if (n_features_use > t->n_features)
    n_features_use = t->n_features;

  size_t *feature_subset = (size_t *)scl_alloc(
      t->scratch, n_features_use * sizeof(size_t), SCL_ML_ALIGNMENT);
  if (!feature_subset)
    return SCL_ERR_OUT_OF_MEMORY;
  for (size_t i = 0; i < n_features_use; i++)
    feature_subset[i] = i;

  int ret = scl_ml_tree_grow(t, ds, indices, ds->n_rows, 0, feature_subset,
                             n_features_use);
...
SCL_WARN_UNUSED scl_error_t scl_ml_tree_predict(scl_ml_tree_t *model,
                                                const scl_ml_dataset_t *ds,
                                                SCL_ML_FLOAT *y_out) {
...
SCL_PURE size_t scl_ml_tree_get_n_features(const scl_ml_tree_t *model) {
  return model ? model->n_features : 0;
}

SCL_PURE size_t scl_ml_tree_get_n_nodes(const scl_ml_tree_t *model) {
  return model ? model->n_nodes : 0;
}

SCL_PURE size_t scl_ml_tree_get_n_leaves(const scl_ml_tree_t *model) {
...
SCL_WARN_UNUSED scl_error_t scl_ml_tree_save(const scl_ml_tree_t *model,
  ...
  memcpy(buffer + off, &model->n_nodes, sizeof(size_t));
  off += sizeof(size_t);
  memcpy(buffer + off, &model->n_leaves, sizeof(size_t));
  off += sizeof(size_t);
  memcpy(buffer + off, &model->n_features, sizeof(size_t));
  off += sizeof(size_t);
  memcpy(buffer + off, &model->is_classifier, sizeof(int));
  off += sizeof(int);
  memcpy(buffer + off, model->nodes, nodes_bytes);
  off += nodes_bytes;
...
SCL_WARN_UNUSED scl_error_t scl_ml_tree_load(scl_ml_tree_t **model,
  ...
  memcpy(&t->n_nodes, buf + off, sizeof(size_t));
  off += sizeof(size_t);
  memcpy(&t->n_leaves, buf + off, sizeof(size_t));
  off += sizeof(size_t);
  memcpy(&t->n_features, buf + off, sizeof(size_t));
  off += sizeof(size_t);
  memcpy(&t->is_classifier, buf + off, sizeof(int));
  off += sizeof(int);

  t->n_capacity = t->n_nodes;
  size_t nodes_bytes = t->n_nodes * sizeof(scl_ml_tree_node_t);
  t->nodes = (scl_ml_tree_node_t *)scl_calloc(
      t->alloc, t->n_capacity, sizeof(scl_ml_tree_node_t), SCL_ML_ALIGNMENT);
  if (!t->nodes) {
    scl_ml_tree_free(t);
    return SCL_ERR_OUT_OF_MEMORY;
  }
