 ...
 * quadratic behaviour (the classic KMP guarantee — important against adversarial
 * inputs that defeat naive matching).
 *
 ...
 */
static scl_error_t kmp_core(scl_allocator_t *alloc, const char *text, size_t tlen,
...
static bool kmp_first_cb(size_t pos, void *user) { *(size_t *)user = pos; return false; }
...
scl_error_t scl_search_kmp(scl_allocator_t * alloc, const char * text, size_t tlen, const char * pat, size_t plen, size_t * pos)
{
    ...
    size_t found = 0;
    scl_error_t r = kmp_core(alloc, text, tlen, pat, plen, kmp_first_cb, &found, NULL);
    if (r == SCL_OK) *pos = found;
    return r;
}

scl_error_t scl_search_kmp_all(scl_allocator_t * alloc, const char * text, size_t tlen, const char * pat, size_t plen, scl_search_match_cb cb, void * user, size_t * out_count)
{
