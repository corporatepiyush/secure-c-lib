 ...
 * relaxation pushes at most once, so the heap never holds more than E+1
 * entries. Stale entries (a vertex pushed again after its distance improved)
 * are skipped on pop by comparing the popped key against the current best
 * distance.
 */
typedef struct {
  int64_t d;
...
scl_error_t scl_search_dijkstra(scl_allocator_t *alloc,
  ...
  if (scl_unlikely(scl_mul_overflow(cap, sizeof(dj_heap_entry_t), &capbytes)))
    return SCL_ERR_SIZE_OVERFLOW;
  dj_heap_entry_t *heap =
      (dj_heap_entry_t *)scl_alloc(alloc, capbytes, alignof(dj_heap_entry_t));
  if (scl_unlikely(heap == NULL))
    return SCL_ERR_OUT_OF_MEMORY;

  size_t hn = 0;
  heap[hn].d = 0;
  heap[hn].v = (size_t)start;
  hn++;
  ...
  while (hn > 0) {
    ...
    for (size_t e = scl_graph_adj_head(graph, top.v); e != SCL_GRAPH_NIL;) {
      ...
      int64_t nd;
      if (!__builtin_add_overflow(dist[top.v], (int64_t)w, &nd) &&
          nd < dist[to]) {
        dist[to] = nd;
        prev[to] = (int)top.v;
