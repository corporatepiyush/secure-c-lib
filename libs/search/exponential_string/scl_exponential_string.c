...
static scl_error_t binary_string_range(const char **strs, size_t lo, size_t hi,
                                       const char *key, size_t *idx) {
  while (lo <= hi) {
    size_t mid = lo + (hi - lo) / 2;
    if (scl_unlikely(!strs[mid]))
      return SCL_ERR_INVALID_ARG; /* guard NULL element */
...
scl_error_t scl_search_exponential_string(const char **SCL_RESTRICT strs,
                                          size_t count, const char *key,
                                          size_t *idx) {
  if (scl_unlikely(strs == NULL))
    return SCL_ERR_NULL_PTR;
  if (scl_unlikely(key == NULL))
    return SCL_ERR_NULL_PTR;
  if (scl_unlikely(idx == NULL))
    return SCL_ERR_NULL_PTR;
  if (scl_unlikely(count == 0))
    return SCL_ERR_EMPTY;

  if (scl_unlikely(!strs[0]))
    return SCL_ERR_INVALID_ARG;
  if (strcmp(strs[0], key) == 0) {
    *idx = 0;
    return SCL_OK;
  }
  if (count == 1)
