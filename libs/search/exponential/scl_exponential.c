...
/* Exponential search. O(log N). Doubles range then binary. Good for unbounded
 * arrays. */
...
static scl_error_t binary_search_range(const void *base, size_t lo, size_t hi,
...
scl_error_t scl_search_exponential_search(const void *base, size_t count,
                                          size_t elem_size, const void *key,
                                          scl_cmp_func_t cmp,
                                          size_t *out_index) {
  ...
  while (bound < count) {
    ...
    if (r >= 0) {
      ...
      if (hi >= count)
        hi = count - 1;
      return binary_search_range(base, bound / 2, hi, elem_size, key, cmp,
                                 out_index);
    }
    if (bound > (SIZE_MAX >> 1))
      break; /* overflow-safe doubling */
    bound *= 2;
  }
  return binary_search_range(base, bound / 2, count - 1, elem_size, key, cmp,
                             out_index);
}
