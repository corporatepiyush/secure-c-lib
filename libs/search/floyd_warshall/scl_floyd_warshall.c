...
scl_error_t scl_search_floyd_warshall(int n, const scl_edge_t *edges,
                                      size_t ecount, int64_t *dist) {
  if (scl_unlikely(edges == NULL))
  ...
  /* Guard against overflow: n*n must fit in size_t, and n*n*sizeof(int64_t)
   * must not overflow. */
  size_t sn = (size_t)n;
  if (scl_unlikely(sn > SIZE_MAX / sn))
    return SCL_ERR_SIZE_OVERFLOW;
  size_t nelems = sn * sn;
  size_t bytes;
  if (scl_unlikely(scl_mul_overflow(nelems, sizeof(int64_t), &bytes)))
    return SCL_ERR_SIZE_OVERFLOW;

  for (size_t i = 0; i < nelems; i++)
    dist[i] = (i % (sn + 1) == 0) ? 0 : INT64_MAX;
  ...
  for (size_t k = 0; k < sn; k++) {
    for (size_t i = 0; i < sn; i++) {
      ...
      for (size_t j = 0; j < sn; j++) {
        ...
        int64_t nd;
        if (__builtin_add_overflow(dik, dkj, &nd))
          continue;
        if (nd < dist[i * sn + j])
          dist[i * sn + j] = nd;
      }
