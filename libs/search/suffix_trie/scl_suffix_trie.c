...
static st_node *st_new_node(scl_suffix_trie_t *t, unsigned char edge) {
    st_node *n = (st_node *)scl_calloc(t->alloc, 1, sizeof(st_node), alignof(max_align_t));
...
static st_node *st_find_child(st_node *parent, unsigned char ch) {
    for (st_node *c = parent->child; c; c = c->sibling)
        if (c->edge == ch) return c;
    return NULL;
}

scl_error_t scl_suffix_trie_build(scl_allocator_t *alloc, scl_suffix_trie_t **out,
                                  const char *text, size_t len, size_t max_len)
{
    if (scl_unlikely(!alloc || !out)) return SCL_ERR_NULL_PTR;
    *out = NULL;
    if (scl_unlikely(!text && len > 0)) return SCL_ERR_NULL_PTR;
    if (max_len == 0) max_len = SCL_SUFFIX_TRIE_DEFAULT_MAX;
    if (scl_unlikely(len > max_len)) return SCL_ERR_SIZE_OVERFLOW;

    scl_suffix_trie_t *t = (scl_suffix_trie_t *)scl_calloc(alloc, 1, sizeof(*t), alignof(max_align_t));
    if (scl_unlikely(!t)) return SCL_ERR_OUT_OF_MEMORY;
    t->alloc = alloc;
    t->text_len = len;

    t->root = st_new_node(t, 0);
    if (scl_unlikely(!t->root)) { scl_suffix_trie_destroy(t); return SCL_ERR_OUT_OF_MEMORY; }
    ...
    for (size_t i = 0; i < len; i++) {
        ...
        for (size_t k = i; k < len; k++) {
            ...
            if (!child) {
                ...
                if (scl_unlikely(!child)) { scl_suffix_trie_destroy(t); return SCL_ERR_OUT_OF_MEMORY; }
...
static const st_node *st_walk(const scl_suffix_trie_t *t, const char *pat, size_t plen) {
    const st_node *cur = t->root;
    for (size_t k = 0; k < plen; k++) {
        unsigned char ch = (unsigned char)pat[k];
        const st_node *c = cur->child;
        while (c && c->edge != ch) c = c->sibling;
        if (!c) return NULL;
        cur = c;
    }
    return cur;
...
bool scl_suffix_trie_contains(const scl_suffix_trie_t *trie, const char *pat, size_t plen) {
    if (scl_unlikely(!trie || (!pat && plen > 0))) return false;
    if (plen == 0) return true;                  /* empty string is a substring */
    return st_walk(trie, pat, plen) != NULL;
}

size_t scl_suffix_trie_count(const scl_suffix_trie_t *trie, const char *pat, size_t plen) {
    if (scl_unlikely(!trie || (!pat && plen > 0))) return 0;
    if (plen == 0) return trie->text_len;        /* occurs at every position */
    const st_node *n = st_walk(trie, pat, plen);
    return n ? n->leaf_count : 0;
}

scl_error_t scl_suffix_trie_find_all(const scl_suffix_trie_t *trie, const char *pat, size_t plen,
                                     scl_search_match_cb cb, void *user, size_t *out_count)
{
    if (scl_unlikely(!trie)) return SCL_ERR_NULL_PTR;
    if (scl_unlikely(plen == 0)) return SCL_ERR_INVALID_ARG;
    if (scl_unlikely(!pat)) return SCL_ERR_NULL_PTR;

    const st_node *start = st_walk(trie, pat, plen);
    if (!start) { if (out_count) *out_count = 0; return SCL_ERR_NOT_FOUND; }
    ...
    size_t cap = 64, top = 0;
    const st_node **stk = (const st_node **)scl_alloc(trie->alloc, cap * sizeof(*stk), alignof(max_align_t));
    if (scl_unlikely(!stk)) return SCL_ERR_OUT_OF_MEMORY;
    ...
    while (top > 0 && !stopped) {
        ...
        if (n->is_leaf) {
            count++;
            if (cb && !cb(n->suffix_index, user)) { stopped = true; break; }
        }
        for (const st_node *c = n->child; c; c = c->sibling) {
            if (top == cap) {
                ...
                const st_node **ns = (const st_node **)scl_realloc(trie->alloc, stk,
                                       cap * sizeof(*stk), nbytes, alignof(max_align_t));
                if (scl_unlikely(!ns)) { scl_free(trie->alloc, stk); return SCL_ERR_OUT_OF_MEMORY; }
                stk = ns;
                cap = ncap;
            }
            stk[top++] = c;
