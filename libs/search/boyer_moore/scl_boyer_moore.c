...
/* Boyer-Moore pattern matcher. Sub-linear avg. Bad-character + good-suffix shift tables. */
 ...
 */
static scl_error_t bm_core(scl_allocator_t *alloc, const char *text, size_t tlen,
                           ...
                           scl_search_match_cb cb, void *user, size_t *out_count)
{
    ...
    if (scl_unlikely(plen > (size_t)INT_MAX)) return SCL_ERR_SIZE_OVERFLOW;
    ...
    if (match_shift < 1) match_shift = 1;
    ...
    while (i <= tlen - plen) {
        ...
        if (j < 0) {
            count++;
            result = SCL_OK;
            if (cb && !cb(i, user)) break;   /* caller asked to stop */
...
static bool bm_first_cb(size_t pos, void *user) { *(size_t *)user = pos; return false; }
...
scl_error_t scl_search_boyer_moore(scl_allocator_t * alloc, const char * text, size_t tlen, const char * pat, size_t plen, size_t * pos)
{
    ...
    size_t found = 0;
    scl_error_t r = bm_core(alloc, text, tlen, pat, plen, bm_first_cb, &found, NULL);
    if (r == SCL_OK) *pos = found;
    return r;
}

scl_error_t scl_search_boyer_moore_all(scl_allocator_t * alloc, const char * text, size_t tlen, const char * pat, size_t plen, scl_search_match_cb cb, void * user, size_t * out_count)
{
