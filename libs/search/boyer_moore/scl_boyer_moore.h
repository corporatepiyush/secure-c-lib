...
scl_error_t scl_search_boyer_moore(scl_allocator_t * alloc, const char * text, size_t tlen, const char * pat, size_t plen, size_t * pos) SCL_WARN_UNUSED;

/* Report every occurrence of pat in text (including overlapping ones) via cb,
 * building the shift tables once. *out_count (optional) receives the number of
 * matches. cb may be NULL to only count. Returns SCL_OK if at least one match
 * was found, SCL_ERR_NOT_FOUND if none, or an error. */
scl_error_t scl_search_boyer_moore_all(scl_allocator_t * alloc, const char * text, size_t tlen, const char * pat, size_t plen, scl_search_match_cb cb, void * user, size_t * out_count) SCL_WARN_UNUSED;
