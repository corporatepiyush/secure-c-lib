...
scl_error_t scl_search_interpolation_search(const int64_t *arr, size_t count,
                                            int64_t key, size_t *out_index) {
  ...
  while (lo <= hi && key >= arr[lo] && key <= arr[hi]) {
    ...
    double frac =
        ((double)key - (double)arr[lo]) / ((double)arr[hi] - (double)arr[lo]);
    if (frac < 0.0)
      frac = 0.0;
    else if (frac > 1.0)
      frac = 1.0;
