 ...
 *
 * Absolute upper bound on decompressed output. A 1 KB DEFLATE stream can
 * legally expand ~1000x, so decompressors must cap output independently
 * of input size. Callers pass max_out per call; 0 selects this default. */
#ifndef SCL_INFLATE_MAX_OUTPUT
#define SCL_INFLATE_MAX_OUTPUT ((size_t)1 << 30) /* 1 GiB */
#endif

/* ── Raw DEFLATE inflate (RFC 1951) ───────────────────────────────
 *
 * Decompress a bare DEFLATE stream (no gzip/zlib wrapper, no checksum).
 * Output is allocated via `alloc`; free with scl_free() or
 * scl_gzip_free_result(). `max_out` caps the decompressed size
 * (0 = SCL_INFLATE_MAX_OUTPUT); exceeding it returns
 * SCL_ERR_SIZE_OVERFLOW. */
scl_error_t scl_inflate_raw(scl_allocator_t *alloc, const void *src,
                            size_t src_len, void **out, size_t *out_len,
                            size_t max_out);

/* ── zlib decompress (RFC 1950) ───────────────────────────────────
 *
 * Decompress a zlib-wrapped DEFLATE stream (2-byte header + Adler-32
 * trailer) — the format carried by PDF /FlateDecode, PNG IDAT, and most
 * wire protocols. Verifies the header check bits and, when present, the
 * Adler-32 checksum. FDICT preset dictionaries are rejected with
 * SCL_ERR_UNSUPPORTED. Same allocation/cap contract as
 * scl_inflate_raw(). */
 ...
 * after freeing. */
static inline void scl_gzip_free_result(scl_allocator_t *alloc, void **out,
                                        size_t *out_len) {
  if (out && *out) {
    scl_free(alloc, *out);
    *out = NULL;
