...
{
    return bf ? bf->inserted : 0;
}

double scl_bloom_false_positive_rate(const scl_bloom_t *bf)
{
    if (!bf || bf->inserted == 0) return 0.0;
    double e = -((double)bf->num_hashes * (double)bf->inserted) / (double)bf->bit_count;
    return pow(1.0 - exp(e), (double)bf->num_hashes);
}
