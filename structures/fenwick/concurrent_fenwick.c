                              ...
                              void (*sub)(void *out, const void *a, const void *b))
{
    ...
    for (size_t i = 1; i <= n; i++)
        memcpy(fw->tree + i * element_size, src + (i - 1) * element_size, element_size);
...
scl_error_t scl_atomic_fenwick_prefix(const scl_atomic_fenwick_t *fw, size_t idx, void *out)
{
    ...
    while (i > 0) {
        if (first) {
            memcpy(acc, fw->tree + i * esize, esize);
            first = false;
        } else {
...
scl_error_t scl_atomic_fenwick_range_query(const scl_atomic_fenwick_t *fw, size_t l, size_t r, void *out)
{
    if (!fw || !out) return SCL_ERR_NULL_PTR;
    ...
    scl_error_t err = scl_atomic_fenwick_prefix(fw, r, out);
    if (err != SCL_OK) { spin_unlock((atomic_flag *)&fw->lock); return err; }

    if (l > 0) {
        err = scl_atomic_fenwick_prefix(fw, l - 1, fw->scratch);
        if (err != SCL_OK) { spin_unlock((atomic_flag *)&fw->lock); return err; }
