...
int main(void)
{
...

    {
        ...
        scl_test_group("bfs");
        SCL_EXPECT_OK(&tr, scl_search_breadth_first_search(a, &g, 0, visited));
        int cnt = 0;
        for (int i = 0; i < 6; i++) if (visited[i]) cnt++;
        SCL_EXPECT_EQ_I(&tr, 6, cnt);
        destroy_graph(&g);
    }
    {
        scl_graph_t g;
        ...
        (void)scl_search_breadth_first_search(a, &g, 0, visited);
        SCL_EXPECT_TRUE(&tr, visited[0] && visited[1] && visited[2] && !visited[3]);
        destroy_graph(&g);
    }
    {
        SCL_EXPECT_EQ_I(&tr, SCL_ERR_NULL_PTR, scl_search_breadth_first_search(a, NULL, 0, (bool*)(uintptr_t)1));
    }
    {
        scl_graph_t g;
        make_graph(&g, 3);
