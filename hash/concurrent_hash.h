#ifndef SCL_CONCURRENT_HASH_H
#define SCL_CONCURRENT_HASH_H

#include "../common/scl_concurrent_common.h"

#ifdef __GNUC__
#pragma GCC diagnostic push
#pragma GCC diagnostic ignored "-Wpedantic"
#endif

typedef size_t (*scl_hash_func_t)(const void *key, size_t len);

typedef struct scl_atomic_hash_entry {
    void *key;
    void *value;
    struct scl_atomic_hash_entry *next;
} scl_atomic_hash_entry_t;

typedef struct {
    atomic_flag lock;
} scl_atomic_hash_bucket_t;

typedef struct {
    scl_atomic_hash_entry_t **entries;
    scl_atomic_hash_bucket_t *buckets;
    size_t bucket_count;
    atomic_size_t count;
    scl_hash_func_t hash_func;
    size_t key_size;
    size_t value_size;
} scl_atomic_hash_t;

scl_error_t scl_atomic_hash_init(scl_allocator_t *alloc, scl_atomic_hash_t *ht, size_t key_size, size_t value_size,
                          size_t bucket_count, scl_hash_func_t hf) SCL_WARN_UNUSED;
void        scl_atomic_hash_destroy(scl_allocator_t *alloc, scl_atomic_hash_t *ht);
scl_error_t scl_atomic_hash_insert(scl_allocator_t *alloc, scl_atomic_hash_t *ht, const void *key, const void *value) SCL_WARN_UNUSED;
scl_error_t scl_atomic_hash_get(const scl_atomic_hash_t *ht, const void *key, void *out_value) SCL_WARN_UNUSED;
scl_error_t scl_atomic_hash_remove(scl_allocator_t *alloc, scl_atomic_hash_t *ht, const void *key) SCL_WARN_UNUSED;
bool        scl_atomic_hash_contains(const scl_atomic_hash_t *ht, const void *key);
size_t      scl_atomic_hash_count(const scl_atomic_hash_t *ht);

#ifdef __GNUC__
#pragma GCC diagnostic pop
#endif

#endif
