...
scl_error_t scl_stack_init(scl_stack_t *stack, size_t element_size, size_t initial_capacity)
{
...
void scl_stack_destroy(scl_stack_t *stack)
{
    if (stack) {
        free(stack->data);
        stack->data = NULL;
...
scl_error_t scl_stack_push(scl_stack_t *stack, const void *element)
{
    ...
    if (stack->count == stack->capacity) {
        ...
        size_t bytes;
        if (scl_mul_overflow(new_cap, stack->element_size, &bytes))
            return SCL_ERR_SIZE_OVERFLOW;
        unsigned char *tmp = realloc(stack->data, bytes);
